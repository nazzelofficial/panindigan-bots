...
export async function loadEvents(client: PanindiganClient): Promise<number> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const event: EventDefinition | undefined = (mod as Record<string, unknown>).default as EventDefinition | undefined
        ?? (mod as Record<string, unknown>).event as EventDefinition | undefined;
      ...
      const handler = (...args: unknown[]) => {
        Promise.resolve(event.execute(client, ...(args as never[]))).catch((err: unknown) => {
