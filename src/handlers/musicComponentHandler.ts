...
export function registerMusicComponentHandlers(client: PanindiganClient): void {
  // Music control buttons
  client.componentHandlers.set("music", async (interaction: ButtonInteraction) => {
    const parts = interaction.customId.split(":");
    const action = parts[1];
    
    if (!interaction.guildId) return;
    
    const player = (client.lavalink as any).players?.get(interaction.guildId);
    if (!player) {
  ...
  client.componentHandlers.set("queue", async (interaction: ButtonInteraction) => {
  ...
  client.componentHandlers.set("search", async (interaction: ButtonInteraction) => {
  ...
  client.componentHandlers.set("lyrics", async (interaction: ButtonInteraction) => {
    const parts = interaction.customId.split(":");
    const action = parts[1];
  ...
  // Filter buttons
  client.componentHandlers.set("filter", async (interaction: ButtonInteraction) => {
    const parts = interaction.customId.split(":");
