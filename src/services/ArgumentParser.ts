...
export class ArgumentParser {
   ...
   */
  static parse(input: string, expectedTypes?: Map<string, string>): ParseResult {
  ...
  private static inferType(value: string): string {
    ...
    if (/^<@&(\d+)>$/.test(value)) return "role";
    if (/^<#(\d+)>$/.test(value)) return "channel";
    return "string";
  }
   ...
   */
  static getSuggestions(input: string, commands: string[]): string[] {
   ...
   */
  static correctSpelling(input: string, commands: string[]): string | null {
    const lowerInput = input.toLowerCase();
    const threshold = 3;
