...
const recoveryState = new Map<string, RecoveryAttempt>();
 ...
 */
export function calculateBackoff(attempt: number, baseDelays: number[] = RETRY.DB_BACKOFF_MS): number {
...
export async function withRecovery<T>(
  ...
  const {
    ...
    baseDelays = RETRY.DB_BACKOFF_MS,
    onRetry,
    onFailure,
  } = options;
 ...
 */
export function getRecoveryState(key: string): RecoveryAttempt | undefined {
 ...
 */
export function clearRecoveryState(key: string): void {
  recoveryState.delete(key);
}
 ...
 */
export function openCircuitBreaker(key: string, resetTimeoutMs = 60_000): void {
 ...
 */
export function getDegradationLevel(): DegradationState {
...
export function isFeatureAvailable(
  feature: "ai" | "music" | "economy" | "moderation" | "general",
): boolean {
