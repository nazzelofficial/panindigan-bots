...
import { withHealthCheck, type HealthCheckResult } from "./recovery.js";
 ...
 */
async function checkDiscordGateway(client: PanindiganClient): Promise<HealthCheckResult> {
 ...
 */
async function checkLavalink(client: PanindiganClient): Promise<HealthCheckResult> {
  if (!client.lavalink) {
    return {
      healthy: false,
 ...
 */
export async function getSystemHealth(client: PanindiganClient): Promise<SystemHealth> {
  ...
  const overall: "operational" | "degraded" | "down" = hasDown
    ? "down"
    : hasDegraded
      ? "degraded"
      : "operational";
  ...
  // System metrics
  const mem = process.memoryUsage();
  const cpuPercent = (cpu.details?.percent as number) ?? 0;
 ...
 */
export function isSystemDegraded(health: SystemHealth): boolean {
