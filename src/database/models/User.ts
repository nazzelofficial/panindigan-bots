...
userSchema.methods.getGuildProfile = function (guildId: string) {
  let profile = this.guilds.find((g: any) => g.guildId === guildId);
  if (!profile) {
...
export const UserModel = ((mongoose.models["User"] as any) ?? model("User", userSchema));
