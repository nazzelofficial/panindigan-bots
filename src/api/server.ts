 ...
 */
export function startApiServer(client: PanindiganClient): void {
  ...
  app.use(async (req, res, next) => {
    ...
    await record.save();
    (req as any).apiGuildId = record.guildId;
    next();
  });

  app.get("/v1/guild", async (req, res) => {
    const guildId = (req as any).apiGuildId as string;
    const guild = (client as any).guilds.cache.get(guildId);
