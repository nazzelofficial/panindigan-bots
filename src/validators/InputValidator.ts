 ...
 */
export function validateString(value: unknown, options: StringValidationOptions = {}): string {
  const {
    minLength    = 0,
    maxLength    = VALIDATION.MAX_STRING_LENGTH,
    required     = false,
    fieldName    = "value",
    allowNewlines = true,
    stripHtml    = true,
...
export function truncateField(value: string, limit: number = EMBED_LIMITS.FIELD_VALUE, suffix = "…"): string {
  if (value.length <= limit) return value;
  return value.slice(0, limit - suffix.length) + suffix;
}

/** Truncate embed description to limit. */
export function truncateDescription(value: string, suffix = "…"): string {
  return truncateField(value, EMBED_LIMITS.DESCRIPTION as number, suffix);
}

/** Truncate embed title to limit. */
export function truncateTitle(value: string, suffix = "…"): string {
  return truncateField(value, EMBED_LIMITS.TITLE as number, suffix);
}
...
export function validateAttachment(
  ...
  const {
    maxBytes     = VALIDATION.MAX_ATTACHMENT_BYTES,
  ...
  if (attachment.size > maxBytes) {
    ...
    throw new ValidationError(
      `${fieldName} exceeds the maximum size of ${maxMB} MB`,
      fieldName,
      { size: attachment.size, maxBytes },
    );
  }
  ...
  if (mime && !allowedTypes.includes(mime)) {
    throw new ValidationError(
      `${fieldName} type "${mime}" is not allowed. Allowed: ${allowedTypes.join(", ")}`,
      fieldName,
      { contentType: mime },
    );
  }
...
export function validateMentionCount(content: string, maxMentions = 5): void {
  const count = (content.match(/<@[!&]?\d+>/g) ?? []).length
    + (content.match(/@(everyone|here)/g) ?? []).length;

  if (count > maxMentions) {
    throw new ValidationError(
      `Message contains too many mentions (${count}). Maximum allowed: ${maxMentions}`,
      "content",
      { count, maxMentions },
    );
  }
...
export function validatePositiveInt(value: unknown, fieldName = "number"): number {
