...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!code) { await ctx.reply({ embeds: [errorEmbed("Provide code to evaluate.")] }); return; }
    ...
    if (silent && !isError) { await ctx.reply({ embeds: [baseEmbed("success").setTitle("✅ Eval completed (silent)")] }); return; }

    const embed = baseEmbed(isError ? "danger" : "success")
      .setTitle(isError ? "❌ Eval Error" : "✅ Eval Result")
      .addFields(
        { name: "Input", value: `\`\`\`js\n${code.slice(0, 1000)}\`\`\``, inline: false },
