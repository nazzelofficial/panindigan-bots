...
import { SupportStaffModel } from '../../database/models/System';
...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === 'add') {
      ...
      let tag = userId;
      try { tag = (await client.users.fetch(userId)).tag; } catch { /* ignore */ }
    ...
    if (sub === 'remove') {
      ...
      if (!result) return interaction.editReply({ content: `❌ \`${userId}\` is not a support staff member.` });
      return interaction.editReply({ content: `✅ \`${userId}\` removed from support staff.` });
    }

    if (sub === 'list') {
      const staff = await SupportStaffModel.find({}).lean() as any[];

      if (staff.length === 0) return interaction.editReply({ content: 'ℹ️ No support staff members configured.' });

      const client = clientRegistry.get()!;
      const lines = await Promise.all(staff.map(async (s: any) => {
        let tag = s.userId;
        try { tag = (await client.users.fetch(s.userId)).tag; } catch { /* ignore */ }
    ...
    if (sub === 'activity') {
      const userId = interaction.options.getString('user_id', true);
      const member = await SupportStaffModel.findOne({ userId }).lean() as any;

      if (!member) return interaction.editReply({ content: `❌ \`${userId}\` is not a support staff member.` });

      const embed = new EmbedBuilder()
        .setTitle(`📋 Support Activity — \`${userId}\``)
        .setColor('#5865F2')
        .addFields(
          { name: 'Tickets Handled', value: (member.ticketsHandled ?? 0).toLocaleString(), inline: true },
          { name: 'DMs Sent', value: (member.dmsSent ?? 0).toLocaleString(), inline: true },
          { name: 'Added', value: `<t:${Math.floor(new Date(member.createdAt).getTime() / 1000)}:R>`, inline: true },
          { name: 'Added By', value: `<@${member.addedBy}>`, inline: true },
        )
        .setTimestamp();
