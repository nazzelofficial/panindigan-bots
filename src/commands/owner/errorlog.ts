...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub.setName('view')
        ...
        .addStringOption(o =>
          o.setName('id').setDescription('Error entry MongoDB ID').setRequired(true)))
    .addSubcommand(sub =>
      sub.setName('clear')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
        ...
        .setDescription(
          entries.length > 0
            ? entries.map((e: any, i: number) =>
                `\`${page * perPage + i + 1}.\` **[\`${e._id}\`]**\n  ${String(e.message ?? 'Unknown error').slice(0, 100)}\n  *<t:${Math.floor(new Date(e.createdAt).getTime() / 1000)}:R>*`
              ).join('\n\n')
            : '*No errors logged. 🎉*',
        )
    ...
    if (sub === 'view') {
      ...
      const entry = await ErrorLogModel.findById(id).lean() as any;

      if (!entry) return interaction.editReply({ content: `❌ Error entry \`${id}\` not found.` });

      const embed = new EmbedBuilder()
        .setTitle(`🔴 Error Entry`)
        .setColor('#ff0000')
        .addFields(
          { name: 'ID', value: `\`${entry._id}\``, inline: true },
          { name: 'Logged', value: `<t:${Math.floor(new Date(entry.createdAt).getTime() / 1000)}:F>`, inline: true },
