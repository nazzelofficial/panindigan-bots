...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    codes.forEach((c: any) => tierCounts[c.tier as keyof typeof tierCounts]++);
    
    const embed = new EmbedBuilder()
      .setTitle('💎 Unused Premium Codes')
      .setColor('#ffd700')
      .addFields(
        { name: 'Total Unused Codes', value: codes.length.toString(), inline: true },
      ...
      .setDescription(codes.slice(0, 25).map((c: any) => 
        `${c.code} - ${c.tier.toUpperCase()} (Created: ${c.createdAt?.toLocaleDateString()})`
      ).join('\n'))
      .setTimestamp();
