...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('type')
        ...
        .setRequired(true)
        .addChoices(
          { name: 'Messages (purge all text channels)', value: 'messages' },
          { name: 'Roles (delete all non-default roles)', value: 'roles' },
          { name: 'Channels (delete all channels)', value: 'channels' },
        ))
    .addStringOption(option =>
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const type = interaction.options.getString('type', true) as WipeType;
      ...
      if (type === 'messages') {
        for (const [, channel] of guild.channels.cache) {
          ...
          try {
            ...
            if (messages.size > 0) {
              await (channel as any).bulkDelete(messages, true);
