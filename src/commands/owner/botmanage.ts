...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s.setName("update").setDescription("git pull and exit (process manager restarts)"),
      )
      .addSubcommand((s) =>
        s
          .setName("maintenance")
  ...
  async execute(ctx) {
    ...
    if (sub === "maintenance") {
      ...
      if (action === "on") {
        await MaintenanceStateModel.findOneAndUpdate(
          { key: "singleton" },
          { $set: { enabled: true, reason: text || "Maintenance in progress" } },
          { upsert: true },
        );
        ...
        await MaintenanceStateModel.findOneAndUpdate(
          { key: "singleton" },
          { $set: { enabled: false, reason: null, message: null } },
          { upsert: true },
        );
    ...
    if (sub === "memory") {
      ...
      const embed = baseEmbed("info")
        ...
        .addFields(
          ...
          { name: "System Free", value: `${(free / 1024 / 1024 / 1024).toFixed(2)} GB`, inline: true },
          { name: "System Total", value: `${(total / 1024 / 1024 / 1024).toFixed(2)} GB`, inline: true },
        );
      await ctx.reply({ embeds: [embed] });
      return;
    }
    ...
    if (sub === "cpu") {
      ...
      const embed = baseEmbed("info")
        ...
        .addFields(
          ...
          { name: "Load Avg (1m)", value: `${load[0]?.toFixed(2)}`, inline: true },
          { name: "Load Avg (5m)", value: `${load[1]?.toFixed(2)}`, inline: true },
    ...
    if (sub === "systemlogs") {
      ...
      if (action === "view") {
        ...
        if ((channel as any)?.isTextBased?.()) {
          await (channel as any).send({ files: [{ attachment: buf, name: latest }] });
    ...
    await ctx.reply({
      embeds: [
        errorEmbed(
          "Unknown subcommand. Use: restart | shutdown | update | maintenance | changelog | botstats | memory | cpu | envcheck | featureflag | systemlogs | debug | reload | reloadall",
        ),
      ],
    });
  },
