...
async function checkDiscordGateway(client: any): Promise<HealthResult> {
  const ping = client.ws.ping;
  if (ping < 0) return { name: 'Discord Gateway', status: 'down', detail: 'No WS ping' };
  if (ping > 500) return { name: 'Discord Gateway', status: 'degraded', latencyMs: ping, detail: 'High latency' };
  return { name: 'Discord Gateway', status: 'ok', latencyMs: ping };
}

async function checkShards(client: any): Promise<HealthResult> {
...
async function checkMemory(): Promise<HealthResult> {
...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    let lavalinkResult: HealthResult;
    try {
      const mgr = (client as any).lavalink;
