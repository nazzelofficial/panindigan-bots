...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const embed = EmbedFactory.base("primary")
      ...
      .addFields(
        { name: "✅ Both Prefix & Slash", value: `${report.both.length} commands`, inline: true },
        { name: "⚠️ Prefix Only", value: `${report.prefixOnly.length} commands`, inline: true },
        { name: "⚠️ Slash Only", value: `${report.slashOnly.length} commands`, inline: true },
      );
