...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === 'missing') {
      ...
      const enStrings = await TranslationModel.find({ language: 'en' }).lean() as any[];
      const langStrings = await TranslationModel.find({ language: lang }).lean() as any[];
      const langKeys = new Set(langStrings.map((s: any) => s.key));
      const missing = enStrings.filter((s: any) => !langKeys.has(s.key)).map((s: any) => s.key);
    ...
    if (sub === 'export') {
      const lang = interaction.options.getString('lang', true).toLowerCase();
      const strings = await TranslationModel.find({ language: lang }).lean() as any[];
      ...
      for (const s of strings) obj[s.key] = s.value;
