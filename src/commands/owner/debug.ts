  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (off === true) {
      await SystemModel.findOneAndUpdate(
        {},
        { debugMode: false, debugModule: null },
        { upsert: true }
      );
      ...
      await SystemModel.findOneAndUpdate(
        {},
        { debugMode: true, debugModule: module },
        { upsert: true }
      );
      ...
      if ((system as any)?.debugMode) {
        await interaction.reply({ content: `🔍 Debug mode is ON for module: ${(system as any)?.debugModule ?? 'unknown'}`, ephemeral: true });
      } else {
