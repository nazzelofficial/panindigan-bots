...
import { TempRoleModel } from "../../database/models/Community.js";
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (botMember && guild.roles.cache.get(role.id)!.position >= botMember.roles.highest.position) {
      await ctx.reply({ embeds: [errorEmbed("I can't assign a role higher than or equal to my highest role.")] }); return;
    }
    ...
    if (durationStr) {
      ...
      if (!ms || ms < 5_000) { await ctx.reply({ embeds: [errorEmbed("Invalid duration. Examples: `10m`, `2h`, `1d`. Minimum: 5s.")] }); return; }
