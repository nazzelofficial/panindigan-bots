...
const command: CommandDefinition = {
  ...
  registerComponents(client) {
    client.componentHandlers.set("selectrole", async (interaction) => {
      ...
      const panelRoleIds = (panel.options as any[]).map((o: any) => o.roleId);
  ...
  async execute(ctx) {
    ...
    if (sub === "create") {
      ...
      await msg.edit({ components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu)] });
      ...
      if ((panel.options as any[]).length >= 25) { await ctx.reply({ embeds: [errorEmbed("Max 25 options per panel.")] }); return; }
      if ((panel.options as any[]).some((o: any) => o.roleId === role.id)) { await ctx.reply({ embeds: [errorEmbed("That role is already in the panel.")] }); return; }

      (panel.options as any[]).push({ roleId: role.id, label, description: description ?? null, emoji: emoji ?? null });
      await panel.save();
      ...
      if (msg) {
        const opts = (panel.options as any[]).map((o: any) => {
          ...
          if (o.emoji) try { opt.setEmoji(o.emoji); } catch {}
          return opt;
        });
        const menu = new StringSelectMenuBuilder()
          ...
          .setMinValues(0)
          .setMaxValues(Math.min((panel as any).maxValues ?? 1, opts.length))
          .addOptions(opts);
        await msg.edit({ components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu)] }).catch(() => {});
      }
      ...
      const messageId = ctx.isSlash ? ctx.interaction!.options.getString("messageid", true) : ctx.args[1];
      const role = ctx.isSlash ? ctx.interaction!.options.getRole("role", true) : guild.roles.cache.get(ctx.args[2]?.replace(/\D/g, "") ?? "");
      if (!messageId || !role) { await ctx.reply({ embeds: [errorEmbed("Provide message ID and role.")] }); return; }

      const panel = await SelectRolePanelModel.findOne({ guildId: guild.id, messageId });
      if (!panel) { await ctx.reply({ embeds: [errorEmbed("Panel not found.")] }); return; }

      const before = (panel.options as any[]).length;
      (panel as any).options = (panel.options as any[]).filter((o: any) => o.roleId !== role.id);
      if ((panel.options as any[]).length === before) { await ctx.reply({ embeds: [errorEmbed("That role wasn't found in this panel.")] }); return; }
      await panel.save();
      ...
      const msg = await ch?.messages.fetch(messageId).catch(() => null);
      if (msg) {
        if ((panel.options as any[]).length === 0) {
          const menu = new StringSelectMenuBuilder()
            .setCustomId(`selectrole:${messageId}`)
            .setPlaceholder(panel.placeholder)
