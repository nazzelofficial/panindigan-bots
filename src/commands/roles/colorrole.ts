...
const command: CommandDefinition = {
  ...
  registerComponents(client) {
    client.componentHandlers.set("colorrole", async (interaction) => {
      ...
      if (!panel) { await interaction.reply({ content: "Color role panel not found.", ephemeral: true }); return; }
  ...
  async execute(ctx) {
    ...
    if (sub === "setup") {
      ...
      if (!(ch as any)?.isTextBased?.()) { await ctx.reply({ embeds: [errorEmbed("Invalid channel.")] }); return; }

      let panel = await ColorRoleModel.findOne({ guildId: guild.id });
      const roleIds: string[] = (panel?.roleIds as string[]) ?? [];
      ...
      const msg = await (ch as any).send({ embeds: [embed], components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu)] });

      if (panel) {
        panel.channelId = (ch as any).id;
        panel.messageId = msg.id;
        await panel.save();
      } else {
        await ColorRoleModel.create({ guildId: guild.id, channelId: (ch as any).id, messageId: msg.id, roleIds: [] });
      }
      ...
      const panel = await ColorRoleModel.findOneAndUpdate(
        { guildId: guild.id },
      ...
      if (panel.channelId && panel.messageId) {
        ...
        if (msg) {
          const opts = [
            ...
            ...(panel.roleIds as string[]).map((id) => {
      ...
      const panel = await ColorRoleModel.findOne({ guildId: guild.id }).lean();
      if (!panel || !(panel.roleIds as string[]).length) { await ctx.reply({ embeds: [infoEmbed("No color roles configured. Use `/colorrole add` to add some.")] }); return; }
      const embed = baseEmbed("primary").setTitle("🎨 Color Roles").setDescription(
        (panel.roleIds as string[]).map((id) => {
          const role = guild.roles.cache.get(id);
      ...
      if (!panel) { await ctx.reply({ embeds: [infoEmbed("Color role picker is not configured.")] }); return; }
      if (panel.channelId && panel.messageId) {
        const ch = guild.channels.cache.get(panel.channelId) as any;
        const msg = await ch?.messages.fetch(panel.messageId).catch(() => null);
