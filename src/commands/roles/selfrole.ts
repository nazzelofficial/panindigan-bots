...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const selfRoles: string[] = (guildDoc as any)?.selfRoleIds ?? [];

    if (sub === "add") {
      // Admin-only action
      if (!member?.permissions.has(PermissionFlagsBits.ManageRoles)) {
        await ctx.reply({ embeds: [errorEmbed("You need **Manage Roles** permission to add self-assignable roles.")] }); return;
      }
      ...
      if (selfRoles.includes(role.id)) { await ctx.reply({ embeds: [infoEmbed(`${role} is already self-assignable.`)] }); return; }
      ...
      if (!member?.permissions.has(PermissionFlagsBits.ManageRoles)) {
        await ctx.reply({ embeds: [errorEmbed("You need **Manage Roles** permission to remove self-assignable roles.")] }); return;
      }
      const role = ctx.isSlash ? ctx.interaction!.options.getRole("role", true) : guild.roles.cache.get(ctx.args[1]?.replace(/\D/g, "") ?? "");
      if (!role) { await ctx.reply({ embeds: [errorEmbed("Role not found.")] }); return; }
      ...
      if (!selfRoles.length) { await ctx.reply({ embeds: [infoEmbed("No self-assignable roles configured. Admins can use `/selfrole add` to add some.")] }); return; }
      ...
      const role = ctx.isSlash ? ctx.interaction!.options.getRole("role", true) : guild.roles.cache.get(ctx.args[1]?.replace(/\D/g, "") ?? "");
      if (!role) { await ctx.reply({ embeds: [errorEmbed("Role not found.")] }); return; }
      if (!selfRoles.includes(role.id)) { await ctx.reply({ embeds: [errorEmbed(`${role} is not a self-assignable role. Use \`/selfrole list\` to see available roles.`)] }); return; }
      if (!member) { await ctx.reply({ embeds: [errorEmbed("Could not find your server profile.")] }); return; }
      if (member.roles.cache.has(role.id)) { await ctx.reply({ embeds: [infoEmbed(`You already have ${role}. Use \`/selfrole drop\` to remove it.`)] }); return; }
      ...
      const role = ctx.isSlash ? ctx.interaction!.options.getRole("role", true) : guild.roles.cache.get(ctx.args[1]?.replace(/\D/g, "") ?? "");
      if (!role) { await ctx.reply({ embeds: [errorEmbed("Role not found.")] }); return; }
      if (!selfRoles.includes(role.id)) { await ctx.reply({ embeds: [errorEmbed(`${role} is not a self-assignable role.`)] }); return; }
      if (!member) { await ctx.reply({ embeds: [errorEmbed("Could not find your server profile.")] }); return; }
      if (!member.roles.cache.has(role.id)) { await ctx.reply({ embeds: [infoEmbed(`You don't have ${role}.`)] }); return; }
