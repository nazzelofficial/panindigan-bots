...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!durationStr) { await ctx.reply({ embeds: [errorEmbed("Provide a duration (e.g. `30m`, `1h`, `1d`).")] }); return; }
    const durationMs = parseDuration(durationStr);
    if (!durationMs || durationMs > 28 * 24 * 60 * 60 * 1000) { await ctx.reply({ embeds: [errorEmbed("Invalid duration. Max: 28 days.")] }); return; }

    const member = await guild.members.fetch(targetId).catch(() => null);
    if (!member) { await ctx.reply({ embeds: [errorEmbed("Member not found.")] }); return; }
    if (!member.moderatable) { await ctx.reply({ embeds: [errorEmbed("I cannot timeout this member.")] }); return; }
    ...
    await sendLogEvent(guild.id, "mute", () =>
      baseEmbed("warning").setTitle("🔇 Member Timed Out").addFields(
