...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!member || !member.bannable) { await ctx.reply({ embeds: [errorEmbed("Member not found or cannot be banned.")] }); return; }
    ...
    await sendLogEvent(guild.id, "ban", () =>
      baseEmbed("warning").setTitle("🧹 Member Softbanned").addFields(
        { name: "User", value: `${member.user.username} (<@${targetId}>)`, inline: true },
