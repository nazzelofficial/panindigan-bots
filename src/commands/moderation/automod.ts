...
const FEATURE_MAP: Record<string, { field: string; label: string; premium?: true }> = {
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("enable").setDescription("Enable an automod protection feature")
          .addStringOption((o) =>
            o.setName("feature").setDescription("Feature to enable").setRequired(true)
              .addChoices(...Object.entries(FEATURE_MAP).map(([v, { label }]) => ({ name: label, value: v }))),
          ),
      )
      .addSubcommand((s) =>
        s.setName("disable").setDescription("Disable an automod protection feature")
          .addStringOption((o) =>
            o.setName("feature").setDescription("Feature to disable").setRequired(true)
              .addChoices(...Object.entries(FEATURE_MAP).map(([v, { label }]) => ({ name: label, value: v }))),
          ),
      )
      ...
      .addSubcommand((s) =>
        s.setName("whitelist").setDescription("Add, remove, or list automod whitelist entries")
          .addStringOption((o) =>
            o.setName("action").setDescription("Action to perform").setRequired(true)
              .addChoices({ name: "Add", value: "add" }, { name: "Remove", value: "remove" }, { name: "List", value: "list" }),
          )
          .addStringOption((o) =>
            o.setName("type").setDescription("Type of exemption").setRequired(false)
      ...
      .addSubcommand((s) =>
        s.setName("badword").setDescription("Add or remove a word from the bad-word filter")
          .addStringOption((o) =>
      ...
      .addSubcommand((s) =>
        s.setName("antimention").setDescription("Set the maximum number of user mentions allowed per message (0 = disable)")
          .addIntegerOption((o) =>
            o.setName("count").setDescription("Max mentions (0 = disabled, 2–50)").setRequired(true).setMinValue(0).setMaxValue(50),
          ),
      )
      ...
      .addSubcommand((s) =>
        s.setName("antiraid").setDescription("Toggle anti-raid protection ⭐")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
      ...
      .addSubcommand((s) =>
        s.setName("antiinvite").setDescription("Toggle blocking of Discord invite links")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antinsfw").setDescription("Toggle AI-powered NSFW content detection ⭐")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antiscam").setDescription("Toggle AI-powered scam/phishing link detection ⭐")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antitoxicity").setDescription("Toggle AI-powered toxicity message filter ⭐")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antialt").setDescription("Toggle blocking of alt/new accounts from joining ⭐")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antibot").setDescription("Toggle blocking of unauthorized bot additions")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antiflood").setDescription("Toggle anti-message-flood protection")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antimassjoin").setDescription("Toggle mass-join raid detection ⭐")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("antighostping").setDescription("Toggle detection and logging of ghost pings")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("anticaps").setDescription("Toggle anti-caps filter")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      ),
  ...
  async execute(ctx) {
    ...
    if (FEATURE_MAP[sub]) {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { [`automod.${field}`]: enabled, "automod.enabled": true } },
        { upsert: true },
      );
      await ctx.reply({
    ...
    if (sub === "enable" || sub === "disable") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { [`automod.${feature.field}`]: enabled, "automod.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "antimention") {
      ...
      if (isNaN(count)) { await ctx.reply({ embeds: [errorEmbed("Provide a valid number (0 to disable).")] }); return; }
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.antiMentionLimit": count, "automod.enabled": true } },
        { upsert: true },
      );
      await ctx.reply({
        embeds: [
    ...
    if (sub === "status" || sub === "config") {
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          { name: "🔗 Anti-Link",           value: fmt(am.antiLink),      inline: true },
          { name: "📨 Anti-Invite",          value: fmt(am.antiInvite),    inline: true },
          { name: "⚡ Anti-Spam ⭐",          value: fmt(am.antiSpam),      inline: true },
          { name: "🚨 Anti-Raid ⭐",          value: fmt(am.antiRaid),      inline: true },
          { name: "🔠 Anti-Caps",            value: fmt(am.antiCaps),      inline: true },
          { name: "🔞 Anti-NSFW (AI) ⭐",    value: fmt(am.antiNsfw),      inline: true },
          { name: "🎣 Anti-Scam (AI) ⭐",    value: fmt(am.antiScam),      inline: true },
          { name: "☠️ Anti-Toxicity ⭐",     value: fmt(am.antiToxicity),  inline: true },
          { name: "👤 Anti-Alt ⭐",           value: fmt(am.antiAlt),       inline: true },
          { name: "🤖 Anti-Bot",             value: fmt(am.antiBot),       inline: true },
          { name: "💬 Anti-Flood",           value: fmt(am.antiFlood),     inline: true },
          { name: "👥 Anti-Mass Join ⭐",     value: fmt(am.antiMassJoin),  inline: true },
          { name: "👻 Anti-Ghost Ping",      value: fmt(am.antiGhostPing), inline: true },
          { name: "📣 Anti-Mention",
            value: am.antiMentionLimit > 0 ? `${on} (max ${am.antiMentionLimit})` : off, inline: true },
          { name: "🚫 Bad-Word Filter",
            value: (am.badWords?.length ?? 0) > 0 ? `${on} (${am.badWords.length} words)` : off, inline: true },
        );
      const domains: string[] = am.linkWhitelistDomains ?? [];
      if (domains.length) embed.addFields({ name: "Whitelisted Domains", value: domains.join(", "), inline: false });
    ...
    if (sub === "whitelist") {
      ...
      if (action === "list") {
        ...
        const embed = baseEmbed("primary")
          ...
          .addFields(
            { name: "Users",    value: (am.whitelistUsers    ?? []).map((i: string) => `<@${i}>`).join(", ")    || "None", inline: false },
            { name: "Roles",    value: (am.whitelistRoles    ?? []).map((i: string) => `<@&${i}>`).join(", ")   || "None", inline: false },
            { name: "Channels", value: (am.whitelistChannels ?? []).map((i: string) => `<#${i}>`).join(", ")    || "None", inline: false },
          );
        await ctx.reply({ embeds: [embed] });
        return;
      }
      ...
      // add / remove
      const fieldMap: Record<string, string> = { user: "whitelistUsers", role: "whitelistRoles", channel: "whitelistChannels" };
    ...
    if (sub === "badword") {
      ...
      if (!word) { await ctx.reply({ embeds: [errorEmbed("Provide a word to add or remove.")] }); return; }
      const op = action === "add" ? "$addToSet" : "$pull";
    ...
    await ctx.reply({
      embeds: [errorEmbed(
        "Unknown subcommand. Use: enable | disable | status | config | whitelist | badword | badwords | antimention | " +
