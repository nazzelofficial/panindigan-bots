...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!targetId) { await ctx.reply({ embeds: [errorEmbed("Specify a member.")] }); return; }

    const member = await guild.members.fetch(targetId).catch(() => null);
    if (!member) { await ctx.reply({ embeds: [errorEmbed("Member not found.")] }); return; }
    if (!member.moderatable) { await ctx.reply({ embeds: [errorEmbed("I cannot moderate this member — they may outrank me.")] }); return; }
    ...
    if (sub === "add") {
      ...
      await sendLogEvent(guild.id, "timeout", () =>
        baseEmbed("warning")
      ...
      await sendLogEvent(guild.id, "untimeout", () =>
        baseEmbed("success")
