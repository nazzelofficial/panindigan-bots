...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!channel?.isTextBased() || channel.type !== ChannelType.GuildText) {
      await ctx.reply({ embeds: [errorEmbed("Use this in a text channel.")] }); return;
    }
    ...
    if (!amount || amount < 1 || amount > 100) { await ctx.reply({ embeds: [errorEmbed("Amount must be between 1 and 100.")] }); return; }
    ...
    if (filter === "embeds") filtered = filtered.filter((m) => m.embeds.length > 0);
