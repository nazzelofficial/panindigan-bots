...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "list") {
      ...
      if (!warns.length) { await ctx.reply({ embeds: [infoEmbed(`${target.username} has no active warnings.`)] }); return; }
      const embed = baseEmbed("warning")
        .setTitle(`⚠️ Warnings — ${target.username} (${warns.length})`)
        .setThumbnail(target.displayAvatarURL())
        .setDescription(warns.map((w: any) => `**Case #${w.caseId}** — ${w.reason ?? "No reason"}\n> By <@${w.moderatorId}> · <t:${Math.floor(new Date(w.createdAt).getTime() / 1000)}:R>`).join("\n\n").slice(0, 4000));
      ...
      if (!updated) { await ctx.reply({ embeds: [errorEmbed(`Warning case #${caseNum} not found.`)] }); return; }
