...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!targetId) { await ctx.reply({ embeds: [errorEmbed("Please specify a member to warn.")] }); return; }
    if (!reason) { await ctx.reply({ embeds: [errorEmbed("Please provide a reason for the warning.")] }); return; }
    ...
    const escalation: any[] = (config as any).moderation?.warnEscalation ?? [];
    const escalationRule = escalation.find((e: any) => e.count === warnCount);
    let escalationMsg = "";
    if (escalationRule) {
      try {
