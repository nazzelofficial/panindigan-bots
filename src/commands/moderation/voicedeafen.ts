...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (member.voice.serverDeaf) return interaction.reply({ content: '❌ That user is already server-deafened.', ephemeral: true });

    await member.voice.setDeaf(true, reason);

    await sendLogEvent(guild.id, 'voicedeafen', () =>
      baseEmbed('warning')
