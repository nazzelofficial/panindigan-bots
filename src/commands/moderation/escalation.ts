...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    // Find current stage
    const currentRule = [...escalation].reverse().find((e: any) => warnCount >= e.count);
    const nextRule = escalation.find((e: any) => e.count > warnCount);
    ...
    // Escalation ladder
    const ladder = escalation.map((e: any) => {
