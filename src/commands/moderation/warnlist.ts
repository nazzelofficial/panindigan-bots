...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!warnings.length) { await ctx.reply({ embeds: [infoEmbed("No active warnings in this server.")] }); return; }
    ...
    if (!page_entries.length) { await ctx.reply({ embeds: [errorEmbed(`Page ${page} doesn't exist. There are only ${totalPages} page(s).`)] }); return; }

    const embed = baseEmbed("warning")
      .setTitle("⚠️ Warned Members")
      .setDescription(
        page_entries.map(([userId, count], i) => {
