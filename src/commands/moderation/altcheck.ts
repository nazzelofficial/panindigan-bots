...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (suspicionPoints.length >= 3 || ageDays < 7) riskLevel = "🔴 High";
    else if (suspicionPoints.length === 2) riskLevel = "🟡 Medium";
    else if (suspicionPoints.length === 1) riskLevel = "🟠 Moderate";
    ...
    const embed = baseEmbed(suspicionPoints.length >= 2 ? "danger" : "warning")
      ...
      .addFields(
        ...
        { name: "Account Created", value: `<t:${Math.floor(createdAt / 1000)}:F>`, inline: true },
        { name: "Has Avatar", value: hasAvatar ? "✅ Yes" : "❌ No", inline: true },
        {
          name: "Joined Server",
