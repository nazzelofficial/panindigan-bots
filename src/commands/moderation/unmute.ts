...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!targetId) { await ctx.reply({ embeds: [errorEmbed("Provide a member to unmute.")] }); return; }
    ...
    if (!member.moderatable) { await ctx.reply({ embeds: [errorEmbed("I cannot moderate this member.")] }); return; }
    ...
    await sendLogEvent(guild.id, "unmute", () =>
      baseEmbed("success")
