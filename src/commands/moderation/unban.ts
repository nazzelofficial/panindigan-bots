...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!targetId) { await ctx.reply({ embeds: [errorEmbed("Provide a user ID.")] }); return; }

    const ban = await guild.bans.fetch(targetId).catch(() => null);
    if (!ban) { await ctx.reply({ embeds: [errorEmbed("That user is not banned.")] }); return; }
    ...
    await sendLogEvent(guild.id, "banRemove", () =>
      baseEmbed("success").setTitle("✅ Member Unbanned").addFields(
        { name: "User", value: `${ban.user.username} (<@${targetId}>)`, inline: true },
