...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "me") {
      ...
      if (!self) { await ctx.reply({ embeds: [errorEmbed("Could not find you in this server.")] }); return; }
      if (!self.manageable) { await ctx.reply({ embeds: [errorEmbed("I cannot change your nickname — my role may be below yours.")] }); return; }
    ...
    if (!targetId) { await ctx.reply({ embeds: [errorEmbed("Provide a member.")] }); return; }

    const member = await guild.members.fetch(targetId).catch(() => null);
    if (!member) { await ctx.reply({ embeds: [errorEmbed("Member not found.")] }); return; }
    if (!member.manageable) { await ctx.reply({ embeds: [errorEmbed("I cannot change this member's nickname — their role may be higher than mine.")] }); return; }

    if (sub === "set") {
      const nick = ctx.isSlash ? ctx.interaction!.options.getString("nickname", true) : ctx.args.slice(2).join(" ");
      if (!nick) { await ctx.reply({ embeds: [errorEmbed("Provide a new nickname.")] }); return; }
