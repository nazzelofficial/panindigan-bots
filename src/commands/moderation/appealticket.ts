  ...
  async execute(ctx) {
    ...
    if (sub === "setup") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { appealChannelId: channel.id } },
        { upsert: true },
      );
      ...
      if (!appeals.length) { await ctx.reply({ embeds: [infoEmbed("No pending appeals.")] }); return; }
      ...
      if (!appeal || appeal.guildId !== guild.id) { await ctx.reply({ embeds: [errorEmbed("Appeal not found.")] }); return; }
      if (appeal.status !== "pending") { await ctx.reply({ embeds: [errorEmbed(`Appeal is already **${appeal.status}**.`)] }); return; }
      ...
      if (modCase?.type === "ban" || modCase?.type === "tempban") {
        ...
        if (member) await member.timeout(null, `Appeal approved by ${ctx.userId}`).catch(() => {});
      }
      ...
      await sendLogEvent(guild.id, "appealapprove", () =>
        baseEmbed("success").setTitle("✅ Appeal Approved").addFields(
          { name: "User", value: `<@${appeal.userId}>`, inline: true },
          { name: "Reviewed By", value: `<@${ctx.userId}>`, inline: true },
          { name: "Appeal Reason", value: appeal.reason.slice(0, 500), inline: false },
        ),
      );
      ...
      if (!reason) { await ctx.reply({ embeds: [errorEmbed("Provide a reason for your appeal.")] }); return; }

      // Check if user already has a pending appeal
      const existing = await AppealTicketModel.findOne({ guildId: guild.id, userId: ctx.userId, status: "pending" });
      if (existing) { await ctx.reply({ embeds: [errorEmbed("You already have a pending appeal. Wait for it to be reviewed before submitting another.")] }); return; }
      ...
      const appealChannelId = (guildDoc as any)?.appealChannelId;
      if (appealChannelId) {
        const ch = guild.channels.cache.get(appealChannelId) as any;
        if (ch?.isTextBased()) {
          const embed = baseEmbed("warning")
            .setTitle("📋 New Appeal Submitted")
            .addFields(
              { name: "User", value: `<@${ctx.userId}>`, inline: true },
              { name: "Appeal ID", value: `\`${(appeal as any)._id}\``, inline: true },
              { name: "Case ID", value: caseId ? String(caseId) : "Not specified", inline: true },
