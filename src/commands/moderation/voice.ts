...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("kick").setDescription("Disconnect a member from a voice channel")
      ...
      .addSubcommand((s) =>
        s.setName("unmute").setDescription("Remove server-mute from a member in voice")
          .addUserOption((o) => o.setName("user").setDescription("Member").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("deafen").setDescription("Server-deafen a member in voice")
          .addUserOption((o) => o.setName("user").setDescription("Member").setRequired(true))
          .addStringOption((o) => o.setName("reason").setDescription("Reason").setRequired(false)),
      )
      .addSubcommand((s) =>
        s.setName("undeafen").setDescription("Remove server-deafen from a member")
          .addUserOption((o) => o.setName("user").setDescription("Member").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("move").setDescription("Move a member to a different voice channel")
          .addUserOption((o) => o.setName("user").setDescription("Member").setRequired(true))
          .addChannelOption((o) => o.setName("channel").setDescription("Target voice channel").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("muteall").setDescription("Server-mute all members in a voice channel")
          .addChannelOption((o) => o.setName("channel").setDescription("Voice channel (defaults to bot's current channel)").setRequired(false)),
      )
      .addSubcommand((s) =>
        s.setName("unmuteall").setDescription("Remove server-mute from all muted members in a voice channel")
          .addChannelOption((o) => o.setName("channel").setDescription("Voice channel (defaults to bot's current channel)").setRequired(false)),
      ),
  async execute(ctx) {
    ...
    if (sub === "kick") {
      ...
      if (!member.voice.channel) { await ctx.reply({ embeds: [errorEmbed("That member is not in a voice channel.")] }); return; }
      await member.voice.disconnect(reason);
      await createModCase({ guildId: guild.id, userId: member.id, moderatorId: ctx.userId, type: "kick" as any, reason });
    ...
    if (sub === "mute") {
      ...
      await createModCase({ guildId: guild.id, userId: member.id, moderatorId: ctx.userId, type: "mute" as any, reason });
    ...
    if (sub === "move") {
      ...
      if (!targetCh) { await ctx.reply({ embeds: [errorEmbed("Provide a target voice channel.")] }); return; }
      const vc = guild.channels.cache.get((targetCh as any).id);
      if (!vc || vc.type !== 2) { await ctx.reply({ embeds: [errorEmbed("The target must be a voice channel.")] }); return; }
      await member.voice.setChannel(vc as any, reason);
    ...
    if (sub === "muteall" || sub === "unmuteall") {
      ...
      let vc = chOpt ? guild.channels.cache.get((chOpt as any).id) : null;
      if (!vc) {
        const botVc = guild.members.me?.voice.channel;
        vc = botVc ?? null;
      }
      if (!vc || vc.type !== 2) { await ctx.reply({ embeds: [errorEmbed("Provide a valid voice channel, or the bot must be in a voice channel.")] }); return; }
      const members = (vc as any).members;
      if (!members?.size) { await ctx.reply({ embeds: [errorEmbed("No members in that voice channel.")] }); return; }
      let count = 0;
      for (const [, m] of members) {
        if (m.id === ctx.client.user!.id) continue;
