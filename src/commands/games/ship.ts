...
function getCompatibility(id1: string, id2: string): number {
...
function getShipStatus(score: number): { emoji: string; label: string; desc: string } {
  if (score >= 90) return { emoji: "💘", label: "PERFECT MATCH", desc: "Soulmates! Tadhana na ang nag-utos." };
  if (score >= 75) return { emoji: "❤️", label: "VERY COMPATIBLE", desc: "Maganda ang chemistry! May pag-asa." };
  if (score >= 60) return { emoji: "🧡", label: "COMPATIBLE", desc: "May potensyal! Subukan na lang." };
  if (score >= 45) return { emoji: "💛", label: "FRIENDS ZONE", desc: "Magagandang kaibigan, pero siguro hanggang doon lang." };
  if (score >= 30) return { emoji: "💚", label: "OKAY LANG", desc: "Baka kailangan pang mas makilala ang isa't isa." };
  if (score >= 15) return { emoji: "🤍", label: "UNLIKELY", desc: "Malabo, pero hindi imposible?" };
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!u1) { await ctx.reply({ embeds: [errorEmbed("User not found.")] }); return; }
    if (!u2) u2 = await ctx.client.users.fetch(ctx.userId);
    if (u1.id === u2!.id) { await ctx.reply({ embeds: [errorEmbed("You need to ship two different users!")] }); return; }
