...
function calculateWPM(text: string, elapsedMs: number): number {
...
function gradeWPM(wpm: number): { grade: string; color: number } {
  if (wpm >= 80) return { grade: 'S — Pro Typist 🏆', color: 0xFFD700 };
  if (wpm >= 60) return { grade: 'A — Advanced ⚡', color: 0x00C851 };
  if (wpm >= 45) return { grade: 'B — Above Average 🟢', color: 0x22BB55 };
  if (wpm >= 30) return { grade: 'C — Average 🟡', color: 0xFFAA00 };
  if (wpm >= 20) return { grade: 'D — Needs Practice 🟠', color: 0xFF6600 };
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const filter = (m: any) => m.author.id === ctx.userId;
    const collector = (channel as any).createMessageCollector({ filter, max: 1, time: 60_000 });
    ...
    collector.on('collect', async (msg: any) => {
      ...
      const resultEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '🏅 Grade', value: grade, inline: false },
          { name: '✅ Exact Match', value: exactMatch ? 'Yes! Perfect! 🎉' : 'No (some differences found)', inline: true },
        )
        .setTimestamp();
