...
function evaluate(guess: string, answer: string): Array<"correct" | "present" | "absent"> {
...
const games = new Map<string, { answer: string; guesses: string[]; rows: string[] }>();
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s.setName("guess").setDescription("Submit a guess")
  ...
  async execute(ctx) {
    ...
    if (sub === "start") {
      if (games.has(ctx.userId)) { await ctx.reply({ embeds: [infoEmbed("You already have a Wordle in progress. Use `/wordle guess <word>` to continue or `/wordle quit` to stop.")] }); return; }
    ...
    if (sub === "guess") {
      const game = games.get(ctx.userId);
      if (!game) { await ctx.reply({ embeds: [infoEmbed("No active Wordle. Start one with `/wordle start`.")] }); return; }

      const guess = (ctx.isSlash ? ctx.interaction!.options.getString("word", true) : ctx.args[1] ?? "").toLowerCase().trim();
      if (!/^[a-z]{5}$/.test(guess)) { await ctx.reply({ embeds: [errorEmbed("Guess must be exactly 5 letters (a-z).")] }); return; }
      ...
      if (won || lost) games.delete(ctx.userId);
      ...
      if (won) embed.addFields({ name: "🎉 Congrats!", value: `You got it in ${game.guesses.length} guess${game.guesses.length > 1 ? "es" : ""}!` });
      if (lost) embed.addFields({ name: "💔 Out of tries", value: `The word was **${game.answer.toUpperCase()}**.` });
