...
function handVal(hand: { rank: string }[]): number {
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addStringOption(o =>
        o.setName('choice').setDescription('Your bet').setRequired(true)
          .addChoices(
  ...
  async execute(ctx) {
    ...
    if (!isNatural) {
      // Player third card
      if (pVal <= 5) playerHand.push(deal());
      ...
      const bankerDraws = (() => {
        if (!playerDrewThird) return bVal <= 5;
        if (bVal <= 2) return true;
        if (bVal === 3) return playerThirdVal !== 8;
        if (bVal === 4) return playerThirdVal >= 2 && playerThirdVal <= 7;
        if (bVal === 5) return playerThirdVal >= 4 && playerThirdVal <= 7;
        if (bVal === 6) return playerThirdVal === 6 || playerThirdVal === 7;
        return false; // bVal 7: stand
      })();
      if (bankerDraws) bankerHand.push(deal());
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(color)
      .addFields(
        { name: `Player (${pVal})`, value: playerHand.map(fmtCard).join(' '), inline: true },
        { name: `Banker (${bVal})`, value: bankerHand.map(fmtCard).join(' '), inline: true },
      )
