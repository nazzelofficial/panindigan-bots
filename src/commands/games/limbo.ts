...
function generateResult(): number {
  const r = Math.random();
  if (r < 0.01) return 1.0; // house edge: instant bust 1% of time
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addNumberOption(o =>
        o.setName('target').setDescription('Target multiplier (1.01× – 1000×)').setRequired(true).setMinValue(1.01).setMaxValue(1000)),
  async execute(ctx) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🎲 Result', value: `${result.toFixed(2)}×`, inline: true },
        { name: '📊 Win Chance', value: `~${winChance}%`, inline: true },
        { name: '💰 Net', value: won ? `+${(payout - bet).toLocaleString()}` : `-${bet.toLocaleString()}`, inline: true },
      )
