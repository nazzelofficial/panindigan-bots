import { SlashCommandBuilder, EmbedBuilder } from 'discord.js';
import type { CommandDefinition } from '@/structures/types';

type Choice = 'rock' | 'paper' | 'scissors' | 'lizard' | 'spock';

const EMOJIS: Record<Choice, string> = {
  rock: '🪨', paper: '📄', scissors: '✂️', lizard: '🦎', spock: '🖖',
};

// beats[A][B] = the action string (A beats B with this action)
const BEATS: Partial<Record<Choice, Partial<Record<Choice, string>>>> = {
  rock:     { scissors: 'crushes', lizard: 'crushes' },
  paper:    { rock: 'covers', spock: 'disproves' },
  scissors: { paper: 'cuts', lizard: 'decapitates' },
  lizard:   { spock: 'poisons', paper: 'eats' },
  spock:    { scissors: 'smashes', rock: 'vaporizes' },
};

const CHOICES: Choice[] = ['rock', 'paper', 'scissors', 'lizard', 'spock'];

const command: CommandDefinition = {
  name: 'rpsls',
  description: 'Play Rock, Paper, Scissors, Lizard, Spock!',
  category: 'Games',
  access: 'general',
  guildOnly: false,
  cooldown: 3,
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addStringOption(o =>
        o.setName('choice').setDescription('Your move').setRequired(true)
          .addChoices(
            { name: '🪨 Rock',     value: 'rock'     },
            { name: '📄 Paper',    value: 'paper'    },
            { name: '✂️ Scissors', value: 'scissors' },
            { name: '🦎 Lizard',   value: 'lizard'   },
            { name: '🖖 Spock',    value: 'spock'    },
          )),
  async execute(ctx) {
    const player = ctx.isSlash ? ctx.interaction!.options.getString('choice', true) as Choice : ctx.args[0]?.toLowerCase() as Choice;
    const bot = CHOICES[Math.floor(Math.random() * CHOICES.length)];

    if (!player || !CHOICES.includes(player)) {
      await ctx.reply({ content: 'Invalid choice. Use: rock, paper, scissors, lizard, or spock.' });
      return;
    }

    let result: 'win' | 'lose' | 'tie';
    let actionText: string;
    let color: number;
    let title: string;

    if (player === bot) {
      result = 'tie';
      actionText = "It's the same! Great minds think alike.";
      color = 0xFFAA00;
      title = "🤝 It's a Tie!";
    } else if (BEATS[player]?.[bot]) {
      result = 'win';
      actionText = `${EMOJIS[player]} **${player}** ${BEATS[player]![bot]} ${EMOJIS[bot]} **${bot}**!`;
      color = 0x00C851;
      title = '🎉 You Win!';
    } else {
      result = 'lose';
      actionText = `${EMOJIS[bot]} **${bot}** ${BEATS[bot]![player]} ${EMOJIS[player]} **${player}**!`;
      color = 0xFF4444;
      title = '❌ You Lose!';
    }

    const embed = new EmbedBuilder()
      .setTitle(title)
      .setColor(color)
      .addFields(
        { name: '🎮 You chose',  value: `${EMOJIS[player]} ${player}`, inline: true },
        { name: '🤖 Bot chose',  value: `${EMOJIS[bot]} ${bot}`,       inline: true },
        { name: '⚡ Action',     value: actionText,                      inline: false },
      )
      .setFooter({ text: 'Rock • Paper • Scissors • Lizard • Spock' })
      .setTimestamp();

    await ctx.reply({ embeds: [embed] });
  },
};
export default command;
