...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("start").setDescription("Start a new hangman game")
          .addStringOption((o) =>
            o.setName("category").setDescription("Word category").setRequired(false)
              .addChoices(...Object.keys(CATEGORIES).map((k) => ({ name: k, value: k }))),
          ),
      )
      .addSubcommand((s) =>
        s.setName("guess").setDescription("Guess a letter or the full word")
  ...
  async execute(ctx) {
    ...
    if (sub === "start") {
      if (activeGames.has(ctx.userId)) { await ctx.reply({ embeds: [infoEmbed("You already have a game. Guess a letter or `/hangman quit`.")] }); return; }
      ...
      if (!game) { await ctx.reply({ embeds: [infoEmbed("No active game. Start with `/hangman start`.")] }); return; }
      const input = (ctx.isSlash ? ctx.interaction!.options.getString("letter", true) : ctx.args[1] ?? "").toLowerCase().trim();
      if (!input) { await ctx.reply({ embeds: [errorEmbed("Provide a letter or word.")] }); return; }
      ...
      if (!/[a-z]/.test(letter)) { await ctx.reply({ embeds: [errorEmbed("Please guess a letter (a-z).")] }); return; }
      if (game.guessed.has(letter)) { await ctx.reply({ embeds: [infoEmbed(`You already guessed **${letter}**.`)] }); return; }
      game.guessed.add(letter);

      const isCorrect = game.word.includes(letter);
      if (!isCorrect) game.wrong++;

      const display = game.word.split("").map((c) => c === "-" ? "-" : (game.guessed.has(c) ? c : "_")).join(" ");
      const won = !display.includes("_");
      const lost = game.wrong >= 6;

      if (won || lost) activeGames.delete(ctx.userId);
      ...
      if (won) embed.addFields({ name: "The word was", value: game.word.toUpperCase() });
      if (lost) embed.addFields({ name: "The word was", value: game.word.toUpperCase() });
