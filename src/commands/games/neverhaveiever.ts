...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("nhie:have").setLabel("🍺 I Have!").setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId("nhie:never").setLabel("✋ Never!").setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId("nhie:next").setLabel("⏭️ Next").setStyle(ButtonStyle.Secondary),
    );

    const haveList: string[] = [];
    const neverList: string[] = [];
    ...
    const makeEmbed = () =>
      baseEmbed("primary")
        ...
        .addFields(
          { name: "🍺 Have", value: haveList.length ? haveList.map((id) => `<@${id}>`).join(", ") : "*(wala pa)*", inline: true },
          { name: "✋ Never", value: neverList.length ? neverList.map((id) => `<@${id}>`).join(", ") : "*(wala pa)*", inline: true },
        )
    ...
    collector.on("collect", async (i: any) => {
      if (i.customId === "nhie:next") {
        if (i.user.id !== ctx.userId) { await i.reply({ embeds: [baseEmbed("warning").setDescription("Ang host lang ang makakapili ng next.")], ephemeral: true }); return; }
        collector.stop("next");
      ...
      if (i.customId === "nhie:have") {
        if (!haveList.includes(uid)) { haveList.push(uid); neverList.splice(neverList.indexOf(uid), 1); }
      } else {
        if (!neverList.includes(uid)) { neverList.push(uid); haveList.splice(haveList.indexOf(uid), 1); }
      }
      await (msg as any).edit({ embeds: [makeEmbed()], components: [row] });
    });

    collector.on("end", async (_: any, reason: string) => {
      if (reason === "time") await (msg as any).edit({ components: [] }).catch(() => {});
    });
  },
};
