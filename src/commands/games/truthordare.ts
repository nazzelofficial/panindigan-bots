...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder).addStringOption((o) =>
      o.setName("choice").setDescription("Truth o Dare?").setRequired(false).addChoices({ name: "Truth", value: "truth" }, { name: "Dare", value: "dare" }),
    ),
  async execute(ctx) {
    const choice = (ctx.isSlash ? ctx.interaction!.options.getString("choice") : ctx.args[0]?.toLowerCase()) as "truth" | "dare" | null;
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      ...
      new ButtonBuilder().setCustomId("tod:random").setLabel("🎲 Random").setStyle(ButtonStyle.Secondary),
    );
    ...
    collector.on("collect", async (i: any) => {
      const type = i.customId === "tod:random" ? (Math.random() < 0.5 ? "truth" : "dare") : i.customId.split(":")[1] as "truth" | "dare";
