...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("channel").setDescription("Ignore a channel from logs")
          .addChannelOption((o) => o.setName("target").setDescription("Channel to ignore").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("user").setDescription("Ignore a user's actions from logs")
          .addUserOption((o) => o.setName("target").setDescription("User to ignore").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("role").setDescription("Ignore all actions from members with a role")
  ...
  async execute(ctx) {
    ...
    if (sub === "channel") {
      ...
      if (!id) { await ctx.reply({ embeds: [errorEmbed("Please specify a channel.")] }); return; }
      ...
      if (!id) { await ctx.reply({ embeds: [errorEmbed("Please specify a user.")] }); return; }
      ...
      if (!id) { await ctx.reply({ embeds: [errorEmbed("Please specify a role.")] }); return; }
