  ...
  async execute(ctx) {
    ...
    if (sub === "setup") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "logging.channelId": (channel as any).id, "logging.enabled": true, "logging.disabledEvents": [] } },
        { upsert: true },
      );
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "logging.enabled": true, "logging.disabledEvents": [] } },
        { upsert: true },
      );
      ...
      if (!event || !ALL_EVENTS.includes(event)) {
        await ctx.reply({ embeds: [errorEmbed(`Unknown event. Valid events: ${ALL_EVENTS.join(", ")}`)] }); return;
      }
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const disabled: string[] = (cfg as any)?.logging?.disabledEvents ?? [];
      ...
      const disabled: string[] = logging.disabledEvents ?? [];
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          ...
          { name: "Overall Status", value: logging.enabled !== false ? "✅ Enabled" : "❌ Disabled", inline: true },
          { name: `✅ Active Events (${enabled.length})`, value: enabled.join(", ") || "None", inline: false },
          { name: `❌ Disabled Events (${disabled.length})`, value: disabled.join(", ") || "None", inline: false },
        );
