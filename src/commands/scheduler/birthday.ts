  ...
  async execute(ctx) {
    ...
    if (sub === "set") {
      ...
      if (!month || !day || month < 1 || month > 12 || day < 1 || day > 31) {
        await ctx.reply({ embeds: [errorEmbed("Invalid month/day.")] }); return;
      }
      await BirthdayModel.findOneAndUpdate(
        { userId: ctx.userId },
        { $set: { month, day }, $addToSet: { guildIds: guild.id } },
        { upsert: true },
      );
      ...
      if (!bday) { await ctx.reply({ embeds: [infoEmbed(`${target.username} has no birthday set.`)] }); return; }
      const now = new Date();
      ...
      if (!birthdays.length) { await ctx.reply({ embeds: [infoEmbed("No birthdays set in this server.")] }); return; }
      const now = new Date();
      const withNext = birthdays.map((b: any) => {
      ...
      await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { birthdayChannelId: (channel as any).id } }, { upsert: true });
