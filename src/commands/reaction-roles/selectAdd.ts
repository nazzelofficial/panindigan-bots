...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!roleIds.length) { await ctx.reply({ embeds: [errorEmbed("Please specify at least one role.")] }); return; }
    ...
    const msg = await (ch as any).send({ content: "Select a role from the dropdown below:", components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(select)] });

    await GuildModel.findOneAndUpdate(
      { guildId: guild.id },
      { $push: { reactionRoles: { type: "select", messageId: msg.id, channelId, roleIds } } },
      { upsert: true },
    );
