...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!ch?.isTextBased()) { await ctx.reply({ embeds: [errorEmbed("Cannot send to that channel.")] }); return; }
    ...
    const msg = await (ch as any).send({ content: `React to get the <@&${roleId}> role!`, components: [row] });

    await GuildModel.findOneAndUpdate(
      { guildId: guild.id },
      { $push: { reactionRoles: { type: "button", messageId: msg.id, channelId, roleId, label } } },
      { upsert: true },
    );
