...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const selects: any[] = ((cfg as any)?.reactionRoles ?? []).filter((r: any) => r.type === "select");
    if (!selects.length) { await ctx.reply({ embeds: [infoEmbed("No select role panels configured.")] }); return; }
    const embed = baseEmbed("primary")
      .setTitle("📋 Select Role Panels")
      .setDescription(selects.map((r) => `msg: \`${r.messageId}\` in <#${r.channelId}>\nRoles: ${(r.roleIds ?? []).map((id: string) => `<@&${id}>`).join(", ")}`).join("\n\n").slice(0, 2048));
