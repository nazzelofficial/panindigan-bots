...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const all: any[] = (cfg as any)?.reactionRoles ?? [];
    const reaction = all.filter((r) => r.type === "reaction");
    if (!reaction.length) { await ctx.reply({ embeds: [infoEmbed("No reaction roles configured. Use `reactionadd` to add one.")] }); return; }
