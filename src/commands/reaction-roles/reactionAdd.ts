...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!messageId || !emoji || !roleId) { await ctx.reply({ embeds: [errorEmbed("Provide message ID, emoji, and role.")] }); return; }

    const channel = guild.channels.cache.get(channelId);
    if (!channel?.isTextBased()) { await ctx.reply({ embeds: [errorEmbed("Channel not found or not a text channel.")] }); return; }

    try {
      const msg = await (channel as any).messages.fetch(messageId);
      await msg.react(emoji);
    } catch {
      await ctx.reply({ embeds: [errorEmbed("Could not find or react to that message. Check the message ID and my permissions.")] }); return;
    }

    await GuildModel.findOneAndUpdate(
      { guildId: guild.id },
      { $push: { reactionRoles: { type: "reaction", messageId, channelId, emoji, roleId } } },
      { upsert: true },
    );
