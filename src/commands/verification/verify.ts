...
const challenges = new Map<string, Map<string, { code: string; expires: number }>>();
...
const command: CommandDefinition = {
  ...
  registerComponents(client) {
    ...
    client.componentHandlers.set("verify", async (interaction) => {
      ...
      const vCfg = (cfg as any)?.verification ?? {};
      if (!vCfg.enabled) { await interaction.reply({ content: "Verification is not enabled in this server.", ephemeral: true }); return; }

      const roleId = vCfg.roleId;
      if (!roleId) { await interaction.reply({ content: "Verification role not configured. Contact an admin.", ephemeral: true }); return; }

      const member = interaction.member as GuildMember;
      if (member.roles.cache.has(roleId)) { await interaction.reply({ content: "✅ You are already verified!", ephemeral: true }); return; }

      const method: string = vCfg.method ?? "button";
      ...
      if (method === "button") {
        ...
        if (!challenges.has(guild.id)) challenges.set(guild.id, new Map());
    ...
    client.componentHandlers.set("verify:captcha", async (interaction) => {
      ...
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const roleId = (cfg as any)?.verification?.roleId;
      if (!roleId) { await interaction.reply({ content: "Verification role not configured.", ephemeral: true }); return; }

      await (interaction.member as GuildMember).roles.add(roleId, "Verified via captcha").catch(() => {});
  ...
  async execute(ctx) {
    ...
    if (sub === "setup") {
      ...
      if (!(channel as any)?.isTextBased?.() || !role) { await ctx.reply({ embeds: [errorEmbed("Provide a valid text channel and role.")] }); return; }

      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "verification.enabled": true, "verification.channelId": (channel as any).id, "verification.roleId": role.id, "verification.method": "button" } },
        { upsert: true },
      );
      ...
      if (!["button", "captcha"].includes(type)) { await ctx.reply({ embeds: [errorEmbed("Method must be: button | captcha")] }); return; }
      ...
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const current = (cfg as any)?.verification?.enabled ?? false;
      ...
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const roleId = (cfg as any)?.verification?.roleId;
      if (!roleId) { await ctx.reply({ embeds: [errorEmbed("No verification role configured.")] }); return; }
