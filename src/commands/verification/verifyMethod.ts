...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("set").setDescription("Set the verification method")
          .addStringOption((o) =>
            o.setName("method").setDescription("Verification method").setRequired(true)
              .addChoices(
                { name: "Button (Free)", value: "button" },
                { name: "Captcha (⭐ Premium)", value: "captcha" },
                { name: "Math Problem (⭐ Premium)", value: "math" },
                { name: "Image CAPTCHA (⭐ Premium)", value: "image" },
              ),
          ),
      )
  ...
  async execute(ctx) {
    ...
    if (sub === "set") {
      ...
      if (!method || !valid.includes(method)) { await ctx.reply({ embeds: [errorEmbed(`Invalid method. Choose from: ${valid.join(", ")}`)] }); return; }
      ...
      const method = (cfg as any)?.verification?.method ?? "button";
