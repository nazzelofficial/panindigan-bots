...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!confirmed) { await ctx.reply({ embeds: [errorEmbed("Cancelled. Pass `confirm: true` to kick unverified members.")] }); return; }
    const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
    const verifiedRoleId = (cfg as any)?.verification?.roleId ?? (cfg as any)?.verifiedRoleId;
    const timeoutMinutes = (cfg as any)?.verification?.timeoutMinutes ?? 1440;
    if (!verifiedRoleId) { await ctx.reply({ embeds: [errorEmbed("No verified role is configured. Use `verifyrole` to set one.")] }); return; }
    ...
    for (const [, member] of unverified) {
      try { await member.kick("Unverified — auto-kick after timeout"); kicked++; } catch { /* skip if unkickable */ }
    }
