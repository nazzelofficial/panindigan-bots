...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "setup") {
      ...
      const result = await GoodbyeService.setup({ guild, channelId: (channel as any).id, message: msg || undefined });
      ...
      if (!goodbyeCfg?.channelId) { await ctx.reply({ embeds: [errorEmbed("Run `goodbye setup` first.")] }); return; }
      ...
      if (!channel?.isTextBased()) { await ctx.reply({ embeds: [errorEmbed("Configured goodbye channel not found.")] }); return; }
      ...
      if (goodbyeCfg.cardEnabled) {
        try {
          ...
          bg.addColorStop(0, "#ED4245"); bg.addColorStop(1, "#991B1B");
          ...
          c.fillStyle = "#fff"; c.font = "bold 28px Sans"; c.fillText(`Goodbye, ${user.username.slice(0, 20)}!`, 240, 100);
          c.fillStyle = "#a0a0b0"; c.font = "18px Sans"; c.fillText(`We now have ${guild.memberCount} members`, 240, 140);
      ...
      if (imageAttachment) embed.setImage("attachment://goodbye.png");
