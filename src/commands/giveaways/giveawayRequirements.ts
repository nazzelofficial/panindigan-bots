...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!giveaway) { await ctx.reply({ embeds: [errorEmbed("No active giveaway found.")] }); return; }
    ...
    if (ctx.isSlash) {
      ...
      if (role) requirements.requiredRoleId = role.id;
      if (level) requirements.minLevel = level;
      if (messages) requirements.minMessages = messages;
    }
    if (!Object.keys(requirements).length) { await ctx.reply({ embeds: [errorEmbed("Please specify at least one requirement.")] }); return; }
