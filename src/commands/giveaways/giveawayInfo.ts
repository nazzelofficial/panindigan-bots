...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!messageId) { await ctx.reply({ embeds: [errorEmbed("Please provide a giveaway message ID.")] }); return; }
    ...
    const embed = baseEmbed("primary")
      ...
      .addFields(
        { name: "Status", value: g.ended ? "✅ Ended" : g.paused ? "⏸️ Paused" : "🟢 Active", inline: true },
        { name: "Winners", value: g.winnerCount.toString(), inline: true },
        { name: "Entries", value: (g.participants?.length ?? 0).toString(), inline: true },
        { name: "Channel", value: `<#${g.channelId}>`, inline: true },
        { name: "Host", value: `<@${g.hostId}>`, inline: true },
        { name: "Ends At", value: `<t:${Math.floor(new Date(g.endsAt).getTime() / 1000)}:R>`, inline: true },
        ...(g.ended && g.winners?.length ? [{ name: "Winners", value: g.winners.map((id: string) => `<@${id}>`).join(", "), inline: false }] : []),
      );
