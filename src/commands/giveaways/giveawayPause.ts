...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if ((giveaway as any).paused) { await ctx.reply({ embeds: [errorEmbed("That giveaway is already paused.")] }); return; }
    await GiveawayModel.findOneAndUpdate({ messageId }, { $set: { paused: true } });
    await ctx.reply({ embeds: [successEmbed(`Giveaway **${(giveaway as any).prize}** paused. No new entries will be accepted until resumed.`)] });
  },
};
