...
async function respondGiveawayIds(interaction: any, filter: Record<string, unknown> = {}) {
  const focused  = String(interaction.options.getFocused() ?? "").trim();
  const guildId  = interaction.guildId;
  try {
    const query: Record<string, unknown> = { guildId, ended: false, ...filter };
    if (focused) query["messageId"] = { $regex: focused };
    const docs = await GiveawayModel.find(query).sort({ endsAt: 1 }).limit(10).lean();
    await interaction.respond(
      (docs as any[]).map((g: any) => ({
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s.setName("delete").setDescription("Delete a giveaway")
          .addStringOption((o) => o.setName("id").setDescription("Giveaway message ID").setRequired(true).setAutocomplete(true)),
      )
      .addSubcommand((s) => s.setName("list").setDescription("List active giveaways in this server"))
      .addSubcommand((s) =>
        s.setName("pause").setDescription("Pause a running giveaway")
          .addStringOption((o) => o.setName("id").setDescription("Giveaway message ID").setRequired(true).setAutocomplete(true)),
      )
      .addSubcommand((s) =>
        s.setName("resume").setDescription("Resume a paused giveaway")
  ...
  registerComponents(client) {
    client.componentHandlers.set("giveaway", async (interaction) => {
      ...
      if ((giveaway as any).blacklistedUsers?.includes(userId)) {
  ...
  async execute(ctx) {
    ...
    if (sub === "start") {
      ...
      if (!prize || !durStr) { await ctx.reply({ embeds: [errorEmbed("Provide a prize and duration.")] }); return; }
      const ms = parseDuration(durStr);
      if (!ms || ms < 10_000) { await ctx.reply({ embeds: [errorEmbed("Invalid duration. Use: 30s, 5m, 1h, 7d")] }); return; }

      const endsAt = new Date(Date.now() + ms);
      const channel = guild.channels.cache.get(channelId ?? "");
      if (!channel || !("send" in channel)) { await ctx.reply({ embeds: [errorEmbed("Invalid channel.")] }); return; }
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          ...
          { name: "Hosted by", value: `<@${ctx.userId}>`,                        inline: true },
        );

      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("giveaway:pending").setLabel("🎉 Enter").setStyle(ButtonStyle.Primary),
      );
      ...
      const updatedRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId(`giveaway:${msg.id}`).setLabel("🎉 Enter").setStyle(ButtonStyle.Primary),
      );
    ...
    if (!id && sub !== "list") { await ctx.reply({ embeds: [errorEmbed("Provide the giveaway message ID.")] }); return; }

    // ── list ───────────────────────────────────────────────────────────────
    if (sub === "list") {
      const docs = await GiveawayModel.find({ guildId, ended: false }).sort({ endsAt: 1 }).limit(10).lean();
      if (!docs.length) { await ctx.reply({ embeds: [infoEmbed("No active giveaways in this server.")] }); return; }
      await ctx.reply({
        embeds: [
          baseEmbed("primary")
            .setTitle("🎉 Active Giveaways")
            .setDescription(
              (docs as any[]).map((g: any, i) =>
                `**${i + 1}.** ${g.prize} — ends <t:${Math.floor(new Date(g.endsAt).getTime() / 1000)}:R>\nID: \`${g.messageId}\``,
              ).join("\n\n"),
            ),
        ],
      });
    ...
    if (!giveaway) { await ctx.reply({ embeds: [errorEmbed("Giveaway not found.")] }); return; }

    // ── end ────────────────────────────────────────────────────────────────
    if (sub === "end") {
      if (giveaway.ended) { await ctx.reply({ embeds: [errorEmbed("Giveaway already ended.")] }); return; }
      giveaway.ended = true;
    ...
    // ── pause ──────────────────────────────────────────────────────────────
    if (sub === "pause") {
      if (giveaway.paused) { await ctx.reply({ embeds: [errorEmbed("Giveaway is already paused.")] }); return; }
    ...
    // ── resume ─────────────────────────────────────────────────────────────
    if (sub === "resume") {
      if (!giveaway.paused) { await ctx.reply({ embeds: [errorEmbed("Giveaway is not paused.")] }); return; }
