...
const TEMPLATES = ["default", "minimal", "dark", "gradient", "banner", "pride"] as const;
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s.setName("reset").setDescription("Reset welcome card to the default template"),
      )
      .addSubcommand((s) =>
        s.setName("preview").setDescription("Preview the current welcome card configuration"),
      ),
  async execute(ctx) {
    ...
    if (sub === "template") {
      ...
      if (!template || !TEMPLATES.includes(template as any)) {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "welcome.cardTemplate": template } },
        { upsert: true },
      );
    ...
    if (sub === "background") {
      ...
      if (!url) { await ctx.reply({ embeds: [errorEmbed("Please provide an image URL.")] }); return; }
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "welcome.cardBackgroundUrl": url } },
        { upsert: true },
      );
    ...
    if (sub === "reset") {
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "welcome.cardTemplate": "default", "welcome.cardBackgroundUrl": null } },
      );
    ...
    if (sub === "preview") {
      ...
      const wc = (cfg as any)?.welcome ?? {};
      const embed = baseEmbed("primary")
        .setTitle("🎨 Welcome Card Configuration")
        .addFields(
          { name: "Template", value: wc.cardTemplate ?? "default", inline: true },
          { name: "Custom Background", value: wc.cardBackgroundUrl ? "[Custom URL set]" : "None (uses template default)", inline: true },
          { name: "Welcome Enabled", value: wc.enabled ? "✅ Yes" : "❌ No", inline: true },
        )
        .setFooter({ text: "Use /welcomecard template or /welcomecard background to customize" });
      if (wc.cardBackgroundUrl) embed.setImage(wc.cardBackgroundUrl);
