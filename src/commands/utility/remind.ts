...
function parseDuration(str: string): number | null {
  ...
  const map: Record<string, number> = { s: 1000, m: 60_000, h: 3_600_000, d: 86_400_000, w: 604_800_000 };
  return v * (map[u] ?? 0);
}
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "set") {
      ...
      if (!durationStr || !text) { await ctx.reply({ embeds: [errorEmbed("Provide duration and reminder text.")] }); return; }
      const ms = parseDuration(durationStr);
      if (!ms || ms < 10_000) { await ctx.reply({ embeds: [errorEmbed("Invalid duration. Use e.g. `10m`, `2h`, `1d`. Minimum: 10 seconds.")] }); return; }
      if (user.reminders.length >= 10) { await ctx.reply({ embeds: [errorEmbed("You can have at most 10 active reminders.")] }); return; }

      const channelId = ctx.interaction?.channelId ?? ctx.message?.channelId ?? "0";
      const id = `r_${Date.now().toString(36)}`;
      user.reminders.push({ id, text, remindAt: new Date(Date.now() + ms), channelId, createdAt: new Date() } as any);
      await user.save();

      const remindAt = Math.floor((Date.now() + ms) / 1000);
      await ctx.reply({ embeds: [successEmbed(`⏰ Reminder set! I'll remind you <t:${remindAt}:R>.\n> ${text}`)] });
    } else if (sub === "list") {
      if (!user.reminders.length) { await ctx.reply({ embeds: [infoEmbed("You have no active reminders.")] }); return; }
      ...
      user.reminders = user.reminders.filter((r: any) => r.id !== id) as any;
      if (user.reminders.length === before) { await ctx.reply({ embeds: [errorEmbed("Reminder not found.")] }); return; }
      await user.save();
