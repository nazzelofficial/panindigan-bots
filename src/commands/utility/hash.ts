...
const ALGORITHMS = ["md5", "sha1", "sha256", "sha512", "sha3-256", "sha3-512"] as const;
type HashAlgo = typeof ALGORITHMS[number];
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addStringOption((o) =>
        o.setName("algorithm").setDescription("Hash algorithm (default: sha256)").setRequired(false)
          .addChoices(
            { name: "MD5", value: "md5" },
  ...
  async execute(ctx) {
    ...
    const algo = (ctx.isSlash ? ctx.interaction!.options.getString("algorithm") : ctx.args[0]) as HashAlgo ?? "sha256";
    const showAll = (ctx.isSlash ? ctx.interaction!.options.getBoolean("all") : false) ?? false;

    if (!text) { await ctx.reply({ embeds: [errorEmbed("Provide a text na ia-hash.")] }); return; }
    ...
    await ctx.reply({
      embeds: [
        baseEmbed("primary")
          ...
          .addFields(
            { name: "Input", value: `\`\`\`${text.slice(0, 200)}\`\`\``, inline: false },
            { name: "Hash", value: `\`\`\`${hash}\`\`\``, inline: false },
            { name: "Length", value: `${hash.length} hex chars / ${hash.length / 2} bytes`, inline: true },
          )
          .setFooter({ text: "Input text is not stored or logged." }),
      ],
    });
