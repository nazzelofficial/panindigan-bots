...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("number").setDescription("Random na numero sa loob ng range")
      ...
      .addSubcommand((s) =>
        s.setName("dice").setDescription("Magtapon ng dice")
          .addStringOption((o) =>
            o.setName("type").setDescription("Dice type (default: d6)").setRequired(false)
              .addChoices(
                { name: "d4", value: "4" }, { name: "d6", value: "6" }, { name: "d8", value: "8" },
                { name: "d10", value: "10" }, { name: "d12", value: "12" }, { name: "d20", value: "20" }, { name: "d100", value: "100" },
              ),
          )
      ...
      .addSubcommand((s) =>
        s.setName("pick").setDescription("Pumili ng random mula sa listahan")
          .addStringOption((o) => o.setName("items").setDescription("Mga pagpipilian, hiwalay ng comma").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("percent").setDescription("Random na percentage (0–100%)")
      ),
  async execute(ctx) {
    ...
    if (sub === "number") {
      ...
      if (min > max) { await ctx.reply({ embeds: [errorEmbed("Ang min ay dapat mas mababa sa max.")] }); return; }
      ...
      await ctx.reply({
        embeds: [
          baseEmbed("primary")
            ...
            .addFields(
              { name: count > 1 ? "Rolls" : "Result", value: rolls.join(", "), inline: true },
              ...(count > 1 ? [{ name: "Total", value: String(total), inline: true }] : []),
            ),
        ],
      });
      return;
    ...
    if (sub === "pick") {
      ...
      if (items.length < 2) { await ctx.reply({ embeds: [errorEmbed("Provide a kahit 2 items, hiwalay ng comma.")] }); return; }
