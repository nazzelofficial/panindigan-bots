...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const results = await UserModel.find({ "guilds.guildId": guild.id })
      ...
      .limit(pageSize)
      .lean();
    if (!results.length) { await ctx.reply({ embeds: [infoEmbed("No message data found for this server.")] }); return; }
    const lines = results.map((u: any, i: number) => {
      const guildData = u.guilds?.find((g: any) => g.guildId === guild.id);
