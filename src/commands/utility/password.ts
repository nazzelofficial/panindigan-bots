...
function generatePassword(length: number, upper: boolean, numbers: boolean, symbols: boolean): string {
  ...
  if (numbers) charset += CHARSET_NUM;
  if (symbols) charset += CHARSET_SYM;
...
function getStrength(length: number, upper: boolean, numbers: boolean, symbols: boolean): string {
  let score = 0;
  if (length >= 12) score++;
  if (length >= 16) score++;
  if (upper) score++;
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (length < 8 || length > 64) { await ctx.reply({ embeds: [errorEmbed("Ang haba ng password ay dapat 8-64 characters.")] }); return; }
