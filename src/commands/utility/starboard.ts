...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("setup").setDescription("Set up the starboard channel")
    ...
    if (sub === "setup") {
      ...
      const embed = baseEmbed("primary").setTitle("⭐ Starboard Configuration")
        .addFields(
          ...
          { name: "Threshold", value: sb.threshold ? `${sb.threshold} ⭐` : "3 ⭐ (default)", inline: true },
        );
