...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("create").setDescription("Create bagong thread")
  ...
  async execute(ctx) {
    ...
    if (sub === "create") {
      ...
      if (!name) { await ctx.reply({ embeds: [errorEmbed("Provide a pangalan ng thread.")] }); return; }
      if (!channel || !(channel as any).threads) { await ctx.reply({ embeds: [errorEmbed("This channel does not support threads.")] }); return; }
      ...
      if (!thread) { await ctx.reply({ embeds: [errorEmbed("Hindi magawa ang thread.")] }); return; }
    ...
    if (sub === "archive") {
      ...
      if (lock) await (thread as any).setLocked(true).catch(() => {});
    ...
    if (sub === "rename") {
      ...
      await (thread as any).setName(name);
    ...
    if (sub === "add") {
      ...
      if (!userId) { await ctx.reply({ embeds: [errorEmbed("Provide a user.")] }); return; }
      await (thread as any).members.add(userId).catch(() => {});
    ...
    if (sub === "remove") {
      const userId = ctx.isSlash ? ctx.interaction!.options.getUser("user", true).id : ctx.args[1]?.replace(/\D/g, "");
      if (!userId) { await ctx.reply({ embeds: [errorEmbed("Provide a user.")] }); return; }
      await (thread as any).members.remove(userId).catch(() => {});
    ...
    if (sub === "info") {
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          ...
          { name: "Type", value: t.type === ChannelType.PrivateThread ? "Private" : "Public", inline: true },
          { name: "Archived", value: t.archived ? "Yes" : "No", inline: true },
          { name: "Locked", value: t.locked ? "Yes" : "No", inline: true },
          { name: "Member Count", value: String(t.memberCount ?? "Unknown"), inline: true },
          { name: "Created", value: `<t:${Math.floor(t.createdTimestamp / 1000)}:R>`, inline: true },
        );
