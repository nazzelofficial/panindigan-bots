...
const SOCIAL_KEYS = ["website", "facebook", "twitter", "instagram", "youtube", "tiktok", "twitch", "github", "discord"] as const;
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s.setName("set").setDescription("Set a social media link (Admin only)")
          .addStringOption((o) =>
  ...
  async execute(ctx) {
    ...
    if (sub === "set") {
      ...
      if (!platform || !SOCIAL_KEYS.includes(platform as any)) { await ctx.reply({ embeds: [errorEmbed(`Invalid platform. Choose from: ${SOCIAL_KEYS.join(", ")}`)] }); return; }
      if (url) {
      ...
      const socials = (cfg as any)?.socials ?? {};
