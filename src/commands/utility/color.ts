  ...
  async execute(ctx) {
    ...
    if (raw.toLowerCase() === "random") {
      ...
      if (parts.length < 3 || parts.some((p) => isNaN(p) || p < 0 || p > 255)) {
        await ctx.reply({ embeds: [errorEmbed("Invalid RGB format. Use `R,G,B` e.g. `255,128,0`.")] }); return;
      }
      hex = `#${parts.map((p) => p.toString(16).padStart(2, "0")).join("")}`;
    } else {
      hex = raw.startsWith("#") ? raw : `#${raw}`;
      if (!/^#[0-9a-fA-F]{6}$/.test(hex)) {
        await ctx.reply({ embeds: [errorEmbed("Invalid hex color. Use format `#RRGGBB` or `RRGGBB`.")] }); return;
      }
    }

    const rgb = hexToRgb(hex);
    if (!rgb) { await ctx.reply({ embeds: [errorEmbed("Invalid color value.")] }); return; }
    ...
    const embed = baseEmbed(hex as any)
