...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    try {
      ...
      await ctx.reply({ embeds: [errorEmbed(`Nagkaroon ng error sa pagproseso: ${(e as Error).message}`)] });
      return;
    }
    ...
    await ctx.reply({
      embeds: [
        baseEmbed("primary")
          .setTitle(title)
          .addFields(
            { name: "Input", value: `\`\`\`${input.slice(0, 400)}\`\`\``, inline: false },
