...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
        ...
        embeds: [
          baseEmbed("primary")
            ...
            .addFields(
              ...
              { name: "Total XP", value: String(profile.totalXp ?? xp), inline: true },
    ...
    if (sub === "balance") {
      ...
      const jobId: string | null = profile.jobId ?? null;
    ...
    if (sub === "reminders") {
      const reminders: any[] = profile.reminders ?? [];
      ...
      embeds: [
        baseEmbed("primary")
          ...
          .addFields(
            ...
            { name: "💼 Job", value: profile.jobId ? (JOBS.find((j) => j.id === profile.jobId)?.name ?? profile.jobId) : "Unemployed", inline: true },
            { name: "💬 Messages", value: String(profile.messageCount ?? 0), inline: true },
