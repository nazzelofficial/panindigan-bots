...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("plain").setDescription("Magpadala ng plain text message")
      ...
      .addSubcommand((s) =>
        s.setName("embed").setDescription("Magpadala ng embed message")
  ...
  async execute(ctx) {
    ...
    const target = chOpt ? guild.channels.cache.get((chOpt as any).id) : (ctx.interaction?.channel ?? ctx.message?.channel);
    if (!target || !(target as any).send) { await ctx.reply({ embeds: [errorEmbed("Hindi valid na channel.")] }); return; }

    if (sub === "plain") {
      const msg = ctx.isSlash ? ctx.interaction!.options.getString("message", true) : ctx.args.join(" ");
      if (!msg) { await ctx.reply({ embeds: [errorEmbed("Provide a message.")] }); return; }
      await (target as any).send(msg);
      if (ctx.isSlash) await ctx.interaction!.reply({ content: "✅ Message sent.", ephemeral: true });
      else if (target.id !== ctx.message?.channelId) await ctx.reply({ embeds: [baseEmbed("success").setDescription("✅ Message sent.")] });
      return;
    }
    ...
    if (sub === "embed") {
      ...
      if (!title || !description) { await ctx.reply({ embeds: [errorEmbed("Provide a title at description.")] }); return; }
      ...
      const embed = baseEmbed("primary").setTitle(title).setDescription(description).setColor(color as any);
      if (footer) embed.setFooter({ text: footer });

      await (target as any).send({ embeds: [embed] });
      if (ctx.isSlash) await ctx.interaction!.reply({ content: "✅ Embed sent.", ephemeral: true });
      else if (target.id !== ctx.message?.channelId) await ctx.reply({ embeds: [baseEmbed("success").setDescription("✅ Embed sent.")] });
      return;
    }
