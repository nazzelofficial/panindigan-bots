...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("set").setDescription("Set rules channel at rules content")
  ...
  async execute(ctx) {
    ...
    if (sub === "view") {
      ...
      const rulesText: string = (cfg as any)?.rulesText ?? "";
      const rulesChannelId: string | null = (cfg as any)?.rulesChannelId ?? null;
      ...
      if (rulesChannelId) embed.addFields({ name: "Rules Channel", value: `<#${rulesChannelId}>`, inline: true });
    ...
    if (sub === "set") {
      ...
      const chId = (chOpt as any)?.id ?? null;
      const rulesText = ctx.isSlash ? ctx.interaction!.options.getString("rules") : ctx.args.slice(1).join(" ");

      const update: Record<string, any> = {};
      if (chId) update.rulesChannelId = chId;
      if (rulesText) update.rulesText = rulesText;
    ...
    if (sub === "post") {
      ...
      if (!rulesChannelId) { await ctx.reply({ embeds: [errorEmbed("Walang configured na rules channel. Use `/setrules set channel:#channel`.")] }); return; }
      if (!rulesText) { await ctx.reply({ embeds: [errorEmbed("No rules text configured. Use `/setrules set rules:...`.")] }); return; }

      const ch = guild.channels.cache.get(rulesChannelId);
      if (!ch || !(ch as any).send) { await ctx.reply({ embeds: [errorEmbed("Rules channel not found or invalid.")] }); return; }
