...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const embed = baseEmbed("primary")
      ...
      .addFields(
        { name: "👤 Owner", value: client.config.bot.ownerId ? `<@${client.config.bot.ownerId}>` : "Not configured", inline: true },
        { name: "📅 Created", value: `<t:${Math.floor((client.user?.createdTimestamp ?? Date.now()) / 1000)}:D>`, inline: true },
        ...
        { name: "💾 RAM Usage", value: `${formatBytes(mem.rss)} (heap: ${formatBytes(mem.heapUsed)})`, inline: true },
