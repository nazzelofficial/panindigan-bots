...
function normalizeVotes(votes: unknown): Map<string, string> {
  ...
  if (votes && typeof votes === "object") {
    return new Map(Object.entries(votes as Record<string, string>));
  }
  return new Map();
}

function buildPollEmbed(question: string, options: string[], votes: Map<string, string>) {
...
const command: CommandDefinition = {
  ...
  registerComponents(client) {
    client.componentHandlers.set("poll", async (interaction) => {
      ...
      const votes = normalizeVotes(poll.votes as unknown);
      votes.set(interaction.user.id, String(voteIndex));
      poll.votes = votes as any;
      await poll.save();
  ...
  async execute(ctx) {
    ...
    if (sub === "create") {
      ...
      if (endsAt) embed.setFooter({ text: `0 total votes · Ends at` }).setTimestamp(endsAt);
      ...
      const msg = await (channel as TextChannel).send({ embeds: [embed], components: rows });
      const realRows: ActionRowBuilder<ButtonBuilder>[] = [];
      let realRow = new ActionRowBuilder<ButtonBuilder>();
      options.forEach((opt, index) => {
        if (index > 0 && index % 5 === 0) {
          realRows.push(realRow);
          realRow = new ActionRowBuilder<ButtonBuilder>();
        }
      ...
      if (sub === "end") {
        ...
        if (message) {
          await message.edit({ embeds: [buildPollEmbed(poll.question, poll.options, normalizeVotes(poll.votes as unknown)).setTitle(`📊 [ENDED] ${poll.question}`)], components: [] }).catch(() => {});
        }
      }
      const embed = buildPollEmbed(poll.question, poll.options, normalizeVotes(poll.votes as unknown));
      if (sub === "end") embed.setTitle(`📊 [ENDED] ${poll.question}`);
