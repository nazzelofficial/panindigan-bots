...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addStringOption((o) =>
        o.setName("format").setDescription("Discord timestamp format").setRequired(false)
          .addChoices(
            { name: "Short Time (3:00 PM)", value: "t" },
            { name: "Long Time (3:00:00 PM)", value: "T" },
            { name: "Short Date (01/01/2025)", value: "d" },
            { name: "Long Date (January 1, 2025)", value: "D" },
            { name: "Short Date+Time (default)", value: "f" },
            { name: "Long Date+Time", value: "F" },
            { name: "Relative (in 5 hours)", value: "R" },
          ),
      ),
  async execute(ctx) {
    ...
    if (!input || input.toLowerCase() === "now") {
      ...
      if (isNaN(date.getTime())) {
        ...
        else { await ctx.reply({ embeds: [errorEmbed(`Hindi ma-parse ang petsa: \`${input}\`\n\nUse format: \`2025-12-25\`, \`2025-12-25 18:00\`, o \`now\`.`)] }); return; }
      }
    }
    ...
    const embed = baseEmbed("primary")
      ...
      .addFields(
        ...
        { name: "ISO 8601", value: `\`${date.toISOString()}\``, inline: true },
        { name: "Selected Format", value: `\`<t:${unix}:${format}>\` → <t:${unix}:${format}>`, inline: false },
        { name: "All Formats", value: allFormats.map((f) => `\`<t:${unix}:${f}>\` → <t:${unix}:${f}>`).join("\n"), inline: false },
      )
