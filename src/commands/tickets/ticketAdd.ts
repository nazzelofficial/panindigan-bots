...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!userId) { await ctx.reply({ embeds: [errorEmbed("Please mention a user to add.")] }); return; }
    const member = guild.members.cache.get(userId) ?? await guild.members.fetch(userId).catch(() => null);
    if (!member) { await ctx.reply({ embeds: [errorEmbed("Could not find that user in this server.")] }); return; }
