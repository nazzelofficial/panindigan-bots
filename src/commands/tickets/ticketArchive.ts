...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    try {
      const currentName: string = (channel as any).name ?? "ticket";
      if (!currentName.startsWith("archived-")) {
        await (channel as any).setName(`archived-${currentName}`, "Ticket archived");
      }
    } catch {
    ...
    await TicketModel.findOneAndUpdate(
      ...
      { $set: { status: "archived", archivedAt: new Date(), archivedReason: reason, archivedBy: ctx.userId } },
    );

    const embed = baseEmbed("warning")
      .setTitle("📦 Ticket Archived")
      .addFields(
        { name: "Archived by", value: `<@${ctx.userId}>`, inline: true },
