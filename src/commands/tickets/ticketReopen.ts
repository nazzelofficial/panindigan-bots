...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (ticket.status === "open") { await ctx.reply({ embeds: [errorEmbed("This ticket is already open.")] }); return; }
    if (ticket.status === "archived") { await ctx.reply({ embeds: [errorEmbed("Archived tickets cannot be reopened.")] }); return; }
    ticket.status = "open";
    ...
    if (opener) {
      await (channel as TextChannel).permissionOverwrites.edit(opener, {
        ViewChannel: true, SendMessages: true,
      }).catch(() => {});
    }
    await sendLogEvent(guild.id, "ticket_reopen", () =>
      new EmbedBuilder()
