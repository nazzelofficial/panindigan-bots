...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const tier: string = (record as any)?.tier ?? "none";
    if (tier === "none") {
      await ctx.reply({ embeds: [infoEmbed(`**${guild.name}** is on the **Free** plan.\n\nType \`premiuminfo\` to see available plans and pricing. Use \`premiumactivate [code]\` to unlock premium features.`)] }); return;
    }
    const activatedAt: Date | null = (record as any)?.activatedAt ?? null;
    const embed = baseEmbed("premium")
      .setTitle("⭐ Premium Status")
      .addFields(
        { name: "Plan", value: TIER_LABELS[tier] ?? tier, inline: true },
        { name: "Server", value: guild.name, inline: true },
        { name: "Activated", value: activatedAt ? `<t:${Math.floor(activatedAt.getTime() / 1000)}:D>` : "Unknown", inline: true },
