...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("set").setDescription("Set the server language (Admin only)")
          .addStringOption((o) =>
            o.setName("language").setDescription("Language to use").setRequired(true)
  ...
  async execute(ctx) {
    ...
    if (sub === "set") {
      ...
      if (!valid) { await ctx.reply({ content: `Invalid language. Choices: ${LANGUAGES.map((l) => l.value).join(", ")}` }); return; }
      ...
      const lang = (cfg as any)?.language ?? "en";
