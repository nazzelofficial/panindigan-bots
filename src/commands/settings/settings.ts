...
function getLatestChangelogExcerpt(): string {
  try {
    ...
    if (startIndex === -1) return content.slice(0, 1800);
...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s.setName("activate").setDescription("Activate a premium code for this server")
  ...
  async execute(ctx) {
    ...
    if (sub === "view") {
      if (!guild) { await ctx.reply({ embeds: [infoEmbed("Use this in a server.")] }); return; }
      const member = ctx.interaction?.member ?? ctx.message?.member;
      if (!(member as any)?.permissions?.has(PermissionFlagsBits.ManageGuild)) {
        await ctx.reply({ embeds: [errorEmbed("You need Manage Server permission.")] }); return;
      }
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          { name: "Prefix", value: `\`${(cfg as any)?.prefix ?? config.bot.defaultPrefix}\``, inline: true },
          { name: "Premium Tier", value: (prem as any)?.tier ?? "free", inline: true },
          { name: "Language", value: (cfg as any)?.language ?? "en", inline: true },
          { name: "Welcome", value: (cfg as any)?.welcome?.enabled ? `✅ <#${(cfg as any).welcome.channelId}>` : "❌", inline: true },
          { name: "Goodbye", value: (cfg as any)?.goodbye?.enabled ? `✅ <#${(cfg as any).goodbye.channelId}>` : "❌", inline: true },
          { name: "Verification", value: (cfg as any)?.verification?.enabled ? "✅" : "❌", inline: true },
          { name: "Anti-Nuke", value: (cfg as any)?.antinuke?.enabled ? "✅" : "❌", inline: true },
          { name: "Automod", value: (cfg as any)?.automod?.enabled ? "✅" : "❌", inline: true },
          { name: "Tickets", value: (cfg as any)?.tickets?.enabled ? "✅" : "❌", inline: true },
          { name: "Leveling", value: (cfg as any)?.leveling?.enabled !== false ? "✅" : "❌", inline: true },
          { name: "Economy", value: (cfg as any)?.economy?.enabled !== false ? "✅" : "❌", inline: true },
          { name: "Disabled Commands", value: ((cfg as any)?.disabledCommands ?? []).join(", ") || "None", inline: false },
        );
      ...
      if (!prem || !(prem as any).active) {
        ...
        const tierLabel: Record<string, string> = { basic: "Basic ⭐", standard: "Standard ⭐⭐", gold: "Gold 🥇", enterprise: "Enterprise 🏆" };
        const embed = baseEmbed("warning")
          .setTitle("⭐ Premium Status")
          .addFields(
            { name: "Tier", value: tierLabel[(prem as any).tier] ?? (prem as any).tier, inline: true },
            { name: "Active", value: "✅ Yes", inline: true },
      ...
      if (!code) { await ctx.reply({ embeds: [errorEmbed("Provide a premium code.")] }); return; }
      const codeDoc = await PremiumCodeModel.findOne({ code: code.toUpperCase().trim(), used: false });
      if (!codeDoc) { await ctx.reply({ embeds: [errorEmbed("Invalid or already-used premium code.")] }); return; }

      await PremiumModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { tier: codeDoc.tier, active: true, grantedBy: ctx.userId } },
        { upsert: true },
      );
      ...
      const supportUrl = (config.bot as any)?.supportServerInvite ?? "https://discord.gg/panindigan";
