...
function toggleFavorite(userId: string, commandName: string): boolean {
  const fav = userFavorites.get(userId) ?? new Set<string>();
...
function isFavorite(userId: string, commandName: string): boolean {
...
function buildHomeEmbed(
  client: { commands: Map<string, CommandDefinition>; guilds: { cache: { size: number } }; ws: { ping: number }; startedAt: number },
  userId: string,
  guildId: string | null,
  ...
  const embed = EmbedFactory.helpDashboard(
    "🤖 Panindigan Help Center",
    `> *"Hindi lang ito bot — ito ay isang ecosystem."*\n\u200b`,
  );

  // ── Bot stats row ────────────────────────────────────────────────────────
  embed.addFields(
    { name: "📦 Commands",   value: `\`${allCmds.length}\``,   inline: true },
    { name: "📂 Categories", value: `\`${categories}\``,       inline: true },
    { name: "⭐ Premium",    value: `\`${premiumCnt}\` cmds`,  inline: true },
...
function buildCategoryListEmbed(allCmds: CommandDefinition[]): import("discord.js").EmbedBuilder {
...
function buildCategoryCommandsEmbed(
  commands: CommandDefinition[],
  category: string,
  page: number,
  prefix: string,
): { embed: import("discord.js").EmbedBuilder; totalPages: number } {
  ...
  const embed = EmbedFactory.helpDashboard(
    ...
    slice.map((c) =>
      `\`/${c.name}\`${c.premium ? " ⭐" : ""}${c.aliases?.length ? ` *(${c.aliases[0]})*` : ""}\n${c.description}`,
...
function buildCommandDetailEmbed(
  ...
  embed.addFields(
    { name: "📂 Kategorya",   value: `${icon} ${cmd.category}`,                                               inline: true  },
    { name: "🔐 Access",      value: accessLabels[cmd.access] ?? cmd.access,                                   inline: true  },
    { name: "👑 Premium",     value: cmd.premium ? "⭐ Yes" : "✅ Free",                                       inline: true  },
    { name: "🏠 Guild Only",  value: cmd.guildOnly === false ? "Hindi (DM supported)" : "Server lang",         inline: true  },
    { name: "❄️ Cooldown",    value: cmd.cooldown ? `⏱️ ${cmd.cooldown}s` : "Wala",                           inline: true  },
    { name: "⚡ Support",     value: cmd.slashData ? "/ Slash + Prefix" : "Prefix lang",                       inline: true  },
  );
...
function buildSearchEmbed(results: CommandDefinition[], query: string): import("discord.js").EmbedBuilder {
  if (!results.length) {
    return EmbedFactory.warning(
      `Walang nahanap para sa \`${query}\`.\n\nSubukan ang iba pang spelling, o i-browse ang mga kategorya gamit ang \`/help\`.`,
      "🔍 Walang Resulta",
    );
  }
  const embed = EmbedFactory.helpDashboard(
    `🔍 Resulta para sa "${query}"`,
    results.map((c, i) =>
...
function buildFavoritesEmbed(userId: string, allCmds: CommandDefinition[]): import("discord.js").EmbedBuilder {
  const favNames = [...(userFavorites.get(userId) ?? [])];
  if (!favNames.length) {
    return EmbedFactory.info(
      "Wala ka pang mga paboritong command.\n\nI-view ang isang command at i-click ang **☆ Favorite** button para mag-save!",
      "⭐ Mga Paborito",
    );
  }
  const favCmds = favNames.map((n) => allCmds.find((c) => c.name === n)).filter(Boolean) as CommandDefinition[];
  const embed   = EmbedFactory.helpDashboard(
    "⭐ Iyong Mga Paboritong Command",
    favCmds.map((c) => `\`/${c.name}\`${c.premium ? " ⭐" : ""} — ${c.description}`).join("\n"),
  );
  embed.setFooter({ text: `${favCmds.length} paborito · I-click ang command para sa detalye`, iconURL: BOT_ICON });
  return embed;
}

function buildRecentEmbed(userId: string, allCmds: CommandDefinition[]): import("discord.js").EmbedBuilder {
  const recentNames = recentlyViewed.get(userId) ?? [];
  if (!recentNames.length) {
    return EmbedFactory.info(
      "Wala ka pang mga na-view na command ngayong session.\n\nSubukan ang \`/help [command]\` para tingnan ang isang command!",
      "🕒 Mga Nakita Mo",
    );
  }
  ...
  const embed = EmbedFactory.helpDashboard(
    "🕒 Mga Huli Mong Nakita",
    recentCmds.map((c) => `\`/${c.name}\`${c.premium ? " ⭐" : ""} — ${c.description}`).join("\n"),
  );
  embed.setFooter({ text: `${recentCmds.length} commands · Session lang ito — mag-re-reset sa restart`, iconURL: BOT_ICON });
  return embed;
}

function buildPopularEmbed(guildId: string | null, allCmds: CommandDefinition[]): import("discord.js").EmbedBuilder {
  const top = topCommands(guildId, 15);
  if (!top.length) {
    return EmbedFactory.info(
      "Wala pang command usage data para sa server na ito.\n\nSubukan ang mga commands at lalabas sila dito! 📊",
      "📈 Mga Sikat na Command",
    );
  }
  const embed = EmbedFactory.helpDashboard(
    "📈 Mga Pinakasikat na Command",
    top.map((t, i) => {
...
function buildGuideEmbed(): import("discord.js").EmbedBuilder {
  const embed = EmbedFactory.helpDashboard(
    "❓ Guide — Ano ang Bago sa v0.2.6",
    `> *"Version 0.2.6 is a complete modernization of Panindigan Official."*\n\u200b`,
  );
...
function mainNavRow(activeTab: string): ActionRowBuilder<ButtonBuilder> {
  const tabs = [
...
function secondNavRow(activeTab: string): ActionRowBuilder<ButtonBuilder> {
  const tabs = [
...
function categorySelectMenu(categories: string[]): ActionRowBuilder<StringSelectMenuBuilder> {
...
function paginationRow(page: number, totalPages: number, category: string): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
...
function commandDetailButtons(cmdName: string, userId: string): ActionRowBuilder<ButtonBuilder> {
...
function searchResultButtons(results: CommandDefinition[]): ActionRowBuilder<ButtonBuilder> | null {
  ...
  async execute(ctx) {
    ...
    if (query && !query.startsWith("search ")) {
      ...
      if (cmd) {
        ...
        const embed    = buildCommandDetailEmbed(cmd as CommandDefinition, prefix, ctx.userId);
        ...
        collector.on("collect", async (i: MessageComponentInteraction) => {
          ...
          if (action === "fav" && data) {
            ...
            const updatedEmbed = buildCommandDetailEmbed(cmd as CommandDefinition, prefix, ctx.userId);
    ...
    collector.on("collect", async (i: MessageComponentInteraction) => {
      ...
      if (id === "help:search") {
        ...
        if (i.isButton()) {
          ...
          await (i as unknown as { showModal: (m: ModalBuilder) => Promise<void> }).showModal(modal);
        }
        return;
      }
      ...
      if (i.componentType === ComponentType.StringSelect && id === "help:category:select") {
        const values = (i as unknown as { values: string[] }).values;
    ...
    const modalFilter = (i: ModalSubmitInteraction) =>
      i.user.id === ctx.userId && i.customId === "help:search:modal";
    ...
    collector.on("end", async (_collected, reason) => {
      if (reason === "user" || reason === "feedback") return;
      try { await ctx.interaction!.editReply({ components: [] }); }
      catch { /* message may have been deleted */ }
    });
  },
