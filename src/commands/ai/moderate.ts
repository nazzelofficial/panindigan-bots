...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!text) { await ctx.reply({ embeds: [errorEmbed("Please provide text to moderate.")] }); return; }
    ...
    try {
      ...
      const categories = item.categories as unknown as Record<string, boolean>;
      const scores = item.category_scores as unknown as Record<string, number>;
      ...
      const embed = baseEmbed(flagged ? "danger" : "success")
        ...
        .addFields(
          { name: "Text (preview)", value: text.length > 200 ? text.slice(0, 200) + "…" : text },
          { name: "Verdict", value: flagged ? `**FLAGGED** — ${triggeredCats.join(", ")}` : `**Clean** — safety score ${safeScore}%` },
        )
