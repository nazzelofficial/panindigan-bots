...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "view") {
      ...
      const persona = (doc as any)?.aiPersona;
      if (!persona) {
        ...
        await ctx.reply({ embeds: [baseEmbed("primary").setTitle("🤖 AI Persona").addFields(
          { name: "Name", value: persona.name ?? "Custom" },
          { name: "Description", value: persona.description ?? "No description" },
        )] });
      }
    } else if (sub === "set") {
      ...
      if (!name || !description) { await ctx.reply({ embeds: [errorEmbed("Please provide both a name and description.")] }); return; }
