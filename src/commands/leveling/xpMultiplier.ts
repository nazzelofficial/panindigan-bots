...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("set").setDescription("Set the XP multiplier")
  ...
  async execute(ctx) {
    ...
    if (sub === "set") {
      ...
      if (isNaN(multi) || multi < 0.1 || multi > 10) { await ctx.reply({ embeds: [errorEmbed("Multiplier must be between 0.1 and 10.")] }); return; }
      ...
      const multi = (cfg as any)?.leveling?.xpMultiplier ?? 1;
