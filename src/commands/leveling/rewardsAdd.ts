...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!level || !roleId) { await ctx.reply({ embeds: [errorEmbed("Please specify a level and a role.")] }); return; }
    const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
    const existing = (cfg as any)?.leveling?.rewards ?? [];
    if (existing.some((r: any) => r.level === level)) {
      await ctx.reply({ embeds: [errorEmbed(`Level **${level}** already has a reward. Remove it first with \`rewardsremove\`.`)] }); return;
    }
