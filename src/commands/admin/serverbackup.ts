...
import { ServerBackupModel } from "../../database/models/Community.js";
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "create") {
      ...
      if (existing >= 5) {
        await ctx.reply({ embeds: [errorEmbed("Maximum 5 backups per server. Delete an old backup first.")] }); return;
      }

      if (guild.memberCount > guild.members.cache.size) await guild.members.fetch().catch(() => {});
      ...
      const data = {
        ...
        channels: guild.channels.cache
          .filter((c) => !c.isThread())
          .map((c) => ({ id: c.id, name: c.name, type: c.type, parentId: (c as any).parentId ?? null, position: (c as any).position ?? 0 })),
      ...
      if (!backup || backup.guildId !== guild.id) { await ctx.reply({ embeds: [errorEmbed("Backup not found.")] }); return; }
      const d = backup.data as any;
      const embed = baseEmbed("primary").setTitle("💾 Backup Info").addFields(
        { name: "Server Name", value: d.name ?? guild.name, inline: true },
        { name: "Member Count", value: String(d.memberCount ?? "?"), inline: true },
        { name: "Captured By", value: `<@${backup.createdBy}>`, inline: true },
        { name: "Roles", value: String(d.roles?.length ?? 0), inline: true },
        { name: "Channels", value: String(d.channels?.length ?? 0), inline: true },
        { name: "Bot Config", value: d.botConfig ? "Included" : "Not included", inline: true },
        { name: "Created", value: `<t:${Math.floor(new Date((backup as any).createdAt).getTime() / 1000)}:F>`, inline: false },
      );
      await ctx.reply({ embeds: [embed] });
