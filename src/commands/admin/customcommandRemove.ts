...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!name) { await ctx.reply({ embeds: [errorEmbed("Please specify the command name.")] }); return; }
    const result = await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $pull: { customCommands: { name } } }, { new: true });
    if (!result) { await ctx.reply({ embeds: [errorEmbed(`No custom command named \`${name}\` found.`)] }); return; }
