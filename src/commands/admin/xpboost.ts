...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "start") {
      ...
      if (!(doc as any)?.xpBoostEvent?.active) { await ctx.reply({ embeds: [infoEmbed("No active XP boost event.")] }); return; }
