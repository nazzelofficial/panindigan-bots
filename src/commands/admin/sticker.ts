import { PermissionFlagsBits, SlashCommandBuilder, StickerFormatType } from "discord.js";
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    function resolveSticker(query: string) {
    ...
    if (sub === "add") {
      ...
      if (!name || !url || !emoji) { await ctx.reply({ embeds: [errorEmbed("Please provide a name, image URL, and related emoji.")] }); return; }
      if (name.length < 2 || name.length > 30) { await ctx.reply({ embeds: [errorEmbed("Sticker name must be 2–30 characters.")] }); return; }
      if (guild.stickers.cache.size >= (guild as any).maximumStickers) { await ctx.reply({ embeds: [errorEmbed(`This server has reached its sticker limit (${(guild as any).maximumStickers}).`)] }); return; }
      try {
    ...
    if (sub === "delete") {
      ...
      if (!query) { await ctx.reply({ embeds: [errorEmbed("Please specify the sticker to delete.")] }); return; }
      const sticker = resolveSticker(query);
      if (!sticker) { await ctx.reply({ embeds: [errorEmbed(`Sticker \`${query}\` not found in this server.`)] }); return; }
    ...
    if (sub === "rename") {
      ...
      if (!query || !newName) { await ctx.reply({ embeds: [errorEmbed("Please provide the sticker and new name.")] }); return; }
      if (newName.length < 2 || newName.length > 30) { await ctx.reply({ embeds: [errorEmbed("Sticker name must be 2–30 characters.")] }); return; }
      const sticker = resolveSticker(query);
      if (!sticker) { await ctx.reply({ embeds: [errorEmbed(`Sticker \`${query}\` not found.`)] }); return; }
    ...
    if (sub === "list") {
      ...
      const fmt = (f: StickerFormatType) => ({ [StickerFormatType.PNG]: "PNG", [StickerFormatType.APNG]: "APNG", [StickerFormatType.Lottie]: "Lottie", [StickerFormatType.GIF]: "GIF" }[f] ?? "Unknown");
    ...
    if (sub === "info") {
      const query = ctx.isSlash ? ctx.interaction!.options.getString("sticker", true) : ctx.args[1];
      if (!query) { await ctx.reply({ embeds: [errorEmbed("Please specify a sticker.")] }); return; }
      const sticker = resolveSticker(query);
      if (!sticker) { await ctx.reply({ embeds: [errorEmbed(`Sticker \`${query}\` not found.`)] }); return; }
      const fmt = (f: StickerFormatType) => ({ [StickerFormatType.PNG]: "PNG", [StickerFormatType.APNG]: "APNG", [StickerFormatType.Lottie]: "Lottie", [StickerFormatType.GIF]: "GIF" }[f] ?? "Unknown");
      const embed = baseEmbed("primary")
        ...
        .addFields(
          ...
          { name: "Format", value: fmt(sticker.format), inline: true },
          { name: "Tags (emoji)", value: sticker.tags ?? "None", inline: true },
          { name: "Created", value: `<t:${Math.floor(sticker.createdTimestamp / 1000)}:R>`, inline: true },
          { name: "Description", value: sticker.description ?? "None", inline: false },
          { name: "URL", value: sticker.url, inline: false },
        );
      await ctx.reply({ embeds: [embed] });
      return;
    }
