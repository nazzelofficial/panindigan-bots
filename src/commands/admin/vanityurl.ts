...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "set") {
      ...
      if (!code) { await ctx.reply({ embeds: [errorEmbed("Provide a vanity code.")] }); return; }
      if (!/^[a-zA-Z0-9-]{3,32}$/.test(code)) {
        await ctx.reply({ embeds: [errorEmbed("Vanity code must be 3-32 characters long and contain only letters, numbers, and hyphens.")] }); return;
      }
      ...
      if (!doc?.vanityUrlCode) { await ctx.reply({ embeds: [infoEmbed("No vanity URL configured. Use `/vanityurl set [code]` to set one.")] }); return; }
      await ctx.reply({ embeds: [successEmbed(`Current vanity code: **${doc.vanityUrlCode}**`)] });

    } else if (sub === "remove") {
      const doc = await GuildModel.findOne({ guildId: guild.id }).lean();
      if (!doc?.vanityUrlCode) { await ctx.reply({ embeds: [infoEmbed("No vanity URL configured.")] }); return; }
