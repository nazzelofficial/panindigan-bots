...
import { WarningTemplateModel } from "../../database/models/Moderation.js";
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "add") {
      ...
      if (!name || !reason) { await ctx.reply({ embeds: [errorEmbed("Provide a template name and reason.")] }); return; }
      ...
      try {
        ...
        if (e.code === 11000) await ctx.reply({ embeds: [errorEmbed(`A template named **${name}** already exists.`)] });
        else throw e;
      }
      ...
      if (!name || !targetUser) { await ctx.reply({ embeds: [errorEmbed("Provide a template name and user.")] }); return; }

      const template = await WarningTemplateModel.findOne({ guildId: guild.id, name: name.toLowerCase() }).lean();
      if (!template) { await ctx.reply({ embeds: [errorEmbed(`Template **${name}** not found. Use \`/warningtemplate list\` to see available templates.`)] }); return; }

      const member = await guild.members.fetch(targetUser.id).catch(() => null);
      if (!member) { await ctx.reply({ embeds: [errorEmbed("Member not found in this server.")] }); return; }
      ...
      await sendLogEvent(guild.id, "warn", () =>
        baseEmbed("warning").setTitle("⚠️ Member Warned (Template)").addFields(
          { name: "User", value: `<@${targetUser.id}> (Warning #${warnCount})`, inline: true },
          { name: "Moderator", value: `<@${ctx.userId}>`, inline: true },
          { name: "Template", value: name, inline: true },
          { name: "Reason", value: template.reason, inline: false },
        ),
      );
      ...
      if (!deleted) { await ctx.reply({ embeds: [errorEmbed(`Template **${name}** not found.`)] }); return; }
