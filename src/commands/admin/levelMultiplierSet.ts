...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    config?.customCommands.forEach((cmd: any) => {
    ...
    const updatedCommands = config?.customCommands.filter((cmd: any) => !cmd.name.startsWith('level_multiplier:')) || [];
    Object.entries(levelMultipliers).forEach(([roleId, mult]) => {
      (updatedCommands as any[]).push({ name: `level_multiplier:${roleId}`, response: mult.toString(), createdBy: ctx.userId, createdAt: new Date() });
    });
