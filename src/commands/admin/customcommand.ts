...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("add").setDescription("Add a custom command")
      ...
      .addSubcommand((s) =>
        s.setName("edit").setDescription("Update the response of an existing custom command")
      ...
      .addSubcommand((s) =>
        s.setName("remove").setDescription("Remove a custom command")
      ...
      .addSubcommand((s) =>
        s.setName("info").setDescription("View details of a specific custom command")
  ...
  async execute(ctx) {
    ...
    if (sub === "add") {
      ...
      if (!trigger || !response) { await ctx.reply({ embeds: [errorEmbed("Provide a trigger and response.")] }); return; }
      ...
      const existing: any[] = (cfg as any)?.customCommands ?? [];
      ...
      if (existing.find((c: any) => c.trigger === trigger)) {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $push: { customCommands: { trigger, response, useEmbed, createdBy: ctx.userId, createdAt: new Date() } } },
        { upsert: true },
      );
    ...
    if (sub === "edit") {
      ...
      const response = ctx.isSlash ? ctx.interaction!.options.getString("response", true) : ctx.args.slice(2).join(" ");
      if (!trigger || !response) { await ctx.reply({ embeds: [errorEmbed("Provide a trigger and new response.")] }); return; }

      const result = await GuildModel.findOneAndUpdate(
        { guildId: guild.id, "customCommands.trigger": trigger },
        { $set: { "customCommands.$.response": response } },
      );
      if (!result) { await ctx.reply({ embeds: [errorEmbed(`No custom command with trigger \`${trigger}\` found.`)] }); return; }
    ...
    if (sub === "remove") {
      const trigger = (ctx.isSlash ? ctx.interaction!.options.getString("trigger", true) : ctx.args[1])?.toLowerCase();
      if (!trigger) { await ctx.reply({ embeds: [errorEmbed("Provide a trigger.")] }); return; }
    ...
    if (sub === "list") {
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const cmds: any[] = (cfg as any)?.customCommands ?? [];
      if (!cmds.length) {
    ...
    if (sub === "info") {
      ...
      if (!trigger) { await ctx.reply({ embeds: [errorEmbed("Provide a trigger.")] }); return; }
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const cmd = (cfg as any)?.customCommands?.find((c: any) => c.trigger === trigger);
      if (!cmd) { await ctx.reply({ embeds: [errorEmbed(`No custom command with trigger \`${trigger}\` found.`)] }); return; }
      await ctx.reply({
        embeds: [
          baseEmbed("primary")
            ...
            .addFields(
              ...
              { name: "Uses Embed", value: cmd.useEmbed ? "Yes" : "No", inline: true },
              { name: "Created By", value: cmd.createdBy ? `<@${cmd.createdBy}>` : "Unknown", inline: true },
              { name: "Created", value: cmd.createdAt ? `<t:${Math.floor(new Date(cmd.createdAt).getTime() / 1000)}:R>` : "Unknown", inline: true },
            ),
        ],
      });
      return;
