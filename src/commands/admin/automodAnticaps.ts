...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("on").setDescription("Enable anti-caps protection"),
      )
      .addSubcommand((s) =>
        s.setName("off").setDescription("Disable anti-caps protection"),
      )
      .addSubcommand((s) =>
        s
          .setName("percent")
      ...
      .addSubcommand((s) =>
        s.setName("status").setDescription("View current anti-caps configuration"),
      ),
  async execute(ctx) {
    ...
    if (sub === "on" || sub === "off") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.antiCaps": enabled, "automod.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "percent") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.capsPercent": percent, "automod.antiCaps": true, "automod.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "status") {
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          { name: "Status", value: am.antiCaps ? "✅ Enabled" : "❌ Disabled", inline: true },
