...
const EVENT_TYPES = ["doublecoins", "triplecoins", "doubleloot", "bonusdaily", "luckydrop"] as const;
...
const command: CommandDefinition = {
        ...
        s.setName("start")
          ...
          .addStringOption((o) =>
            o.setName("type").setDescription("Event type").setRequired(true)
              .addChoices(
                { name: "Double Coins (all earnings ×2)", value: "doublecoins" },
                { name: "Triple Coins (all earnings ×3)", value: "triplecoins" },
                { name: "Double Loot (drops & fishing ×2)", value: "doubleloot" },
                { name: "Bonus Daily (daily reward ×3)", value: "bonusdaily" },
                { name: "Lucky Drop (random drops every 30min)", value: "luckydrop" },
              ),
          )
  ...
  async execute(ctx) {
    ...
    if (sub === "start") {
      ...
      if (!ms) { await ctx.reply({ embeds: [errorEmbed("Invalid duration. Use `1h`, `30m`, `2d`, etc.")] }); return; }
      if (!EVENT_LABELS[type]) { await ctx.reply({ embeds: [errorEmbed(`Unknown event type. Valid: ${Object.keys(EVENT_LABELS).join(", ")}`)] }); return; }
      ...
      if (!(doc as any)?.economyEvent?.active) { await ctx.reply({ embeds: [infoEmbed("No active economy event.")] }); return; }
      ...
      const doc = await GuildModel.findOne({ guildId: guild.id }).lean();
      const event = (doc as any)?.economyEvent;
      if (!event?.active || (event.expiresAt && new Date(event.expiresAt) < new Date())) {
        await ctx.reply({ embeds: [infoEmbed("No active economy event.")] }); return;
      }
      await ctx.reply({
        embeds: [
          successEmbed(
