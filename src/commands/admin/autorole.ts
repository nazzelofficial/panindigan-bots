    ...
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s
          ...
          .addStringOption((o) =>
            o.setName("type").setDescription("Who gets this role").setRequired(false)
  ...
  async execute(ctx) {
    ...
    if (sub === "add") {
      ...
      if (botMember && guild.roles.cache.get(role.id)!.position >= botMember.roles.highest.position) {
        await ctx.reply({ embeds: [errorEmbed("That role is higher than or equal to my highest role.")] }); return;
      }
      if (type === "bots") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $pull: { autoRoleIds: role.id }, $set: { autoRoleBotId: null } },
      );
      await ctx.reply({ embeds: [successEmbed(`Auto-role removed: ${role}.`)] });
    } else if (sub === "list") {
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const roles = (cfg as any)?.autoRoleIds ?? [];
      const botRole = (cfg as any)?.autoRoleBotId;
      if (!roles.length && !botRole) { await ctx.reply({ embeds: [infoEmbed("No auto-roles configured.")] }); return; }
      const embed = baseEmbed("primary")
        .setTitle("🎭 Auto-Roles")
        .addFields(
          { name: "Member Roles", value: roles.length ? roles.map((id: string) => `<@&${id}>`).join(", ") : "None", inline: false },
          { name: "Bot Role", value: botRole ? `<@&${botRole}>` : "None", inline: false },
        );
