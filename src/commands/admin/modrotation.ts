...
const command: CommandDefinition = {
    ...
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("setup")
          ...
          .addStringOption((o) =>
            o.setName("schedule").setDescription("Schedule format: 'user1,user2,user3' cycled daily/weekly").setRequired(true),
          )
          .addStringOption((o) =>
            o.setName("interval").setDescription("Rotation interval").setRequired(false)
  ...
  async execute(ctx) {
    ...
    if (sub === "setup") {
      ...
      if (!schedule) { await ctx.reply({ embeds: [errorEmbed("Provide a schedule (comma-separated user mentions or IDs).")] }); return; }

      // Parse user IDs from mentions or raw IDs
      const userIds = schedule.split(",").map((s: string) => s.replace(/\D/g, "").trim()).filter(Boolean);
      if (userIds.length < 2) { await ctx.reply({ embeds: [errorEmbed("Provide at least 2 moderators in the rotation.")] }); return; }
      ...
      const rotation = (doc as any)?.modRotation;
      if (!rotation?.enabled || !rotation.userIds?.length) {
        await ctx.reply({ embeds: [infoEmbed("No moderation rotation configured. Use `/modrotation setup` to set one up.")] }); return;
      }
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          { name: "Current On-Duty", value: `<@${currentMod}>`, inline: true },
          { name: "Interval", value: rotation.interval ?? "weekly", inline: true },
          { name: "Rotation Order", value: rotation.userIds.map((id: string, i: number) => `${i + 1}. <@${id}>${i === rotation.currentIndex % rotation.userIds.length ? " 👈 current" : ""}`).join("\n"), inline: false },
        );
      if (rotation.channelId) embed.addFields({ name: "Announcement Channel", value: `<#${rotation.channelId}>`, inline: true });
      if (rotation.lastRotationAt) embed.addFields({ name: "Last Rotated", value: `<t:${Math.floor(new Date(rotation.lastRotationAt).getTime() / 1000)}:R>`, inline: true });
