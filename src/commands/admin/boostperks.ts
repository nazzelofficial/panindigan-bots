  ...
  async execute(ctx) {
    ...
    if (sub === "setup") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $addToSet: { boostPerkRoleIds: { roleId: role.id, description } } },
        { upsert: true },
      );
      ...
      const perks: any[] = (doc as any)?.boostPerkRoleIds ?? [];
      if (!perks.length) { await ctx.reply({ embeds: [infoEmbed("No booster perks configured. Use `/boostperks setup` to add one.")] }); return; }

      const embed = baseEmbed("premium")
        .setTitle("💎 Booster Perks")
        .setDescription(perks.map((p: any) => `${p.roleId ? `<@&${p.roleId}>` : p} — ${p.description ?? "Booster role"}`).join("\n"))
      ...
      await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $pull: { boostPerkRoleIds: { roleId: role.id } } as any });
