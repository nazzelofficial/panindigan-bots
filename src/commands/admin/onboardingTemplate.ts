...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("enable").setDescription("Enable onboarding for new members"),
      )
      .addSubcommand((s) => s.setName("disable").setDescription("Disable onboarding"))
      .addSubcommand((s) =>
        s.setName("addstep").setDescription("Add an onboarding step")
      ...
      .addSubcommand((s) =>
        s.setName("clearsteps").setDescription("Remove all onboarding steps"),
      ),
  async execute(ctx) {
    ...
    if (sub === "enable") {
      ...
      if (!title || !description) { await ctx.reply({ embeds: [errorEmbed("Provide both a title and description.")] }); return; }
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      if (((cfg as any)?.onboarding?.steps ?? []).length >= 10) { await ctx.reply({ embeds: [errorEmbed("Maximum of 10 onboarding steps.")] }); return; }
      ...
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const steps = (cfg as any)?.onboarding?.steps ?? [];
      const embed = baseEmbed("primary")
        .setTitle("🎉 Onboarding Steps")
        .setDescription(steps.length ? steps.map((s: any, i: number) => `**${i + 1}. ${s.title}**\n${s.description}`).join("\n\n") : "No steps configured.")
        .setFooter({ text: `Status: ${(cfg as any)?.onboarding?.enabled ? "Enabled" : "Disabled"} • ${steps.length}/10 steps` });
