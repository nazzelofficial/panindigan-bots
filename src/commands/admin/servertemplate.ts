...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "save") {
      ...
      if (!name) { await ctx.reply({ embeds: [errorEmbed("Provide a template name.")] }); return; }
      ...
      const snapshot = {
        ...
        channels: guild.channels.cache
          ...
          .map((c) => ({ name: c.name, type: c.type, parent: (c as any).parentId ?? null, position: (c as any).position ?? 0 })),
        capturedAt: new Date(),
        capturedBy: ctx.userId,
      };

      try {
        await ServerTemplateModel.findOneAndUpdate(
          { guildId: guild.id, name: name.toLowerCase() },
          { guildId: guild.id, name: name.toLowerCase(), data: snapshot },
          { upsert: true },
        );
      ...
      if (!template) { await ctx.reply({ embeds: [errorEmbed(`Template **${name}** not found.`)] }); return; }
