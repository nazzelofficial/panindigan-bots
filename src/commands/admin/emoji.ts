...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    function resolveEmoji(query: string) {
    ...
    if (sub === "add") {
      ...
      if (!name || !url) { await ctx.reply({ embeds: [errorEmbed("Please provide a name and image URL.")] }); return; }
      if (!/^[a-zA-Z0-9_]{2,32}$/.test(name)) { await ctx.reply({ embeds: [errorEmbed("Emoji name must be 2–32 characters: letters, numbers, and underscores only.")] }); return; }
      if (guild.emojis.cache.size >= (guild as any).maximumEmojis) { await ctx.reply({ embeds: [errorEmbed(`This server has reached its emoji limit (${(guild as any).maximumEmojis}).`)] }); return; }
      try {
    ...
    if (sub === "delete") {
      ...
      if (!query) { await ctx.reply({ embeds: [errorEmbed("Please specify the emoji to delete.")] }); return; }
      const emoji = resolveEmoji(query);
      if (!emoji) { await ctx.reply({ embeds: [errorEmbed(`Emoji \`${query}\` not found in this server.`)] }); return; }
    ...
    if (sub === "rename") {
      ...
      if (!query || !newName) { await ctx.reply({ embeds: [errorEmbed("Please provide the emoji and new name.")] }); return; }
      if (!/^[a-zA-Z0-9_]{2,32}$/.test(newName)) { await ctx.reply({ embeds: [errorEmbed("Emoji name must be 2–32 characters: letters, numbers, and underscores only.")] }); return; }
      const emoji = resolveEmoji(query);
      if (!emoji) { await ctx.reply({ embeds: [errorEmbed(`Emoji \`${query}\` not found in this server.`)] }); return; }
    ...
    if (sub === "info") {
      const query = ctx.isSlash ? ctx.interaction!.options.getString("emoji", true) : ctx.args[1];
      if (!query) { await ctx.reply({ embeds: [errorEmbed("Please specify an emoji.")] }); return; }
      const emoji = resolveEmoji(query);
      if (!emoji) { await ctx.reply({ embeds: [errorEmbed(`Emoji \`${query}\` not found in this server.`)] }); return; }
      const embed = baseEmbed("primary")
