...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (sub === "command") {
      ...
      if (!ctx.client.commands.has(cmdName)) { await ctx.reply({ embeds: [errorEmbed(`Command \`${cmdName}\` doesn't exist.`)] }); return; }
      if (["disable", "enable"].includes(cmdName)) { await ctx.reply({ embeds: [errorEmbed("You cannot disable the disable/enable command itself.")] }); return; }

      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $addToSet: { disabledCommands: cmdName } },
        { upsert: true },
      );
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $pull: { disabledCommands: cmdName } },
      );
      await ctx.reply({ embeds: [successEmbed(`Command \`${cmdName}\` has been enabled.`)] });
    } else if (sub === "list") {
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const disabled: string[] = (cfg as any)?.disabledCommands ?? [];
      if (!disabled.length) { await ctx.reply({ embeds: [infoEmbed("No commands are disabled.")] }); return; }
