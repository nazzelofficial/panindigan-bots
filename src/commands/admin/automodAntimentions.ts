...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s.setName("status").setDescription("View current anti-mention configuration"),
      ),
  async execute(ctx) {
    ...
    if (sub === "on") {
      ...
      const currentLimit: number = (cfg as any)?.automod?.antiMentionLimit ?? 0;
      const limit = currentLimit > 0 ? currentLimit : 5;
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.antiMentionLimit": limit, "automod.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "off") {
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.antiMentionLimit": 0 } },
      );
    ...
    if (sub === "status") {
      const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
      const limit: number = (cfg as any)?.automod?.antiMentionLimit ?? 0;
      const embed = baseEmbed("primary")
        .setTitle("📣 Anti-Mention Status")
        .addFields(
          { name: "Status", value: limit > 0 ? "✅ Enabled" : "❌ Disabled", inline: true },
          { name: "Max mentions", value: limit > 0 ? `${limit} per message` : "N/A", inline: true },
        );
