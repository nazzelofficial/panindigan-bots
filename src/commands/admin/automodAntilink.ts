    ...
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s
          ...
          .addStringOption((o) =>
            o.setName("domain").setDescription("Domain to whitelist (e.g. youtube.com)").setRequired(false),
          ),
      )
      .addSubcommand((s) =>
        s.setName("status").setDescription("View current anti-link configuration"),
      ),
  async execute(ctx) {
    ...
    if (sub === "on" || sub === "off") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.antiLink": enabled, "automod.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "whitelist") {
      ...
      if (action === "add") {
        if (!domain) { await ctx.reply({ embeds: [errorEmbed("Please provide a domain to whitelist.")] }); return; }
        await GuildModel.findOneAndUpdate(
          { guildId: guild.id },
          { $addToSet: { "automod.linkWhitelistDomains": domain } },
          { upsert: true },
        );
        await ctx.reply({ embeds: [successEmbed(`**${domain}** added to the link whitelist.`)] });
      } else if (action === "remove") {
        if (!domain) { await ctx.reply({ embeds: [errorEmbed("Please provide a domain to remove.")] }); return; }
        await GuildModel.findOneAndUpdate(
          { guildId: guild.id },
          { $pull: { "automod.linkWhitelistDomains": domain } },
        );
        await ctx.reply({ embeds: [successEmbed(`**${domain}** removed from the link whitelist.`)] });
      } else if (action === "list") {
        const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
        const domains: string[] = (cfg as any)?.automod?.linkWhitelistDomains ?? [];
    ...
    if (sub === "status") {
      ...
      const embed = baseEmbed("primary")
        ...
        .addFields(
          { name: "Status", value: am.antiLink ? "✅ Enabled" : "❌ Disabled", inline: true },
          { name: "Also blocks invites", value: am.antiInvite ? "✅ Yes" : "❌ No", inline: true },
          { name: "Whitelisted domains", value: (am.linkWhitelistDomains ?? []).length.toString(), inline: true },
        );
      await ctx.reply({ embeds: [embed] });
      return;
    }
