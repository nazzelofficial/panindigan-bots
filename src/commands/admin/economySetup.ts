    ...
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s
          ...
          .addStringOption((o) =>
            o.setName("name").setDescription('Currency name (e.g. "Coins")').setRequired(true).setMaxLength(32),
          )
          .addStringOption((o) =>
            o.setName("symbol").setDescription('Currency symbol (e.g. "🪙")').setRequired(false).setMaxLength(8),
          ),
      )
      .addSubcommand((s) =>
        s
          ...
          .addIntegerOption((o) =>
            o.setName("amount").setDescription("Starting coins").setRequired(true).setMinValue(0).setMaxValue(100000),
          ),
      )
      .addSubcommand((s) =>
        s.setName("view").setDescription("View current economy configuration"),
      )
      .addSubcommand((s) =>
        s.setName("reset").setDescription("Reset economy config to defaults"),
      ),
  async execute(ctx) {
    ...
    if (sub === "currency") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "economy.currencyName": name, "economy.currencySymbol": symbol } },
        { upsert: true },
      );
      ...
      const eco = (cfg as any)?.economy ?? {};
      const embed = baseEmbed("primary")
        .setTitle("💰 Economy Configuration")
        .addFields(
          { name: "Currency", value: `${eco.currencySymbol ?? "🪙"} ${eco.currencyName ?? "Coins"}`, inline: true },
          { name: "Starting Balance", value: `${(eco.startingBalance ?? 0).toLocaleString()}`, inline: true },
          { name: "Tax Rate", value: `${eco.taxRate ?? 0}%`, inline: true },
        );
