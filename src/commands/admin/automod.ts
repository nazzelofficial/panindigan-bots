...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder)
      .addSubcommand((s) =>
        s.setName("setlog").setDescription("Set the automod violation log channel")
          .addChannelOption((o) => o.setName("channel").setDescription("Log channel").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("spamthreshold").setDescription("Set the anti-spam detection threshold (messages per 5 seconds)")
          .addIntegerOption((o) =>
            o.setName("count").setDescription("Number of messages before triggering (default: 5)").setRequired(true).setMinValue(2).setMaxValue(20),
          ),
      )
      .addSubcommand((s) =>
        s.setName("capsthreshold").setDescription("Set the minimum percentage of uppercase characters to trigger anti-caps")
          .addIntegerOption((o) =>
            o.setName("percent").setDescription("Percentage of uppercase (default: 70)").setRequired(true).setMinValue(50).setMaxValue(100),
          ),
      )
      .addSubcommand((s) =>
        s.setName("emojithreshold").setDescription("Set the maximum number of emoji allowed per message")
          .addIntegerOption((o) =>
            o.setName("count").setDescription("Max emoji per message (default: 10)").setRequired(true).setMinValue(3).setMaxValue(30),
          ),
      )
      .addSubcommand((s) =>
        s.setName("raidmode").setDescription("Toggle raid mode — forces strict verification on all new joins")
          .addBooleanOption((o) => o.setName("enabled").setDescription("Enable or disable").setRequired(true)),
      )
      .addSubcommand((s) =>
        s.setName("muteduration").setDescription("Set how long automod mutes last")
          .addIntegerOption((o) =>
            o.setName("minutes").setDescription("Duration in minutes (default: 10)").setRequired(true).setMinValue(1).setMaxValue(1440),
          ),
      )
  ...
  async execute(ctx) {
    ...
    if (sub === "setlog") {
      ...
      const chId = (ch as any)?.id ?? ctx.args[1]?.replace(/\D/g, "");
      if (!chId) { await ctx.reply({ embeds: [errorEmbed("Provide a log channel.")] }); return; }
      // Store in logging.channels map under 'automod' key
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "logging.channels.automod": chId, "logging.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "spamthreshold") {
      ...
      if (!count) { await ctx.reply({ embeds: [errorEmbed("Provide a valid threshold.")] }); return; }
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.antiSpam": true, "automod.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "capsthreshold") {
      ...
      if (!pct) { await ctx.reply({ embeds: [errorEmbed("Provide a valid percentage.")] }); return; }
      // capsPercent is a real schema field on AutoModSchema
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "automod.capsPercent": pct, "automod.antiCaps": true, "automod.enabled": true } },
        { upsert: true },
      );
    ...
    if (sub === "raidmode") {
      ...
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $set: { "raidMode.enabled": enabled, "raidMode.enabledAt": enabled ? new Date() : null, "raidMode.enabledBy": ctx.userId } },
        { upsert: true },
      );
      await ctx.reply({
        embeds: [
    ...
    if (sub === "muteduration") {
      ...
      if (!mins) { await ctx.reply({ embeds: [errorEmbed("Provide a valid duration.")] }); return; }
    ...
    if (sub === "overview") {
      ...
      const logCh = (cfg as any)?.logging?.channels?.automod;
      const embed = baseEmbed("primary").setTitle("🛡️ Automod Admin Overview").addFields(
        { name: "Log Channel",   value: logCh ? `<#${logCh}>` : "Not set",       inline: true },
        { name: "Raid Mode",     value: rm.enabled ? "🚨 ON" : "Off",             inline: true },
        { name: "Caps Threshold", value: `${am.capsPercent ?? 70}%`,              inline: true },
      );
