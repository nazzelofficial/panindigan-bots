...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const query: any = { guildId, available: true };
    if (search) query.item = { $regex: search, $options: "i" };
    ...
    const lines = listings.map((l: any, i: number) =>
      `**${page * perPage + i + 1}.** \`${l._id}\` — **${l.item}** × ${l.quantity} @ **${l.price.toLocaleString()} coins** — <@${l.sellerId}>${l.description ? `\n   *${l.description}*` : ""}`
    );
