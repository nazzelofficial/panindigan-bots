...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const targetProfile = targetUser?.guilds.find((g: any) => g.guildId === guildId);
    if (trade.requestedCoins > 0) {
      const targetBalance = (targetProfile as any)?.balance ?? 0;
    ...
    if (trade.offeredCoins > 0) {
      ...
      const offererProfile = offererUser?.guilds.find((g: any) => g.guildId === guildId);
      const offererBalance = (offererProfile as any)?.balance ?? 0;
    ...
    if (trade.offeredCoins > 0) {
      ...
      const offererProfile = offererUser?.guilds.find((g: any) => g.guildId === guildId);
      if (offererProfile) {
        (offererProfile as any).balance = ((offererProfile as any).balance ?? 0) - trade.offeredCoins;
        await offererUser?.save();
      }
      if (targetProfile) {
        (targetProfile as any).balance = ((targetProfile as any).balance ?? 0) + trade.offeredCoins;
        await targetUser?.save();
      }
    }
    if (trade.requestedCoins > 0) {
      if (targetProfile) {
        (targetProfile as any).balance = ((targetProfile as any).balance ?? 0) - trade.requestedCoins;
        await targetUser?.save();
      }
      const offererUser = await UserModel.findOne({ userId: trade.offererId });
      const offererProfile = offererUser?.guilds.find((g: any) => g.guildId === guildId);
      if (offererProfile) {
        (offererProfile as any).balance = ((offererProfile as any).balance ?? 0) + trade.requestedCoins;
        await offererUser?.save();
      }
    }
      ...
      await offerer.send({
        embeds: [
          new EmbedBuilder()
            ...
            .addFields(
              { name: "You Gave", value: trade.offeredCoins > 0 ? `${trade.offeredCoins.toLocaleString()} coins` : "Nothing", inline: true },
              { name: "You Received", value: trade.requestedCoins > 0 ? `${trade.requestedCoins.toLocaleString()} coins` : "Nothing", inline: true },
            ),
        ],
      });
    } catch { /* DMs disabled */ }
    ...
    const embed = baseEmbed("success")
      .setTitle("✅ Trade Accepted")
      .addFields(
        { name: "Trading With", value: `<@${trade.offererId}>`, inline: true },
        { name: "You Received", value: trade.offeredCoins > 0 ? `${trade.offeredCoins.toLocaleString()} coins` : "Nothing", inline: true },
        { name: "You Gave", value: trade.requestedCoins > 0 ? `${trade.requestedCoins.toLocaleString()} coins` : "Nothing", inline: true },
      );
