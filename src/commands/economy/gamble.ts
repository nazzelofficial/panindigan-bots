...
const SLOT_MULTIPLIERS: Record<string, number> = { "💎": 10, "7️⃣": 7, "⭐": 5, "🍇": 3, "🍊": 2, "🍋": 1.5, "🍒": 1.2 };

function spin(): string[] {
  return Array.from({ length: 3 }, () => SLOT_SYMBOLS[Math.floor(Math.random() * SLOT_SYMBOLS.length)]);
}

function slotsMultiplier(reels: string[]): number {
  if (reels[0] === reels[1] && reels[1] === reels[2]) return (SLOT_MULTIPLIERS[reels[0]] ?? 1) * 3;
  if (reels[0] === reels[1] || reels[1] === reels[2]) return 0.5;
  return 0;
}
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!betAmt || betAmt < 1) { await ctx.reply({ embeds: [errorEmbed("Invalid bet amount.")] }); return; }
    if (betAmt > wallet) { await ctx.reply({ embeds: [errorEmbed(`Insufficient funds. Wallet: 🪙 **${wallet.toLocaleString()}**.`)] }); return; }
    ...
    if (sub === "slots") {
      ...
      (profile as any).balance = wallet + net;
      (profile as any).totalGambled = ((profile as any).totalGambled ?? 0) + betAmt;
      if (net > 0) (profile as any).totalGambledWon = ((profile as any).totalGambledWon ?? 0) + net;
      if (net > 0) (profile as any).gamesWon = ((profile as any).gamesWon ?? 0) + 1;
      else if (net < 0) (profile as any).gamesLost = ((profile as any).gamesLost ?? 0) + 1;
      else (profile as any).gamesTied = ((profile as any).gamesTied ?? 0) + 1;
      await user.save();
      const embed = baseEmbed(net > 0 ? "success" : net < 0 ? "danger" : "info")
        ...
        .setFooter({ text: `Balance: ${((profile as any).balance).toLocaleString()} 🪙` });
      ...
      (profile as any).balance = wallet + net;
      (profile as any).totalGambled = ((profile as any).totalGambled ?? 0) + betAmt;
      if (won) { (profile as any).totalGambledWon = ((profile as any).totalGambledWon ?? 0) + betAmt; (profile as any).gamesWon = ((profile as any).gamesWon ?? 0) + 1; }
      else (profile as any).gamesLost = ((profile as any).gamesLost ?? 0) + 1;
      await user.save();
      await ctx.reply({ embeds: [baseEmbed(won ? "success" : "danger").setTitle(`🪙 Coinflip — ${flip.toUpperCase()}`).setDescription(won ? `✅ You guessed **${side}** correctly! +**${betAmt.toLocaleString()} 🪙**` : `❌ It was **${flip}**. You lost **${betAmt.toLocaleString()} 🪙**.`).setFooter({ text: `Balance: ${((profile as any).balance).toLocaleString()} 🪙` })] });
    } else {
      // Roulette
      ...
      if (!isNaN(choiceNum) && choiceNum === roll) mult = 35;
      else if (choice === "green" && isGreen) mult = 14;
      else if (choice === "red" && isRed) mult = 2;
      else if (choice === "black" && isBlack) mult = 2;

      const winnings = mult > 0 ? Math.floor(betAmt * mult) : 0;
      const net = winnings - betAmt;
      (profile as any).balance = wallet + net;
      (profile as any).totalGambled = ((profile as any).totalGambled ?? 0) + betAmt;
      if (net > 0) { (profile as any).totalGambledWon = ((profile as any).totalGambledWon ?? 0) + net; (profile as any).gamesWon = ((profile as any).gamesWon ?? 0) + 1; }
      else (profile as any).gamesLost = ((profile as any).gamesLost ?? 0) + 1;
      await user.save();
      await ctx.reply({ embeds: [baseEmbed(net > 0 ? "success" : "danger").setTitle("🎡 Roulette").setDescription(`The ball landed on **${roll}** (${color})!\n\n${net > 0 ? `🎉 You won **${winnings.toLocaleString()} 🪙**!` : `😔 You lost **${betAmt.toLocaleString()} 🪙**.`}`).setFooter({ text: `Balance: ${((profile as any).balance).toLocaleString()} 🪙` })] });
    }
  },
};
