...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const shopItems: any[] = (cfg as any)?.shopItems ?? [];

    if (sub === "view") {
      if (!shopItems.length) { await ctx.reply({ embeds: [infoEmbed("The shop is empty. An admin can add items with `/shop add`.")] }); return; }
      ...
      if (!item) { await ctx.reply({ embeds: [errorEmbed("Item not found in the shop.")] }); return; }
      if (item.stock >= 0 && item.stock < qty) { await ctx.reply({ embeds: [errorEmbed(`Not enough stock. Only **${item.stock}** left.`)] }); return; }
      ...
      if (!profile) { user.guilds.push({ guildId: guild.id } as any); await user.save(); profile = user.guilds[user.guilds.length - 1]; }
      const wallet: number = (profile as any).balance ?? 0;
      if (wallet < totalCost) { await ctx.reply({ embeds: [errorEmbed(`You need 🪙 **${totalCost.toLocaleString()}** but only have 🪙 **${wallet.toLocaleString()}**.`)] }); return; }

      (profile as any).balance -= totalCost;
      (profile as any).totalSpent = ((profile as any).totalSpent ?? 0) + totalCost;
      const inv: any[] = (profile as any).inventory ?? [];
      const existing = inv.find((i) => i.itemId === item.id);
      if (existing) existing.quantity = (existing.quantity ?? 1) + qty;
      else inv.push({ itemId: item.id, name: item.name, quantity: qty });
      (profile as any).inventory = inv;
      await user.save();
      ...
      await ctx.reply({ embeds: [successEmbed(`Purchased **${qty}× ${item.name}** for 🪙 **${totalCost.toLocaleString()}**. Balance: 🪙 **${((profile as any).balance).toLocaleString()}**.`)] });
    } else if (sub === "add") {
      const member = ctx.interaction?.member ?? ctx.message?.member;
      if (!(member as any)?.permissions?.has("ManageGuild")) { await ctx.reply({ embeds: [errorEmbed("You need **Manage Server** to add shop items.")] }); return; }
      ...
      if (!name || !price) { await ctx.reply({ embeds: [errorEmbed("Provide item name and price.")] }); return; }
      const id = name.toLowerCase().replace(/\s+/g, "-");
      await GuildModel.findOneAndUpdate(
        { guildId: guild.id },
        { $push: { shopItems: { id, name, price, description, stock } } },
        { upsert: true },
      );
      await ctx.reply({ embeds: [successEmbed(`Added **${name}** to the shop for 🪙 **${price.toLocaleString()}**.`)] });
    } else if (sub === "remove") {
      const member = ctx.interaction?.member ?? ctx.message?.member;
      if (!(member as any)?.permissions?.has("ManageGuild")) { await ctx.reply({ embeds: [errorEmbed("You need **Manage Server** to remove shop items.")] }); return; }
      const name = ctx.isSlash ? ctx.interaction!.options.getString("name", true) : ctx.args[1];
