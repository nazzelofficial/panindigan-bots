...
const command: CommandDefinition = {
    ...
    (b as SlashCommandBuilder)
      ...
      .addSubcommand((s) =>
        s
          ...
          .addStringOption((o) =>
            o.setName("job").setDescription("Job ID").setRequired(true)
  ...
  async execute(ctx) {
    ...
    const currentJobId: string | null = (profile as any).jobId ?? null;
    ...
    if (sub === "info") {
      ...
      if (!job) { await ctx.reply({ embeds: [errorEmbed(`No job found with ID \`${jobId}\`. Use \`/jobs list\`.`)] }); return; }
      const embed = baseEmbed("primary")
        .setTitle(`💼 ${job.name}`)
        .addFields(
          { name: "Job ID", value: `\`${job.id}\``, inline: true },
          { name: "Pay Range", value: `🪙 ${job.payMin}–${job.payMax} per shift`, inline: true },
          { name: "Cooldown", value: `${job.cooldownHours} hours`, inline: true },
        )
    ...
    if (sub === "apply") {
      ...
      const job = JOBS.find((j) => j.id === jobId);
      if (!job) { await ctx.reply({ embeds: [errorEmbed(`No job found with ID \`${jobId}\`. Use \`/jobs list\`.`)] }); return; }
      if (currentJobId === job.id) { await ctx.reply({ embeds: [infoEmbed(`Naka-apply ka na sa **${job.name}**.`)] }); return; }
      (profile as any).jobId = job.id;
      await user.save();
    ...
    if (sub === "resign") {
      if (!currentJobId) { await ctx.reply({ embeds: [infoEmbed("You do not have a current job.")] }); return; }
      const jobName = JOBS.find((j) => j.id === currentJobId)?.name ?? currentJobId;
      (profile as any).jobId = null;
      await user.save();
    ...
    if (sub === "current") {
      if (!currentJobId) { await ctx.reply({ embeds: [infoEmbed("You do not have a current job. Use `/jobs apply` para mag-apply.")] }); return; }
      const job = JOBS.find((j) => j.id === currentJobId);
      if (!job) { await ctx.reply({ embeds: [infoEmbed("Invalid job ID on your profile. Use `/jobs resign` and apply again.")] }); return; }
      await ctx.reply({
        embeds: [
          baseEmbed("primary")
            ...
            .addFields(
              ...
              { name: "Pay", value: `🪙 ${job.payMin}–${job.payMax}/shift`, inline: true },
              { name: "Cooldown", value: `${job.cooldownHours}h`, inline: true },
            ),
        ],
      });
      return;
