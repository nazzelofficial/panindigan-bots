...
function weightedRandom(): typeof ORES[number] {
...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const lastMine: Date | null = (profile as any).lastMine ?? null;
    const now = Date.now();
    ...
    (profile as any).lastMine = new Date();
    const ore = weightedRandom();
    (profile as any).balance = ((profile as any).balance ?? 0) + ore.value;
    await user.save();
    ...
    const embed = baseEmbed("primary")
      ...
      .addFields(
        { name: "💰 Coins Earned", value: `+${ore.value.toLocaleString()}`, inline: true },
        { name: "💼 Balance", value: `🪙 **${((profile as any).balance).toLocaleString()}**`, inline: true },
      )
