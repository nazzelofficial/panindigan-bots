...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!auction.highestBidderId) {
      ...
      if (winnerUser) {
        const winnerProfile = winnerUser.guilds.find((g: any) => g.guildId === guildId);
        if (winnerProfile) {
          (winnerProfile as any).balance = ((winnerProfile as any).balance ?? 0) - auction.currentBid;
      ...
      resultEmbed = baseEmbed("success")
        ...
        .addFields(
          ...
          { name: "💰 Winning Bid", value: `🪙 ${auction.currentBid.toLocaleString()}`, inline: true },
        )
    ...
    try {
      const channel = guild.channels.cache.get(auction.channelId) as TextChannel;
