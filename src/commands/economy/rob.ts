...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    if (!target || target.bot || target.id === ctx.userId) {
      await ctx.reply({ embeds: [errorEmbed("Choose a valid target user.")] }); return;
    }

    const robberDoc = await UserModel.findOneAndUpdate(
      { userId: ctx.userId },
      { $setOnInsert: { userId: ctx.userId } },
      { upsert: true, new: true },
    );
    let robberProfile = robberDoc.guilds.find((g: any) => g.guildId === guild.id);
    if (!robberProfile) {
      robberDoc.guilds.push({ guildId: guild.id } as any);
      await robberDoc.save();
      robberProfile = robberDoc.guilds[robberDoc.guilds.length - 1];
    }

    const lastRob: Date | null = (robberProfile as any).lastCrime;
    const now = Date.now();
    ...
    const victimProfile = (victimDoc as any)?.guilds?.find((g: any) => g.guildId === guild.id) ?? {};
    const victimWallet: number = victimProfile.balance ?? 0;
    ...
    (robberProfile as any).lastCrime = new Date();
    ...
    if (success) {
      ...
      (robberProfile as any).balance = ((robberProfile as any).balance ?? 0) + stolen;
      (robberProfile as any).totalEarned = ((robberProfile as any).totalEarned ?? 0) + stolen;
      await robberDoc.save();

      // Deduct from victim
      await UserModel.updateOne(
        { userId: target.id, "guilds.guildId": guild.id },
        { $inc: { "guilds.$.balance": -stolen } },
      );
      ...
      const wallet: number = (robberProfile as any).balance ?? 0;
      (robberProfile as any).balance = Math.max(0, wallet - fine);
      await robberDoc.save();
