...
const command: CommandDefinition = {
  ...
  slashData: (b) =>
    (b as SlashCommandBuilder).addStringOption((o) =>
      o.setName("name").setDescription("Name of the saved queue to delete").setRequired(true).setAutocomplete(true),
    ),
  ...
  async execute(ctx) {
    ...
    if (!name) { await ctx.reply({ embeds: [errorEmbed("Please provide the name of the saved queue to delete.")] }); return; }
    const deleted = await SavedQueueModel.findOneAndDelete({ guildId: guild.id, userId: ctx.userId, name });
    if (!deleted) { await ctx.reply({ embeds: [errorEmbed(`No saved queue named **"${name}"** found.`)] }); return; }
