...
const command: CommandDefinition = {
  ...
  async execute(ctx) {
    ...
    const embed = baseEmbed("primary")
      ...
      .addFields(
        { name: "Status", value: paused ? "⏸️ Paused" : "▶️ Playing", inline: true },
        { name: "Volume", value: `🔊 ${volume}%`, inline: true },
        { name: "Queue", value: `${player.queue?.tracks?.length ?? 0} track(s) queued`, inline: true },
      );

    const row1 = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("music:previous").setEmoji("⏮️").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("music:pause").setEmoji(paused ? "▶️" : "⏸️").setStyle(ButtonStyle.Primary),
  ...
  registerComponents(client) {
    client.componentHandlers.set("music", async (interaction) => {
      ...
      if (!client.lavalink) { await interaction.reply({ content: "Music not configured.", ephemeral: true }); return; }
      const guild = interaction.guild;
      if (!guild) return;
      const player = (client.lavalink as any).players?.get(guild.id);
      if (!player) { await interaction.reply({ content: "No active player.", ephemeral: true }); return; }
