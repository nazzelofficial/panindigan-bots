...
const recentMessages = new Map<string, number[]>(); // `${guildId}:${userId}` -> timestamps, for spam/flood detection

function isWhitelisted(config: any, message: Message): boolean {
  if (config.automod.whitelistUsers?.includes(message.author.id)) return true;
  if (message.member?.roles.cache.some((r) => config.automod.whitelistRoles?.includes(r.id))) return true;
  if (config.automod.whitelistChannels?.includes(message.channelId)) return true;
  return false;
}
 ...
 */
export async function runAutomodChecks(client: PanindiganClient, message: Message): Promise<boolean> {
  if (!message.guildId || !message.member || message.member.permissions.has("Administrator")) return false;

  const config = await GuildModel.findOne({ guildId: message.guildId }).lean();
  if (!config?.automod?.enabled) return false;
  if (isWhitelisted(config, message)) return false;
  ...
  if (automod.antiLink) {
    ...
    const allowed = links.every((link) => automod.linkWhitelistDomains?.some((d: string) => link.includes(d)));
  ...
  if (automod.badWords?.length) {
    ...
    if (automod.badWords.some((w: string) => lower.includes(w.toLowerCase()))) {
...
async function violate(client: PanindiganClient, message: Message, reason: string): Promise<void> {
  await message.delete().catch(() => {});
