...
import { COLORS, MUSIC, CUSTOM_ID } from "../../constants/index.js";
...
export interface MusicDashboardState {
  guildId: string;
 ...
 */
export function generateProgressBar(position: number, duration: number, length = 20): string {
 ...
 */
export function buildNowPlayingEmbed(state: MusicDashboardState): EmbedBuilder {
  const { nowPlaying, volume, loopMode, isShuffled, isPaused, queue } = state;
  
  if (!nowPlaying) {
    return EmbedFactory.music(
      "Walang naka-play na kanta ngayon.\n\nGamitin ang `/play` para mag-queue ng kanta!",
      "🎵 Now Playing",
    );
  }
  ...
  const embed = EmbedFactory.music("", "🎵 Now Playing")
    ...
    .setDescription(
      `${nowPlaying.artist ? `**Artist:** ${nowPlaying.artist}\n` : ""}` +
    ...
    .addFields(
      ...
      { name: "🔁 Loop", value: loopMode === "off" ? "Off" : loopMode === "track" ? "Track" : "Queue", inline: true },
      { name: "🔀 Shuffle", value: isShuffled ? "On" : "Off", inline: true },
      { name: "📋 Queue", value: `${queue.length} track${queue.length !== 1 ? "s" : ""}`, inline: true },
    );
 ...
 */
export function buildQueueEmbed(state: MusicDashboardState, page = 1, pageSize = 10): EmbedBuilder {
  const { queue, nowPlaying } = state;
  
  if (queue.length === 0 && !nowPlaying) {
    return EmbedFactory.music(
      "Ang queue ay walang laman.\n\nGamitin ang `/play` para magdagdag ng kanta!",
      "📋 Music Queue",
    );
  }
  ...
  const embed = EmbedFactory.music(description || "Walang tracks sa queue.", "📋 Music Queue")
    .addFields(
      { name: "📊 Queue Stats", value: `${queue.length} track${queue.length !== 1 ? "s" : ""} · ${formatDurationHuman(totalDuration)} total`, inline: true },
 ...
 */
export function buildPlaybackControls(state: MusicDashboardState): ActionRowBuilder<ButtonBuilder> {
 ...
 */
export function buildSecondaryControls(state: MusicDashboardState): ActionRowBuilder<ButtonBuilder> {
 ...
 */
export function buildQueueNavigation(currentPage: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
 ...
 */
export function buildFilterMenu(activeFilters: string[]): ActionRowBuilder<StringSelectMenuBuilder> {
  const filters = [
 ...
 */
export function generateRecommendations(queue: QueueItem[], limit = 5): string[] {
 ...
 */
export function buildRecommendationsEmbed(queue: QueueItem[]): EmbedBuilder {
  const recommendations = generateRecommendations(queue);
  
  if (recommendations.length === 0) {
    return EmbedFactory.music(
      "Magdagdag ng maraming kanta sa queue para makakuha ng recommendations!",
      "🔮 Smart Recommendations",
    );
  }
  ...
  return EmbedFactory.music(
    `${description}\n\n_I-click ang recommendation o gamitin ang \`/play\` para maghanap._`,
    "🔮 Smart Recommendations",
  );
}
