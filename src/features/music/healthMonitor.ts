...
import { getNodeHealthInfo, isNodeHealthy, getAllNodesHealthInfo } from "../../utils/music.js";
...
/**
 * Node health tracking state
 */
interface NodeHealthTracking {
  nodeId: string;
...
const nodeHealthTracking = new Map<string, NodeHealthTracking>();

/**
 * Starts the health monitoring background service
 * @param client The Discord client instance
 */
export function startHealthMonitor(client: PanindiganClient): void {
  if (isMonitoring) {
 ...
 * @param client The Discord client instance
 */
function performHealthCheck(client: PanindiganClient): void {
  if (!client.lavalink) {
    return;
  }
...
export function getHealthMonitorStatus(): {
  ...
  intervalMs: number;
  nodeTracking: Record<string, NodeHealthTracking>;
} {
  const nodeTrackingObj: Record<string, NodeHealthTracking> = {};
 ...
 */
export function getNodeHealthTracking(nodeId: string): NodeHealthTracking | null {
