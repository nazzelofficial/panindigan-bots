...
import type { Queue, Track } from "lavalink-client";
...
export function createProgressBar(current: number, total: number, length = 20): string {
...
// ─── Metadata Resolver ───────────────────────────────────────────────────────
export function resolveMetadata(track: Track): {
  title: string;
  artist: string;
  album: string | null;
  artwork: string;
...
// ─── Now Playing Embed ────────────────────────────────────────────────────────
export function createNowPlayingEmbed(queue: Queue, track: Track, position: number): EmbedBuilder {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: "Artist", value: metadata.artist, inline: true },
      { name: "Duration", value: metadata.duration, inline: true },
      { name: "Source", value: metadata.source.charAt(0).toUpperCase() + metadata.source.slice(1), inline: true },
    )
    .addFields(
      { 
        name: "Progress", 
  ...
  // Add badges
  const badges: string[] = [];
  if (metadata.isStream) badges.push("🔴 Live");
  if (metadata.isExplicit) badges.push("🔞 Explicit");
  if (metadata.isVerified) badges.push("✓ Verified");
  ...
  embed.addFields(
    { name: "Queue Position", value: "1 (Current)", inline: true },
    { name: "Queue Size", value: queueSize > 0 ? `${queueSize} songs` : "Empty", inline: true },
    { name: "Remaining", value: queueSize > 0 ? formatDuration(queueDuration) : "None", inline: true },
  );
...
export function createTrackFinishedEmbed(track: Track): EmbedBuilder {
  const metadata = resolveMetadata(track);
  const color = SOURCE_COLORS[metadata.source] || 0x5865F2;
...
export function createAutoPlayingNextEmbed(track: Track): EmbedBuilder {
  const metadata = resolveMetadata(track);
  const color = SOURCE_COLORS[metadata.source] || 0x5865F2;
...
export function createQueueFinishedEmbed(): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0x5865F2)
    .setTitle("🎵 Queue Finished")
    .setDescription("All songs have finished playing.\n\nThank you for listening!")
    .addFields(
      { name: "Status", value: "Leaving voice channel in 30 seconds...", inline: false },
    )
    .setTimestamp();
}

// ─── Autoplay Started Embed ────────────────────────────────────────────────────
export function createAutoplayStartedEmbed(): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0x5865F2)
...
// ─── Reconnecting Embed ────────────────────────────────────────────────────────
export function createReconnectingEmbed(): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0xFEE75C)
...
// ─── Node Restored Embed ────────────────────────────────────────────────────────
export function createNodeRestoredEmbed(nodeName: string): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0x57F287)
...
// ─── Premium Button Row ────────────────────────────────────────────────────────
export function createMusicButtonRow(isPaused: boolean, loopMode: string, isShuffle: boolean): ActionRowBuilder<ButtonBuilder> {
...
// ─── Queue Embed ──────────────────────────────────────────────────────────────
export function createQueueEmbed(queue: Queue, page: number = 1, perPage: number = 10): EmbedBuilder {
  const currentTrack = queue.current;
  const tracks = queue.tracks as Track[];
...
export function createQueueNavigationButtons(page: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
  const row = new ActionRowBuilder<ButtonBuilder>();
...
export function createSearchResultsEmbed(query: string, tracks: Track[], page: number = 1, perPage: number = 5): EmbedBuilder {
  const totalPages = Math.ceil(tracks.length / perPage) || 1;
...
export function createSearchNavigationButtons(page: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
  const row = new ActionRowBuilder<ButtonBuilder>();
...
// ─── Lyrics Embed ──────────────────────────────────────────────────────────────
export function createLyricsEmbed(title: string, artist: string, lyrics: string, page: number = 1, totalPages: number = 1): EmbedBuilder {
  const embed = new EmbedBuilder()
    .setColor(0x5865F2)
    .setTitle("🎤 Lyrics")
...
export function createLyricsNavigationButtons(page: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
  const row = new ActionRowBuilder<ButtonBuilder>();
...
export function createPlaylistLoadingEmbed(playlistName: string, artwork: string, loaded: number, total: number): EmbedBuilder {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Loaded", value: `${loaded} / ${total} songs`, inline: true },
      { name: "Status", value: "Processing...", inline: true },
    )
    .setTimestamp();
}

export function createPlaylistLoadedEmbed(playlistName: string, artwork: string, loaded: number, totalDuration: number): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0x57F287)
    ...
    .setThumbnail(artwork)
    .addFields(
      { name: "Songs Imported", value: `${loaded} songs`, inline: true },
...
// ─── Filter UI Embed ────────────────────────────────────────────────────────────
export function createFilterUIEmbed(activeFilters: string[]): EmbedBuilder {
  const embed = new EmbedBuilder()
    .setColor(0x5865F2)
...
export function createFilterSelectMenu(): ActionRowBuilder<any> {
...
export function createSearchingEmbed(query: string): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0xFEE75C)
    .setTitle("🔍 Searching...")
    .setDescription(`Looking for: **${query}**`)
    .addFields(
      { name: "Status", value: "Searching across platforms...", inline: false },
    )
    .setTimestamp();
}

export function createResolvingEmbed(track: string): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0xFEE75C)
    .setTitle("⏳ Resolving...")
    .setDescription(`Resolving track: **${track}**`)
    .addFields(
      { name: "Status", value: "Fetching audio source...", inline: false },
    )
    .setTimestamp();
}

export function createLoadingPlaylistEmbed(playlistName: string): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0xFEE75C)
    .setTitle("📂 Loading Playlist...")
    .setDescription(`**${playlistName}**`)
    .addFields(
      { name: "Status", value: "Fetching playlist information...", inline: false },
    )
    .setTimestamp();
}

export function createConnectingEmbed(): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0xFEE75C)
    .setTitle("🔌 Connecting...")
    .setDescription("Connecting to voice channel...")
    .addFields(
      { name: "Status", value: "Establishing voice connection...", inline: false },
    )
    .setTimestamp();
}

export function createBufferingEmbed(): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(0xFEE75C)
    .setTitle("⏳ Buffering...")
    .setDescription("Buffering audio...")
    .addFields(
      { name: "Status", value: "Preloading audio for smooth playback...", inline: false },
    )
    .setTimestamp();
}
