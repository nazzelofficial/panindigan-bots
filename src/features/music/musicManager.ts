...
import {
  validateLavalinkConfig,
  getMusicStatus,
  MusicStatus,
  type MusicStatusInfo,
  getNodeHealthInfo,
  isNodeHealthy,
} from "../../utils/music.js";
 ...
 */
function parseNodeConfigs(): LavalinkNodeConfig[] {
...
function calculateBackoff(attempt: number): number {
  const delay = Math.min(
    RECONNECT_CONFIG.initialDelay * Math.pow(RECONNECT_CONFIG.backoffMultiplier, attempt - 1),
 ...
 */
function selectHealthiestNode(lavalink: LavalinkManager): any | null {
  ...
  for (const node of nodes.values()) {
    if (!isNodeHealthy(node)) continue;
 ...
 */
async function performFailover(lavalink: LavalinkManager, failedNodeId: string): Promise<void> {
 ...
 */
function logConnectionAttempt(attempt: number, maxAttempts: number): void {
 ...
 */
function logConnectionSuccess(nodeId: string, latency?: number): void {
 ...
 */
function logConnectionFailure(attempt: number, error: Error): void {
 ...
 */
export async function initLavalink(client: PanindiganClient): Promise<void> {
  ...
  try {
    ...
    lavalink.nodeManager.on("error", (node, error) => {
      logConnectionFailure(1, error as Error);
 ...
 */
export function getMusicStatusInfo(client: PanindiganClient): MusicStatusInfo {
 ...
 */
export function isMusicSystemReady(client: PanindiganClient): boolean {
