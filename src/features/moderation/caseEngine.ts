...
export interface CreateCaseInput {
  ...
  userId: string;
  moderatorId: string;
  type: "warn" | "mute" | "unmute" | "timeout" | "untimeout" | "kick" | "ban" | "tempban" | "softban" | "unban" | "note";
  reason?: string;
  duration?: number;
  expiresAt?: Date;
}
 ...
 */
export async function createModCase(input: CreateCaseInput) {
  const guild = await GuildModel.findOneAndUpdate(
    { guildId: input.guildId },
    { $setOnInsert: { guildId: input.guildId } },
...
export async function getActiveWarningCount(guildId: string, userId: string): Promise<number> {
