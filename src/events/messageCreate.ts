...
function sanitizeForMessage(payload: ReplyPayload): Omit<ReplyPayload, "ephemeral"> {
  const { ephemeral: _ephemeral, ...rest } = payload;
  return rest;
}

async function getGuildPrefix(client: PanindiganClient, guildId: string): Promise<string> {
  const cacheKey = `prefix:${guildId}`;
  const cached   = botCache.get<string>(cacheKey);
  if (cached !== undefined) return cached;

  const guildConfig = await GuildModel.findOne({ guildId }, { prefix: 1 }).lean();
  const prefix      = (guildConfig as { prefix?: string } | null)?.prefix ?? client.config.bot.defaultPrefix;
  botCache.set(cacheKey, prefix, CACHE_TTL.GUILD_CONFIG);
  return prefix;
}

async function handlePrefixCommand(client: PanindiganClient, message: Message, prefix: string): Promise<void> {
  ...
  const ctx: RunContext = {
    ...
    hasCooldown:  (commandName: string) => client.isOnCooldown(commandName, message.author.id, 0),
    reply:        async (payload) => message.reply(sanitizeForMessage(payload) as never),
  };
  ...
  async execute(client, message: Message) {
    ...
    try {
      if (message.guildId) {
        ...
        await grantMessageXp(client, message).catch((err: Error) =>
          log.error("XP grant failed", { error: err.message }),
        );

        const blocked = await runAutomodChecks(client, message).catch((err: Error) => {
      ...
      let prefix = client.config.bot.defaultPrefix as string;
      if (message.guildId) {
