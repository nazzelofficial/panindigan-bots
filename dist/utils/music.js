...
export function validateLavalinkConfig() {
    ...
    if (!host || !port || !password) {
        const missing = [];
        if (!host)
            missing.push("LAVALINK_HOST");
        if (!port)
            missing.push("LAVALINK_PORT");
        if (!password)
            missing.push("LAVALINK_PASSWORD");
        return {
            isValid: false,
 ...
 */
export function getMusicStatus(client) {
    ...
    nodes?.forEach((node) => {
        ...
        if (node.connected === true || node.state === "connected") {
            connectedCount++;
        }
    });
    if (connectedCount === 0) {
 ...
 */
export function isMusicReady(client) {
 ...
 */
export function getMusicUnavailableMessage(client) {
    const status = getMusicStatus(client);
 ...
 */
export function getNodeHealthInfo(node) {
    const stats = node?.stats;
    return {
        nodeId: node.id,
        connected: node.connected === true || node.state === "connected",
 ...
 */
export function getHealthyNode(client) {
    const lavalink = client.lavalink;
 ...
 */
export function getConnectedNode(client) {
 ...
 */
export function validateMusicOperation(client) {
    const status = getMusicStatus(client);
 ...
 */
export function getAllNodesHealthInfo(client) {
    const lavalink = client.lavalink;
    if (!lavalink?.nodeManager?.nodes) {
        return [];
    }
