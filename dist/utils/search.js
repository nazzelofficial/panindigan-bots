 ...
 */
export function fuzzySearch(query, commands, options = {}) {
    const { limit = 10, maxScore = 5, includePremium = true, userPermissions = [], } = options;
    ...
    const scored = commands.map((cmd) => {
        const fields = [
            ...
            ...(cmd.aliases?.map((a) => ({ value: a, type: "alias" })) ?? []),
    ...
    const filtered = scored.filter((s) => {
        if (s.score >= maxScore || s.score === Infinity)
            return false;
        if (!includePremium && s.command.premium)
            return false;
        return true;
    });
 ...
 */
export function getDidYouMean(query, commands) {
 ...
 */
export function autocomplete(prefix, commands, limit = 10) {
 ...
 */
export function searchInCategory(query, commands, category, options = {}) {
    const categoryCommands = commands.filter((c) => c.category.toLowerCase() === category.toLowerCase());
    return fuzzySearch(query, categoryCommands, options);
}
/**
 * Get related commands based on category and keywords.
 */
export function getRelatedCommands(command, allCommands, limit = 5) {
    const sameCategory = allCommands.filter((c) => c.category === command.category && c.name !== command.name);
