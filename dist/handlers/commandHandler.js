...
import { resolveAccessTier, tierSatisfies, TIER_LABELS } from "@/utils/permissions";
...
import { MaintenanceStateModel } from "@/database/models/System";
...
export const hitCounter = new Map();
function recordHit(guildId, commandName) {
    const key = guildId ?? "__dm__";
    const bucket = hitCounter.get(key) ?? new Map();
...
export function topCommands(guildId, n = 5) {
    ...
    for (const file of files) {
        try {
            ...
            if (!command.description)
                log.warn(`Command "${command.name}" is missing a description`, { file });
            if (!command.category)
                log.warn(`Command "${command.name}" is missing a category`, { file });
