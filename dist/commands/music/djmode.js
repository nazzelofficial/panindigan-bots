...
const command = {
    ...
    cooldown: 5,
    slashData: (b) => b
        .addSubcommand((s) => s.setName("enable").setDescription("Enable DJ mode — restricts music controls to DJ role"))
    ...
    async execute(ctx) {
        ...
        if (sub === "enable") {
            ...
            const enabled = cfg?.music?.djMode ?? false;
            const djRoleId = cfg?.djRoleIds?.[0] ?? null;
...
//# sourceMappingURL=djmode.js.map