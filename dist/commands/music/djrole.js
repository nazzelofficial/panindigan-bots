...
const command = {
    ...
    cooldown: 5,
    slashData: (b) => b
        .addSubcommand((s) => s.setName("add").setDescription("Add a DJ role")
        .addRoleOption((o) => o.setName("role").setDescription("Role to grant DJ access").setRequired(true)))
        .addSubcommand((s) => s.setName("remove").setDescription("Remove a DJ role")
    ...
    async execute(ctx) {
        ...
        if (sub === "add") {
            ...
            const ids = cfg?.djRoleIds ?? [];
            const embed = baseEmbed("primary").setTitle("🎧 DJ Roles").setDescription(ids.length ? ids.map((id) => `• <@&${id}>`).join("\n") : "No DJ roles configured.").setFooter({ text: `DJ Mode: ${cfg?.music?.djMode ? "Enabled" : "Disabled"}` });
