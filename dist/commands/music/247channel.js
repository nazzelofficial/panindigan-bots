...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!channelId) {
            await ctx.reply({ embeds: [errorEmbed("Please specify a voice channel.")] });
            return;
        }
        const ch = guild.channels.cache.get(channelId);
        if (!ch?.isVoiceBased()) {
            await ctx.reply({ embeds: [errorEmbed("That is not a voice channel.")] });
            return;
        }
...
//# sourceMappingURL=247channel.js.map