...
const command = {
    ...
    async execute(ctx) {
        ...
        if (current >= required) {
            ...
            await player.skip();
            const embed = successEmbed(`Vote passed (${current}/${listeners}) — skipped **${skipped?.info?.title ?? "the current track"}**.`);
            await ctx.reply({ embeds: [embed] });
        }
        else {
            const embed = baseEmbed("info")
                .setTitle("🗳️ Vote Skip")
                .setDescription(`<@${ctx.userId}> voted to skip **${player.queue.current?.info?.title ?? "the current track"}**.\n\n**Votes:** ${current}/${required} needed (${listeners} listener${listeners !== 1 ? "s" : ""})`)
...
//# sourceMappingURL=voteskip.js.map