...
const command = {
    ...
    guildOnly: true,
    cooldown: 5,
    slashData: (b) => b.addStringOption((o) => o.setName("note").setDescription("Note to add to this ticket").setRequired(true).setMaxLength(500)),
    async execute(ctx) {
        ...
        if (!note) {
            await ctx.reply({ embeds: [errorEmbed("Please provide a note.")] });
            return;
        }
