...
async function nextTicketNumber(guildId) {
    ...
    return doc?.tickets?.nextTicketNumber ?? 1;
}
...
const command = {
    ...
    slashData: (b) => b
        // ── setup ──
        .addSubcommand((s) => s.setName("setup").setDescription("Create a default ticket panel with a button in this channel"))
        // ── panel ──
        ...
        .setRequired(true)
        .addChoices({ name: "Low", value: "low" }, { name: "Medium", value: "medium" }, { name: "High", value: "high" }, { name: "Emergency", value: "emergency" })))
    ...
    async execute(ctx) {
        ...
        if (effectiveSub === "panel:create") {
            ...
            const channel = ctx.interaction?.channel ?? ctx.message?.channel;
            if (!channel?.isTextBased()) {
                await ctx.reply({ embeds: [errorEmbed("Use in a text channel.")] });
                return;
            }
            ...
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`ticket:open:${panel._id.toString()}`).setLabel("📩 Open a Ticket").setStyle(ButtonStyle.Primary));
            const sent = await channel.send({ embeds: [embed], components: [row] });
            panel.messageId = sent.id;
            await panel.save();
        ...
        if (effectiveSub === "panel:list") {
            ...
            if (!panels.length) {
                await ctx.reply({ embeds: [infoEmbed("No panels found. Run `ticket setup` first.")] });
                return;
            }
            const embed = baseEmbed("primary").setTitle("🎫 Ticket Panels").setDescription(panels.map((p, i) => `**${i + 1}.** ${p.name} — <#${p.channelId}>`).join("\n"));
        ...
        if (effectiveSub === "panel:delete") {
            ...
            if (!panel) {
                await ctx.reply({ embeds: [errorEmbed(`No panel named **${name}**.`)] });
                return;
            }
        ...
        if (effectiveSub === "close") {
            ...
            await tktDoc.save();
            const ch = guild.channels.cache.get(tktDoc.channelId);
            if (ch) {
            ...
            if (guildCfg?.tickets?.logChannelId) {
                await sendLogEvent(guild.id, "ticketClose", () => baseEmbed("warning").setTitle("🎫 Ticket Closed").setDescription(`**Ticket #${tktDoc.ticketNumber}** closed by <@${ctx.userId}>\nReason: ${reason}`));
            }
        ...
        if (effectiveSub === "reopen") {
            if (!await requireTicket())
                return;
            if (tktDoc.status !== "closed") {
                await ctx.reply({ embeds: [errorEmbed("This ticket is not closed.")] });
                return;
            }
            tktDoc.status = "open";
        ...
        if (effectiveSub === "rename") {
            ...
            if (!newName) {
                await ctx.reply({ embeds: [errorEmbed("Provide a new name.")] });
                return;
            }
            const ch = guild.channels.cache.get(tktDoc.channelId);
        ...
        if (effectiveSub === "claim") {
            if (!await requireTicket())
                return;
            if (tktDoc.claimedBy) {
                await ctx.reply({ embeds: [errorEmbed(`Already claimed by <@${tktDoc.claimedBy}>.`)] });
                return;
            }
        ...
        if (effectiveSub === "add") {
            ...
            if (ch)
                await ch.permissionOverwrites.edit(targetUser.id, { ViewChannel: true, SendMessages: true }).catch(() => { });
            if (!tktDoc.participants.includes(targetUser.id)) {
                tktDoc.participants.push(targetUser.id);
                await tktDoc.save();
            }
        ...
        if (effectiveSub === "priority") {
            if (!await requireTicket())
                return;
            const level = (ctx.isSlash ? ctx.interaction.options.getString("level", true) : ctx.args[1]?.toLowerCase());
            if (!["low", "medium", "high", "emergency"].includes(level)) {
                await ctx.reply({ embeds: [errorEmbed("Valid priorities: low, medium, high, emergency.")] });
                return;
            }
            tktDoc.priority = level;
            await tktDoc.save();
            const emoji = { low: "🟢", medium: "🟡", high: "🔴", emergency: "🚨" }[level] ?? "⚪";
        ...
        if (effectiveSub === "transcript") {
            ...
            if (logGuild?.tickets?.logChannelId) {
                const logCh = guild.channels.cache.get(logGuild.tickets.logChannelId);
        ...
        if (effectiveSub === "rate") {
            if (!await requireTicket())
                return;
            if (tktDoc.status !== "closed") {
                await ctx.reply({ embeds: [errorEmbed("You can only rate a closed ticket.")] });
                return;
            }
            if (tktDoc.openerId !== ctx.userId) {
                await ctx.reply({ embeds: [errorEmbed("Only the ticket opener can rate the support.")] });
                return;
            }
            ...
            if (stars < 1 || stars > 5) {
                await ctx.reply({ embeds: [errorEmbed("Rating must be 1-5.")] });
                return;
            }
            tktDoc.rating = stars;
        ...
        if (effectiveSub === "stats") {
            ...
            const embed = baseEmbed("primary")
                ...
                .addFields({ name: "Total Tickets", value: String(total), inline: true }, { name: "Open", value: String(open), inline: true }, { name: "Closed", value: String(closed), inline: true }, { name: "Archived", value: String(archived), inline: true }, { name: "Avg Rating", value: String(avgRating), inline: true });
            await ctx.reply({ embeds: [embed] });
            return;
        }
        // ── move ──────────────────────────────────────────────────────────────────
        if (effectiveSub === "move") {
            ...
            const category = guild.channels.cache.find((c) => c.type === ChannelType.GuildCategory && (c.id === catQuery || c.name.toLowerCase() === catQuery.toLowerCase()));
            if (!category) {
                await ctx.reply({ embeds: [errorEmbed("Category not found.")] });
                return;
            }
            const ch = guild.channels.cache.get(tktDoc.channelId);
        ...
        if (effectiveSub === "setlogs") {
            ...
            if (!logCh?.isTextBased?.()) {
                await ctx.reply({ embeds: [errorEmbed("Please specify a valid text channel.")] });
                return;
            }
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { "tickets.logChannelId": logCh.id } }, { upsert: true });
            await ctx.reply({ embeds: [successEmbed(`Ticket logs will be sent to <#${logCh.id}>.`)] });
            return;
        }
        // ── setcategory ───────────────────────────────────────────────────────────
        if (effectiveSub === "setcategory") {
            ...
            if (!catCh || catCh.type !== ChannelType.GuildCategory) {
                await ctx.reply({ embeds: [errorEmbed("Please specify a valid category channel.")] });
                return;
            }
    ...
    registerComponents(client) {
        client.componentHandlers.set("ticket", async (interaction) => {
            ...
            if (action === "open") {
                ...
                if (guildCfg?.tickets?.logChannelId) {
                    await sendLogEvent(guild.id, "ticketOpen", () => baseEmbed("success").setTitle("🎫 Ticket Opened").setDescription(`**#${ticketNumber}** opened by <@${interaction.user.id}>\n<#${ticketChannel.id}>`));
                }
                return;
            }
            if (action === "close_btn") {
