...
const command = {
    ...
    guildOnly: true,
    cooldown: 5,
    slashData: (b) => b.addUserOption((o) => o.setName("user").setDescription("User to remove from the ticket").setRequired(true)),
    async execute(ctx) {
        ...
        if (!userId) {
            await ctx.reply({ embeds: [errorEmbed("Please mention a user to remove.")] });
            return;
        }
        if (userId === ticket.openerId) {
            await ctx.reply({ embeds: [errorEmbed("Cannot remove the ticket opener.")] });
            return;
        }
...
//# sourceMappingURL=ticketRemove.js.map