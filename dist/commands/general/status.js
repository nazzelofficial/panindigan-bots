...
function getHealthIcon(ok, degraded = false) {
    if (ok && !degraded)
        return "✅ Operational";
    if (degraded)
        return "⚠️ Degraded";
...
const command = {
    ...
    async execute(ctx) {
        ...
        const lavalinkOk = lava != null && lava.nodeManager.nodes.size > 0
            && [...lava.nodeManager.nodes.values()].some((n) => n.connected ?? n.options?.connected);
        const playerCount = lava
            ? [...(lava.nodeManager?.nodes?.values() ?? [])].reduce((acc, n) => acc + (n.stats?.playingPlayers ?? 0), 0)
            : 0;
        ...
        // ── Build embed ───────────────────────────────────────────────────────────
        const embed = EmbedFactory[color](`**Health Score: ${scoreIcon} ${healthScore}/100**`, "🔴 Panindigan System Status");
        embed
            .setTitle("🔴 Panindigan System Status")
            .setDescription(`**${scoreIcon} Health Score: ${healthScore}/100**\n` +
            (healthScore >= 90
