...
export default {
    ...
    async execute(interaction) {
        ...
        if (member.voice.channelId === channel.id)
            return interaction.reply({ content: '❌ That user is already in that channel.', ephemeral: true });
        const fromChannel = member.voice.channel.name;
        await member.voice.setChannel(channel.id);
        await sendLogEvent(guild.id, 'voicemove', () => baseEmbed('info')
            .setTitle('🔀 Voice Move')
            .addFields({ name: 'User', value: `${user.tag} (<@${user.id}>)`, inline: true }, { name: 'Moderator', value: `<@${interaction.user.id}>`, inline: true }, { name: 'From', value: fromChannel, inline: true }, { name: 'To', value: String(channel?.name ?? 'unknown'), inline: true }));
