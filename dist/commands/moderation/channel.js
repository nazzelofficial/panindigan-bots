...
const command = {
    ...
    cooldown: 5,
    slashData: (b) => b
        .addSubcommand((s) => s.setName("create").setDescription("Create a new channel (text, voice, or announcement)")
        ...
        .addSubcommand((s) => s.setName("delete").setDescription("Delete a channel from the server")
        ...
        .addStringOption((o) => o.setName("reason").setDescription("Audit log reason").setRequired(false)))
        .addSubcommand((s) => s.setName("topic").setDescription("Set or clear the topic of a text channel")
        .addStringOption((o) => o.setName("topic").setDescription("New topic (leave blank to clear)").setRequired(false))
        .addChannelOption((o) => o.setName("channel").setDescription("Target channel (defaults to current)").setRequired(false)))
        .addSubcommand((s) => s.setName("clone").setDescription("Clone a channel with the same permissions and settings")
        .addChannelOption((o) => o.setName("channel").setDescription("Channel to clone (defaults to current)").setRequired(false))
        .addStringOption((o) => o.setName("reason").setDescription("Audit log reason").setRequired(false)))
        .addSubcommand((s) => s.setName("nsfw").setDescription("Toggle the NSFW flag on a text channel")
        .addChannelOption((o) => o.setName("channel").setDescription("Target channel (defaults to current)").setRequired(false)))
        .addSubcommand((s) => s.setName("info").setDescription("View detailed information about a channel")
        .addChannelOption((o) => o.setName("channel").setDescription("Target channel (defaults to current)").setRequired(false))),
    async execute(ctx) {
        ...
        const resolveChannel = (opt) => {
            if (!ctx.isSlash)
                return ctx.message?.channel;
            const ch = ctx.interaction.options.getChannel(opt);
            return ch ? (guild.channels.cache.get(ch.id) ?? ctx.interaction.channel) : ctx.interaction.channel;
        };
        if (sub === "create") {
            const name = ctx.isSlash ? ctx.interaction.options.getString("name", true) : ctx.args[1];
            if (!name) {
                await ctx.reply({ embeds: [errorEmbed("Provide a pangalan ng channel.")] });
                return;
            }
        ...
        if (sub === "delete") {
            ...
            const name = target.name ?? "channel";
        ...
        if (sub === "rename") {
            ...
            await target.setName(name, reason);
        ...
        if (sub === "topic") {
            ...
            const target = resolveChannel("channel");
            if (!target || !target.setTopic) {
                await ctx.reply({ embeds: [errorEmbed("Not a valid text channel.")] });
                return;
            }
            await target.setTopic(topic || null, reason);
        ...
        if (sub === "clone") {
            ...
            const cloned = await target.clone({ reason });
            await ctx.reply({ embeds: [successEmbed(`Channel cloned as ${cloned}.`)] });
            return;
        }
        if (sub === "nsfw") {
            const target = resolveChannel("channel");
            if (!target || !target.setNSFW) {
                await ctx.reply({ embeds: [errorEmbed("Text channel lang ang maaring i-toggle ng NSFW.")] });
                return;
            }
            const current = target.nsfw ?? false;
            await target.setNSFW(!current, reason);
        ...
        if (sub === "info") {
            ...
            const embed = baseEmbed("primary")
                ...
                .addFields({ name: "ID", value: ch.id, inline: true }, { name: "Type", value: ChannelType[ch.type] ?? String(ch.type), inline: true }, { name: "Created", value: `<t:${Math.floor((ch.createdTimestamp ?? Date.now()) / 1000)}:R>`, inline: true });
            if (ch.topic)
                embed.addFields({ name: "Topic", value: ch.topic, inline: false });
            if (ch.nsfw !== undefined)
                embed.addFields({ name: "NSFW", value: ch.nsfw ? "Yes" : "No", inline: true });
            if (ch.rateLimitPerUser !== undefined)
                embed.addFields({ name: "Slowmode", value: ch.rateLimitPerUser ? `${ch.rateLimitPerUser}s` : "Off", inline: true });
            if (ch.members)
                embed.addFields({ name: "Members", value: String(ch.members.size), inline: true });
