...
const command = {
    ...
    async execute(ctx) {
        ...
        if (warnCount >= 3)
            suspicionPoints.push(`⚠️ ${warnCount} warnings in the last 30 days`);
        if (muteCount >= 2)
            suspicionPoints.push(`⚠️ ${muteCount} mutes/timeouts in the last 30 days`);
        if (automodCases >= 5)
            suspicionPoints.push(`⚠️ Triggered automod ${automodCases} times recently`);
        if (ageDays < 7)
            suspicionPoints.push(`🚨 New account (${ageDays} days old)`);
        if (!user.avatar)
            suspicionPoints.push("⚠️ No avatar — common in spam bots");
        let verdict = "🟢 Unlikely Spammer";
        if (suspicionPoints.length >= 4)
            verdict = "🔴 Very Likely Spammer";
        else if (suspicionPoints.length >= 3)
            verdict = "🟠 Likely Spammer";
        else if (suspicionPoints.length >= 2)
            verdict = "🟡 Possible Spammer";
        const embed = baseEmbed(suspicionPoints.length >= 3 ? "danger" : "warning")
            .setTitle(`🔎 Spam Analysis — ${user.username}`)
            .setThumbnail(user.displayAvatarURL())
            .addFields({ name: "Verdict", value: verdict, inline: true }, { name: "Account Age", value: `${ageDays} days`, inline: true }, { name: "Server Member", value: member ? "✅ Yes" : "❌ No", inline: true }, { name: "Warnings (30d)", value: String(warnCount), inline: true }, { name: "Mutes (30d)", value: String(muteCount), inline: true }, { name: "Automod Hits (30d)", value: String(automodCases), inline: true });
...
//# sourceMappingURL=spamcheck.js.map