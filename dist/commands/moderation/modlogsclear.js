...
const command = {
    ...
    slashData: (b) => b
        .addUserOption((o) => o.setName("user").setDescription("User whose mod history to clear").setRequired(true))
        .addStringOption((o) => o
        .setName("type")
        .setDescription("Clear only a specific type (leave empty to clear all)")
        .setRequired(false)
        .addChoices({ name: "warn", value: "warn" }, { name: "mute/timeout", value: "mute" }, { name: "kick", value: "kick" }, { name: "ban", value: "ban" }, { name: "note", value: "note" })),
    async execute(ctx) {
        ...
        const filter = { guildId: guild.id, userId: targetId };
        if (type)
            filter.type = type;
        ...
        await sendLogEvent(guild.id, "modlogsClear", () => baseEmbed("warning")
        ...
        await ctx.reply({
            embeds: [
                successEmbed(`Cleared **${count}** ${type ? `\`${type}\`` : ""} case${count !== 1 ? "s" : ""} for <@${targetId}>.`),
            ],
        });
    },
};
export default command;
//# sourceMappingURL=modlogsclear.js.map