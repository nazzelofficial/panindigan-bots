...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!targetId) {
            await ctx.reply({ embeds: [EmbedFactory.error("Provide a member.")] });
            return;
        }
        const member = await guild.members.fetch(targetId).catch(() => null);
        if (!member) {
            await ctx.reply({ embeds: [EmbedFactory.error("Member not found.")] });
            return;
        }
        if (!member.kickable) {
            await ctx.reply({ embeds: [EmbedFactory.error("I cannot kick this member.")] });
            return;
        }
        ...
        await sendLogEvent(guild.id, "kick", () => EmbedFactory.moderation(`**User:** <@${targetId}>\n**Moderator:** <@${ctx.userId}>\n**Reason:** ${reason}`, "👢 Member Kicked"));
