...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!targetId) {
            await ctx.reply({ embeds: [errorEmbed("Provide a user to compare against.")] });
            return;
        }
        ...
        if (!top.length) {
            await ctx.reply({ embeds: [infoEmbed(`No members found with similar patterns to **${targetUser.username}** at threshold ${threshold}.`)] });
            return;
        }
        const embed = baseEmbed("warning")
            .setTitle(`🔍 Similar Accounts — ${targetUser.username}`)
            .setDescription(top.map((s, i) => `**${i + 1}.** <@${s.member.id}> (\`${s.member.user.username}\`)\n↳ ${s.reason}`).join("\n\n").slice(0, 4000))
            .setFooter({ text: `Found ${top.length} similar account${top.length !== 1 ? "s" : ""} · Threshold: ${threshold}` });
        await ctx.reply({ embeds: [embed] });
    },
};
export default command;
//# sourceMappingURL=similaraccounts.js.map