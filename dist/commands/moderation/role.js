...
const command = {
    ...
    slashData: (b) => b
        ...
        .addSubcommand((s) => s.setName("info").setDescription("View information about a role")
        .addRoleOption((o) => o.setName("role").setDescription("Role").setRequired(true))),
    async execute(ctx) {
        ...
        if (sub === "add") {
            ...
            if (!userId || !roleId) {
                await ctx.reply({ embeds: [errorEmbed("Provide a user and role.")] });
                return;
            }
        ...
        if (sub === "create") {
            ...
            if (!name) {
                await ctx.reply({ embeds: [errorEmbed("Provide a role name.")] });
                return;
            }
            let color;
            if (colorInput) {
                ...
                if (isNaN(color)) {
                    await ctx.reply({ embeds: [errorEmbed("Invalid hex color. Use the format: `#FF0000`.")] });
                    return;
                }
            }
        ...
        if (sub === "delete") {
            ...
            if (!roleId) {
                await ctx.reply({ embeds: [errorEmbed("Provide a role.")] });
                return;
            }
            const role = guild.roles.cache.get(roleId);
            if (!role) {
        ...
        if (sub === "color") {
            ...
            if (!roleId || !colorInput) {
                await ctx.reply({ embeds: [errorEmbed("Provide a role and hex color.")] });
                return;
            }
            const role = guild.roles.cache.get(roleId);
            if (!role) {
            ...
            if (isNaN(color)) {
                await ctx.reply({ embeds: [errorEmbed("Invalid hex color.")] });
                return;
            }
        ...
        if (sub === "info") {
            ...
            const embed = baseEmbed("primary")
                ...
                .addFields({ name: "ID", value: role.id, inline: true }, { name: "Color", value: role.hexColor, inline: true }, { name: "Members", value: String(memberCount), inline: true }, { name: "Hoisted", value: role.hoist ? "Yes" : "No", inline: true }, { name: "Mentionable", value: role.mentionable ? "Yes" : "No", inline: true }, { name: "Managed", value: role.managed ? "Yes (bot/integration)" : "No", inline: true }, { name: "Position", value: String(role.position), inline: true }, { name: "Created", value: `<t:${Math.floor(role.createdTimestamp / 1000)}:R>`, inline: true });
