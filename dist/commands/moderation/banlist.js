...
const command = {
    ...
    slashData: (b) => b
        ...
        .addStringOption((o) => o.setName("search").setDescription("Find isang partikular na user (username/ID)").setRequired(false)),
    async execute(ctx) {
        ...
        if (!bans) {
            await ctx.reply({ embeds: [errorEmbed("Hindi ma-fetch ang ban list. Siguraduhing may sapat akong permission.")] });
            return;
        }
        if (!bans.size) {
            await ctx.reply({ embeds: [baseEmbed("primary").setTitle("🔨 Ban List").setDescription("No banned users in this server.")] });
            return;
        }
