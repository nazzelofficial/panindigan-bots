...
const command = {
    ...
    async execute(ctx) {
        ...
        if (sub === "add") {
            ...
            if (!note) {
                await ctx.reply({ embeds: [errorEmbed("Provide a note.")] });
                return;
            }
            ...
            if (!notes.length) {
                await ctx.reply({ embeds: [infoEmbed(`No notes for ${target.username}.`)] });
                return;
            }
            const embed = baseEmbed("primary")
                .setTitle(`📝 Notes — ${target.username}`)
                .setDescription(notes.map((n) => `**Case #${n.caseId}** — ${n.reason}\n> By <@${n.moderatorId}> · <t:${Math.floor(new Date(n.createdAt).getTime() / 1000)}:R>`).join("\n\n").slice(0, 4000));
