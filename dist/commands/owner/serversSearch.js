...
export default {
    ...
    async execute(interaction) {
        ...
        const matches = Array.from(client.guilds.cache.values())
            .filter((guild) => guild.name.toLowerCase().includes(searchName))
            .slice(0, 10);
        if (matches.length === 0) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#00ff00')
            .setDescription(matches.map((guild) => `**${guild.name}** - ${guild.memberCount} members (ID: ${guild.id})`).join('\n'))
            .setTimestamp();
        await interaction.reply({ embeds: [embed], ephemeral: true });
    },
};
//# sourceMappingURL=serversSearch.js.map