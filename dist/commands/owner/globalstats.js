    ...
    async execute(interaction) {
        ...
        if (sub === 'overview') {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#5865F2')
                .addFields({ name: '🏠 Servers', value: totalServers.toLocaleString(), inline: true }, { name: '👥 Total Members', value: totalMembers.toLocaleString(), inline: true }, { name: '📢 Channels', value: totalChannels.toLocaleString(), inline: true }, { name: '🔧 Shards', value: shardCount.toString(), inline: true }, { name: '⏱️ Uptime', value: uptimeStr, inline: true }, { name: '💾 RAM', value: `${ramMB} MB`, inline: true }, { name: '⚡ Ping', value: `${client.ws.ping}ms`, inline: true }, { name: '💎 Premium Servers', value: premiumCount.toLocaleString(), inline: true }, { name: '📋 Commands Loaded', value: totalCommandsLoaded.toLocaleString(), inline: true }, { name: '📦 discord.js', value: `v${djsVersion ?? '?'}`, inline: true }, { name: '🟩 Node.js', value: process.version, inline: true })
                .setTimestamp()
        ...
        if (sub === 'growth') {
            ...
            const newest10 = sorted.slice(0, 10).map(g => `• **${g.name}** (${g.memberCount.toLocaleString()} members) — joined <t:${Math.floor((g.joinedTimestamp ?? 0) / 1000)}:R>`).join('\n');
            const embed = new EmbedBuilder()
                .setTitle('📈 Server Growth — Bot Network')
                .setColor('#5865F2')
                .addFields({ name: '🏠 Total Servers', value: totalServers.toLocaleString(), inline: true }, { name: '🆕 10 Most Recently Added', value: newest10 || 'N/A', inline: false })
        ...
        if (sub === 'revenue') {
            ...
            const premiums = await PremiumModel.find({}).lean();
            const packs = await ServerPackModel.find({ active: true }).lean();
            const grantCounts = { basic: 0, standard: 0, gold: 0, enterprise: 0 };
            const refundCounts = { basic: 0, standard: 0, gold: 0, enterprise: 0 };
            for (const p of premiums) {
                for (const h of (p.history ?? [])) {
                    const tier = h.tier ?? p.tier;
            ...
            const packRevenue = packs.reduce((sum, p) => sum + (PACK_PRICES[p.packType] ?? 0), 0);
            const netRevenue = grossRevenue + packRevenue - refundTotal;
            const embed = new EmbedBuilder()
                .setTitle('💰 Revenue Report — Premium Grant History')
                .setColor('#FFD700')
                .addFields({ name: '📊 Gross Revenue (PHP)', value: `₱${(grossRevenue + packRevenue).toLocaleString()}`, inline: true }, { name: '↩️ Refunds (PHP)', value: `₱${refundTotal.toLocaleString()}`, inline: true }, { name: '💵 Net Revenue (PHP)', value: `₱${netRevenue.toLocaleString()}`, inline: true }, { name: '🥈 Basic Grants', value: grantCounts.basic.toLocaleString(), inline: true }, { name: '🥇 Standard Grants', value: grantCounts.standard.toLocaleString(), inline: true }, { name: '👑 Gold Grants', value: grantCounts.gold.toLocaleString(), inline: true }, { name: '💠 Enterprise Grants', value: grantCounts.enterprise.toLocaleString(), inline: true }, { name: '📦 Active Packs', value: packs.length.toLocaleString(), inline: true }, { name: '📦 Pack Revenue (PHP)', value: `₱${packRevenue.toLocaleString()}`, inline: true }, { name: '↩️ Basic Refunds', value: refundCounts.basic.toLocaleString(), inline: true }, { name: '↩️ Standard Refunds', value: refundCounts.standard.toLocaleString(), inline: true }, { name: '↩️ Gold/Ent Refunds', value: (refundCounts.gold + refundCounts.enterprise).toLocaleString(), inline: true })
...
//# sourceMappingURL=globalstats.js.map