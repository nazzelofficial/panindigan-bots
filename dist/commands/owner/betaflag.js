...
export default {
    ...
    async execute(interaction) {
        ...
        if (sub === 'remove') {
            ...
            if (!result)
                return interaction.editReply({ content: `❌ Server \`${guildId}\` does not have beta access.` });
            const client = clientRegistry.get();
            const serverName = client.guilds.cache.get(guildId)?.name ?? guildId;
            return interaction.editReply({ content: `✅ Beta access removed from **${serverName}**.` });
        }
        if (sub === 'list') {
            const betaServers = await BetaFlagModel.find({}).lean();
            if (betaServers.length === 0)
                return interaction.editReply({ content: 'ℹ️ No servers have beta access currently.' });
            const client = clientRegistry.get();
            const lines = betaServers.map((s) => {
...
//# sourceMappingURL=betaflag.js.map