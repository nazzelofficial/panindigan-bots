...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!cmd) {
            await ctx.reply({ embeds: [errorEmbed("Provide a command.")] });
            return;
        }
        ...
        const embed = baseEmbed(isError ? "danger" : "success")
            ...
            .addFields({ name: "Command", value: `\`\`\`sh\n${cmd.slice(0, 500)}\`\`\``, inline: false }, { name: "stdout", value: `\`\`\`\n${stdout.slice(0, 900)}\`\`\``, inline: false });
        if (stderr)
            embed.addFields({ name: "stderr", value: `\`\`\`\n${stderr.slice(0, 900)}\`\`\``, inline: false });
