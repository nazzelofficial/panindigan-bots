...
const command = {
    ...
    async execute(ctx) {
        ...
        if (guildId) {
            ...
            if (!guild) {
                await ctx.reply({ embeds: [errorEmbed(`Bot is not in guild \`${guildId}\`.`)] });
                return;
            }
            ...
            const embed = baseEmbed("primary")
                ...
                .addFields({ name: "ID", value: guild.id, inline: true }, { name: "Members", value: guild.memberCount.toString(), inline: true }, { name: "Owner", value: owner ? `${owner.user.username} (${owner.id})` : guild.ownerId, inline: true }, { name: "Channels", value: guild.channels.cache.size.toString(), inline: true }, { name: "Roles", value: guild.roles.cache.size.toString(), inline: true }, { name: "Boost Level", value: String(guild.premiumTier), inline: true });
        ...
        if (!pageGuilds.length) {
            await ctx.reply({ embeds: [infoEmbed("No guilds on this page.")] });
            return;
        }
        const embed = baseEmbed("primary")
            .setTitle(`📋 Servers (${guilds.length} total)`)
            .setDescription(pageGuilds.map((g, i) => `**${(page - 1) * perPage + i + 1}.** ${g.name} \`${g.id}\` — **${g.memberCount}** members`).join("\n"))
