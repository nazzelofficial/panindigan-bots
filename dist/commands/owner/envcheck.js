...
export default {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#00ff00')
            .addFields({ name: 'Required Variables', value: requiredStatus.map(s => `${s.status} ${s.name}`).join('\n'), inline: true }, { name: 'Optional Variables', value: optionalStatus.map(s => `${s.status} ${s.name}`).join('\n'), inline: true })
            .setTimestamp();
