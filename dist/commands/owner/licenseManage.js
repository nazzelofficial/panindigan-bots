...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('revoke')
    ...
    async execute(interaction) {
        ...
        if (sub === 'generate') {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#FFD700')
                .addFields({ name: 'License Key', value: `\`${licenseKey}\``, inline: false }, { name: 'Server ID', value: guildId, inline: true }, { name: 'Issued', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true });
            if (whiteLabelName)
                embed.addFields({ name: 'White-Label Name', value: whiteLabelName, inline: true });
        ...
        if (sub === 'list') {
            const active = await LicenseModel.find({ active: true }).lean();
            if (active.length === 0)
                return interaction.editReply({ content: 'ℹ️ No active white-label licenses.' });
            const lines = active.map((l) => `• \`${l.licenseKey}\` — Server: \`${l.guildId}\`${l.whiteLabelName ? ` (**${l.whiteLabelName}**)` : ''}\n  Issued <t:${Math.floor(new Date(l.createdAt).getTime() / 1000)}:R>`);
            const embed = new EmbedBuilder()
...
//# sourceMappingURL=licenseManage.js.map