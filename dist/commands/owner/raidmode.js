...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('reason').setDescription('Reason for raid mode').setRequired(false)))
    ...
    async execute(interaction) {
        ...
        try {
            await GuildModel.findOneAndUpdate({ guildId: serverId }, { 'settings.raidMode': enabled, 'settings.raidModeReason': reason }, { upsert: true });
            if (enabled) {
                ...
                for (const [, channel] of guild.channels.cache) {
                    if (!channel.isTextBased())
                        continue;
                    try {
                        await channel.permissionOverwrites.edit(guild.roles.everyone, { SendMessages: false }, { reason: `[RAIDMODE ON] ${reason}` });
                        locked++;
                    }
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setColor('#ff0000')
                    .addFields({ name: 'Server', value: `${guild.name} (\`${guild.id}\`)`, inline: true }, { name: 'Channels Locked', value: locked.toString(), inline: true }, { name: 'Reason', value: reason, inline: false })
                    .setTimestamp()
                ...
                for (const [, channel] of guild.channels.cache) {
                    if (!channel.isTextBased())
                        continue;
                    try {
                        await channel.permissionOverwrites.edit(guild.roles.everyone, { SendMessages: null }, { reason: `[RAIDMODE OFF] Raid mode lifted by ${interaction.user.tag}` });
                        unlocked++;
                    }
                    catch { /* skip */ }
                }
                const embed = new EmbedBuilder()
                    ...
                    .setColor('#00ff00')
                    .addFields({ name: 'Server', value: `${guild.name} (\`${guild.id}\`)`, inline: true }, { name: 'Channels Restored', value: unlocked.toString(), inline: true })
                    .setTimestamp()
