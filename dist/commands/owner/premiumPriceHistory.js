...
export default {
    ...
    async execute(interaction) {
        ...
        const priceHistory = system?.premiumPriceHistory || [];
        const tierHistory = priceHistory.filter((h) => h.tier === tier);
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#ffd700')
            .setDescription(tierHistory.map((h) => `${h.date.toLocaleString()}: ₱${h.oldPrice} → ₱${h.newPrice} (by ${h.changedBy})`).join('\n'))
            .setTimestamp();
        await interaction.reply({ embeds: [embed], ephemeral: true });
    },
};
//# sourceMappingURL=premiumPriceHistory.js.map