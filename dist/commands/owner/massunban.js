...
export default {
    data: new SlashCommandBuilder()
        .setName('massunban')
        .setDescription('Unban multiple users simultaneously in a specific server (owner only)')
        ...
        .setDescription('Space or comma-separated user IDs to unban')
        .setRequired(true)),
    category: 'Owner',
    accessTier: 'owner',
    async execute(interaction) {
        ...
        if (userIds.length > 200) {
            return interaction.reply({ content: '❌ Cannot mass unban more than 200 users at once.', ephemeral: true });
        }
        await interaction.reply({ content: `🔓 Mass unbanning **${userIds.length}** users from **${guild.name}**...`, ephemeral: true });
        let success = 0;
        let failed = 0;
        const failedIds = [];
        for (const uid of userIds) {
            try {
                await guild.bans.remove(uid, `Mass unban by ${interaction.user.tag}`);
                success++;
            }
            catch {
                failed++;
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Server', value: `${guild.name} (\`${guild.id}\`)`, inline: true }, { name: 'Requested', value: userIds.length.toString(), inline: true }, { name: 'Unbanned', value: success.toString(), inline: true }, { name: 'Failed/Not Banned', value: failed.toString(), inline: true });
...
//# sourceMappingURL=massunban.js.map