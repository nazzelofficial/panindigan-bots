...
const command = {
    ...
    async execute(ctx) {
        ...
        if (sub === "grant") {
            ...
            if (!guildId || !TIERS.includes(tier)) {
                await ctx.reply({ embeds: [errorEmbed("Provide valid guild ID and tier.")] });
                return;
            }
            await PremiumModel.findOneAndUpdate({ guildId }, { $set: { tier, active: true, grantedBy: ctx.userId, grantedAt: new Date() } }, { upsert: true });
            await ctx.reply({ embeds: [successEmbed(`✅ **${TIER_LABELS[tier]}** granted to guild \`${guildId}\`.`)] });
        }
        else if (sub === "revoke") {
            const guildId = ctx.isSlash ? ctx.interaction.options.getString("guildid", true) : ctx.args[1];
            if (!guildId) {
                await ctx.reply({ embeds: [errorEmbed("Provide guild ID.")] });
                return;
            }
            ...
            if (!prem || !prem.active) {
                ...
                const embed = baseEmbed("warning")
                    ...
                    .addFields({ name: "Tier", value: TIER_LABELS[prem.tier] ?? prem.tier, inline: true }, { name: "Active", value: "✅", inline: true }, { name: "Granted by", value: prem.grantedBy ? `<@${prem.grantedBy}>` : "Code activation", inline: true });
            ...
            if (!tier || !TIERS.includes(tier)) {
                await ctx.reply({ embeds: [errorEmbed("Provide valid tier.")] });
                return;
            }
            const codes = [];
            for (let i = 0; i < count; i++) {
                const code = `PAN-${tier.toUpperCase().slice(0, 3)}-${nanoid(8).toUpperCase()}`;
                await PremiumCodeModel.create({ code, tier: tier, createdBy: ctx.userId });
                codes.push(code);
            }
            ...
            if (!codes.length) {
                await ctx.reply({ embeds: [infoEmbed("No unused premium codes.")] });
                return;
            }
...
//# sourceMappingURL=premiummgr.js.map