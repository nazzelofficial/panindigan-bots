...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!prize) {
            await ctx.reply({ embeds: [errorEmbed("Please provide a prize.")] });
            return;
        }
        ...
        if (!durationMs || durationMs < 60_000) {
            await ctx.reply({ embeds: [errorEmbed("Invalid duration. Examples: `10m`, `1h`, `2d`. Minimum: 1 minute.")] });
            return;
        }
        if (durationMs > 7 * 24 * 60 * 60 * 1000) {
            await ctx.reply({ embeds: [errorEmbed("Maximum giveaway duration is 7 days.")] });
            return;
        }
