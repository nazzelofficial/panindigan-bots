...
function getTone(text) {
    ...
    if (/[!]{2,}|\bwow\b|\bomg\b|\bamazing\b/i.test(text))
        return 'Excited / Enthusiastic';
    if (/[?]{1,}/.test(text) && !/[.!]/.test(text))
        return 'Inquisitive / Questioning';
    if (/\b(sorry|apologize|forgive|excuse)\b/i.test(text))
        return 'Apologetic';
    if (/\b(please|kindly|would you|could you)\b/i.test(text))
        return 'Polite / Formal';
    if (/\b(must|need|should|have to|required)\b/i.test(text))
        return 'Assertive / Directive';
    if (/\b(think|believe|consider|maybe|perhaps|possibly)\b/i.test(text))
        return 'Contemplative / Uncertain';
    if (/\b(haha|lol|😂|🤣|funny|hilarious)\b/i.test(text))
        return 'Humorous / Casual';
...
export default {
    ...
    cooldown: 3,
    slashData: (b) => b
        .addStringOption((o) => o.setName("text").setDescription("Text to analyze (max 1000 chars)").setRequired(true).setMaxLength(1000)),
    async execute(ctx) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(color)
            .addFields({ name: '📊 Sentiment', value: `**${sentiment}**`, inline: true }, { name: '🎭 Tone', value: tone, inline: true }, { name: '📈 Polarity', value: `${result.score > 0 ? '+' : ''}${result.score.toFixed(2)} (${pct}% positive)`, inline: true }, { name: '🔵 Confidence', value: `${confidence}%`, inline: true }, { name: '✅ Positive cues', value: result.positive.toString(), inline: true }, { name: '❌ Negative cues', value: result.negative.toString(), inline: true }, { name: '📏 Words', value: wordCount.toString(), inline: true }, { name: '📖 Sentences', value: result.sentences.toString(), inline: true }, { name: '📉 Polarity Bar', value: bar, inline: false })
