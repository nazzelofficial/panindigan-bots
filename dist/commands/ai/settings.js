...
export default {
    ...
    cooldown: 5,
    slashData: (b) => b
        .addSubcommand((s) => s.setName("view").setDescription("View current AI settings for this server"))
        .addSubcommand((s) => s
        .setName("toggle")
        ...
        .addIntegerOption((o) => o.setName("limit").setDescription("Messages per day (0 = unlimited)").setRequired(true).setMinValue(0).setMaxValue(500)))
        .addSubcommand((s) => s
        .setName("channel")
        .setDescription("Restrict AI commands to a specific channel (or remove restriction)")
        .addChannelOption((o) => o.setName("channel").setDescription("Channel to restrict AI to (leave empty to allow everywhere)").setRequired(false))),
    async execute(ctx) {
        ...
        if (sub === "toggle") {
            ...
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { "ai.enabled": enabled } }, { upsert: true });
        ...
        if (sub === "limit") {
            ...
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { "ai.dailyLimit": limit } }, { upsert: true });
        ...
        if (sub === "channel") {
            ...
            if (channel) {
                await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { "ai.channelId": channel.id } }, { upsert: true });
                await ctx.reply({ content: `✅ AI commands are now restricted to <#${channel.id}>.` });
            }
            else {
                await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $unset: { "ai.channelId": 1 } });
