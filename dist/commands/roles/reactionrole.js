    ...
    async execute(ctx) {
        ...
        if (sub === "add") {
            ...
            if (!messageId || !emoji || !role || !channel?.isTextBased()) {
                await ctx.reply({ embeds: [errorEmbed("Provide message ID, emoji, role, and ensure it's a text channel.")] });
                return;
            }
            const msg = await channel.messages.fetch(messageId).catch(() => null);
            if (!msg) {
                await ctx.reply({ embeds: [errorEmbed("Message not found in that channel.")] });
                return;
            }
            ...
            await ReactionRoleModel.findOneAndUpdate({ guildId: guild.id, messageId, emoji }, { guildId: guild.id, channelId: channel.id, messageId, emoji, roleId: role.id }, { upsert: true });
            ...
            if (!deleted) {
                await ctx.reply({ embeds: [errorEmbed("Reaction role not found.")] });
                return;
            }
            ...
            if (!rrs.length) {
                await ctx.reply({ embeds: [infoEmbed("No reaction roles configured.")] });
                return;
            }
            const embed = baseEmbed("primary").setTitle("🎭 Reaction Roles").setDescription(rrs.map((r) => `${r.emoji} → <@&${r.roleId}> (msg: \`${r.messageId}\` in <#${r.channelId}>)`).join("\n").slice(0, 4000));
