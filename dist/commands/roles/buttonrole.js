...
const command = {
    ...
    slashData: (b) => b
        ...
        .addSubcommand((s) => s.setName("addbutton")
    ...
    async execute(ctx) {
        ...
        if (sub === "create") {
            ...
            await ButtonRoleModel.create({ guildId: guild.id, messageId: msg.id, channelId: channel.id, title, buttons: [] });
            ...
            if (panel.buttons.length >= 25) {
                await ctx.reply({ embeds: [errorEmbed("Max 25 buttons per panel.")] });
                return;
            }
            panel.buttons.push({ roleId: role.id, label, emoji: emoji ?? null });
            await panel.save();
            // Rebuild buttons on the message
            const ch = guild.channels.cache.get(panel.channelId);
            const msg = await ch?.messages.fetch(messageId).catch(() => null);
            if (!msg) {
                await ctx.reply({ embeds: [errorEmbed("Message not found.")] });
                return;
            }
            const rows = [];
            ...
            if (!panels.length) {
                await ctx.reply({ embeds: [infoEmbed("No button role panels.")] });
                return;
            }
            const embed = baseEmbed("primary").setTitle("🔘 Button Role Panels").setDescription(panels.map((p) => `**${p.title}** — ${p.buttons.length} button(s) · <#${p.channelId}> · \`${p.messageId}\``).join("\n"));
