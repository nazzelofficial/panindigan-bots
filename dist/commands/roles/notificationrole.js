...
const command = {
    ...
    slashData: (b) => b
        ...
        .addStringOption((o) => o.setName("event").setDescription("Event type that triggers the ping").setRequired(true)
        .addChoices({ name: "Giveaway", value: "giveaway" }, { name: "Announcements", value: "announcement" }, { name: "Updates/Changelog", value: "update" }, { name: "Events", value: "event" }, { name: "Streams", value: "stream" }, { name: "Polls", value: "poll" }, { name: "Maintenance", value: "maintenance" }))
    ...
    async execute(ctx) {
        ...
        if (sub === "add") {
            ...
            const existing = doc?.notificationRoles ?? [];
            if (existing.some((n) => n.roleId === role.id)) {
                await ctx.reply({ embeds: [errorEmbed(`${role} is already a notification role. Remove it first if you want to change it.`)] });
                return;
            }
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $push: { notificationRoles: { roleId: role.id, description } } }, { upsert: true });
            ...
            const result = await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $pull: { notificationRoles: { roleId: role.id } } });
            if (!result) {
                await ctx.reply({ embeds: [errorEmbed("That role is not configured as a notification role.")] });
                return;
            }
            ...
            const doc = await GuildModel.findOne({ guildId: guild.id }).lean();
            const notifRoles = doc?.notificationRoles ?? [];
            ...
            const embed = baseEmbed("primary")
                ...
                .setDescription(notifRoles.map((n) => `<@&${n.roleId}> — ${n.description}`).join("\n"))
...
//# sourceMappingURL=notificationrole.js.map