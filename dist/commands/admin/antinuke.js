...
const command = {
    ...
    slashData: (b) => b
        ...
        .setRequired(false)
        .addChoices({ name: "Ban", value: "ban" }, { name: "Kick", value: "kick" }, { name: "Strip Roles", value: "strip-roles" }))
        .addIntegerOption((o) => o.setName("threshold").setDescription("Global action threshold (default: 3)").setRequired(false).setMinValue(1).setMaxValue(20))
        .addIntegerOption((o) => o.setName("window").setDescription("Detection window in seconds (default: 10)").setRequired(false).setMinValue(3).setMaxValue(60)))
        ...
        .setRequired(true)
        .addChoices({ name: "channel-delete", value: "channelDelete" }, { name: "channel-create", value: "channelCreate" }, { name: "ban", value: "ban" }, { name: "kick", value: "kick" }, { name: "role-delete", value: "roleDelete" }, { name: "webhook-create", value: "webhookCreate" }))
        .addIntegerOption((o) => o.setName("count").setDescription("Maximum actions before triggering (1–20)").setRequired(true).setMinValue(1).setMaxValue(20))
        .addIntegerOption((o) => o.setName("window").setDescription("Detection window in seconds (3–60)").setRequired(false).setMinValue(3).setMaxValue(60)))
        .addSubcommand((s) => s
        .setName("punishment")
    ...
    async execute(ctx) {
        ...
        if (sub === "enable") {
            ...
            const punLabels = { ban: "🔨 Ban", kick: "👢 Kick", "strip-roles": "🎭 Strip Roles" };
            await ctx.reply({
                embeds: [
                    successEmbed(`🛡️ Anti-nuke **enabled**.\n` +
        ...
        if (sub === "whitelist") {
            ...
            if (action === "list") {
                ...
                const users = an.whitelistUsers ?? [];
                const roles = an.whitelistRoles ?? [];
                const embed = baseEmbed("primary")
                    .setTitle("🛡️ Anti-Nuke Whitelist")
                    .addFields({ name: "Whitelisted Users", value: users.length ? users.map((id) => `<@${id}>`).join(", ") : "None", inline: false }, { name: "Whitelisted Roles", value: roles.length ? roles.map((id) => `<@&${id}>`).join(", ") : "None", inline: false });
            ...
            if (!user) {
                await ctx.reply({ embeds: [errorEmbed("Please provide a user to add or remove.")] });
                return;
            }
        ...
        if (sub === "threshold") {
            ...
            if (!THRESHOLD_ACTIONS.includes(action)) {
            ...
            if (!count || count < 1) {
                await ctx.reply({ embeds: [errorEmbed("Provide a valid count (1–20).")] });
                return;
            }
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { [`antinuke.thresholds.${action}`]: count, "antinuke.windowSeconds": window } }, { upsert: true });
        ...
        if (sub === "punishment") {
            ...
            const labels = { ban: "🔨 Ban", kick: "👢 Kick", "strip-roles": "🎭 Strip Roles" };
        ...
        if (sub === "status") {
            ...
            const an = cfg?.antinuke ?? {};
            if (!an.enabled) {
                await ctx.reply({ embeds: [infoEmbed("Anti-nuke is currently **disabled**.")] });
                return;
            }
            ...
            const embed = baseEmbed("primary")
                ...
                .addFields({ name: "Enabled", value: "✅ Yes", inline: true }, { name: "Punishment", value: labels[an.punishment] ?? an.punishment ?? "strip-roles", inline: true }, { name: "Window", value: `${an.windowSeconds ?? 10}s`, inline: true }, { name: "Thresholds", value: `Channel Delete: **${t.channelDelete ?? 3}**\n` +
                ...
                inline: false,
            }, { name: "Whitelisted Users", value: (an.whitelistUsers ?? []).length ? an.whitelistUsers.map((id) => `<@${id}>`).join(", ") : "None", inline: false }, { name: "Whitelisted Roles", value: (an.whitelistRoles ?? []).length ? an.whitelistRoles.map((id) => `<@&${id}>`).join(", ") : "None", inline: false });
            await ctx.reply({ embeds: [embed] });
            return;
        }
