...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!message) {
            await ctx.reply({ embeds: [errorEmbed("Please provide an announcement message.")] });
            return;
        }
        ...
        if (!channel?.isTextBased()) {
            await ctx.reply({ embeds: [errorEmbed("That is not a text channel.")] });
            return;
        }
        ...
        if (!botMember?.permissionsIn(channel).has(PermissionFlagsBits.SendMessages)) {
            await ctx.reply({ embeds: [errorEmbed("I don't have permission to send messages in that channel.")] });
            return;
        }
        let parsedColor;
        try {
...
//# sourceMappingURL=growthAnnounce.js.map