...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!name || !response) {
            await ctx.reply({ embeds: [errorEmbed("Provide both a command name and a response.")] });
            return;
        }
        if (!/^[a-z0-9_-]+$/.test(name)) {
            await ctx.reply({ embeds: [errorEmbed("Command name can only contain letters, numbers, underscores, and hyphens.")] });
            return;
        }
        const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
        const existing = cfg?.customCommands ?? [];
        if (existing.some((c) => c.name === name)) {
            await ctx.reply({ embeds: [errorEmbed(`A custom command named \`${name}\` already exists. Remove it first.`)] });
            return;
        }
        if (existing.length >= 100) {
            await ctx.reply({ embeds: [errorEmbed("Maximum of 100 custom commands reached.")] });
            return;
        }
        await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $push: { customCommands: { id: randomUUID(), name, response, createdBy: ctx.userId, createdAt: new Date() } } }, { upsert: true });
        await ctx.reply({ embeds: [successEmbed(`Custom command \`${name}\` created. Members can trigger it with your server prefix + \`${name}\`.`)] });
    },
};
export default command;
//# sourceMappingURL=customcommandAdd.js.map