...
const command = {
    ...
    slashData: (b) => b
        ...
        .addSubcommand((s) => s.setName("setmute").setDescription("Set the server mute role")
    ...
    async execute(ctx) {
        ...
        if (sub === "add" || sub === "remove") {
            ...
            if (!type || !roleId) {
                await ctx.reply({ embeds: [errorEmbed("Provide a role type and role.")] });
                return;
            }
            const fieldMap = { mod: "modRoleIds", admin: "adminRoleIds", dj: "djRoleIds" };
            const field = fieldMap[type];
            if (!field) {
                await ctx.reply({ embeds: [errorEmbed("Invalid type. Valid options: mod | admin | dj")] });
                return;
            }
        ...
        if (sub === "setmute") {
            ...
            if (!roleId) {
                await ctx.reply({ embeds: [errorEmbed("Provide a mute role.")] });
                return;
            }
        ...
        if (sub === "list") {
            ...
            const doc = cfg;
            const fmtRoles = (ids) => ids?.length ? ids.map((id) => `<@&${id}>`).join(", ") : "None";
            await ctx.reply({
                embeds: [
                    baseEmbed("primary")
                        .setTitle("👥 Server Staff Roles")
                        .addFields({ name: "🛡️ Admin Roles", value: fmtRoles(doc?.adminRoleIds ?? []), inline: false }, { name: "🔨 Mod Roles", value: fmtRoles(doc?.modRoleIds ?? []), inline: false }, { name: "🎵 DJ Roles", value: fmtRoles(doc?.djRoleIds ?? []), inline: false }, { name: "🔇 Mute Role", value: doc?.muteRoleId ? `<@&${doc.muteRoleId}>` : "None", inline: false }),
                ],
            });
            return;
        }
