...
const command = {
    ...
    slashData: (b) => b
        ...
        .addStringOption((o) => o.setName("word").setDescription("Word or phrase to block").setRequired(true).setMaxLength(50)))
        .addSubcommand((s) => s
        .setName("remove")
        ...
        .addSubcommand((s) => s.setName("list").setDescription("View all filtered words"))
        .addSubcommand((s) => s.setName("clear").setDescription("Remove all filtered words at once"))
        .addSubcommand((s) => s.setName("enable").setDescription("Enable the bad-word filter"))
        .addSubcommand((s) => s.setName("disable").setDescription("Disable the bad-word filter (words are kept for later)")),
    async execute(ctx) {
        ...
        if (sub === "add") {
            const word = (ctx.isSlash ? ctx.interaction.options.getString("word", true) : ctx.args.slice(1).join(" "))?.toLowerCase().trim();
            if (!word) {
                await ctx.reply({ embeds: [errorEmbed("Please provide a word to add.")] });
                return;
            }
            const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
            const existing = cfg?.automod?.badWords ?? [];
            ...
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $addToSet: { "automod.badWords": word }, $set: { "automod.enabled": true } }, { upsert: true });
        ...
        if (sub === "remove") {
            const word = (ctx.isSlash ? ctx.interaction.options.getString("word", true) : ctx.args.slice(1).join(" "))?.toLowerCase().trim();
            if (!word) {
                await ctx.reply({ embeds: [errorEmbed("Please provide a word to remove.")] });
                return;
            }
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $pull: { "automod.badWords": word } });
        ...
        if (sub === "clear") {
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { "automod.badWords": [] } });
            await ctx.reply({ embeds: [successEmbed("All filtered words have been cleared.")] });
            return;
        }
        if (sub === "enable" || sub === "disable") {
            const enabled = sub === "enable";
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { "automod.badWordFilterEnabled": enabled, "automod.enabled": true } }, { upsert: true });
...
//# sourceMappingURL=automodBadwords.js.map