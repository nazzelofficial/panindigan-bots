    ...
    async execute(ctx) {
        ...
        if (sub === "add") {
            ...
            if ((doc?.adminRoleIds ?? []).includes(role.id)) {
                await ctx.reply({ embeds: [infoEmbed(`${role} already has admin bot access.`)] });
                return;
            }
            ...
            if (!adminRoles.length) {
                await ctx.reply({ embeds: [infoEmbed("No admin roles configured. Members need the **Administrator** Discord permission by default.")] });
                return;
            }
            const embed = baseEmbed("primary")
                .setTitle("🔐 Admin Roles")
                .setDescription(adminRoles.map((id) => `<@&${id}>`).join("\n"))
