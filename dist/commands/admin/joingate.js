...
const command = {
    ...
    async execute(ctx) {
        ...
        if (sub === "setup") {
            ...
            if (days < 1 || days > 365) {
                await ctx.reply({ embeds: [errorEmbed("Minimum account age must be between 1 and 365 days.")] });
                return;
            }
            ...
            const doc = await GuildModel.findOne({ guildId: guild.id }).lean();
            const gate = doc?.joinGate;
            if (!gate?.enabled) {
                await ctx.reply({ embeds: [infoEmbed("Join gate is not configured. Use `/joingate setup [days]` to enable it.")] });
                return;
            }
            await ctx.reply({
                embeds: [
                    successEmbed(`🛡️ **Join Gate Status: Active**\nMinimum account age: **${gate.minAccountAgeDays} day${gate.minAccountAgeDays !== 1 ? "s" : ""}**\nKick message: "${gate.kickMessage}"`),
                ],
            });
        }
    },
