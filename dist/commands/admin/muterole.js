    ...
    async execute(ctx) {
        ...
        if (sub === "set") {
            ...
            if (existing?.muteRoleId && guild.roles.cache.has(existing.muteRoleId)) {
                await ctx.reply({ embeds: [infoEmbed(`Mute role is already set to <@&${existing.muteRoleId}>. Delete it first or use \`/muterole set\` to change it.`)] });
                return;
            }
            const role = await guild.roles.create({ name: "Muted", color: "#808080", reason: `Mute role created by ${ctx.userId}` });
            for (const [, channel] of guild.channels.cache) {
                if (channel.isTextBased()) {
                    await channel.permissionOverwrites.edit(role, { SendMessages: false, AddReactions: false, CreatePublicThreads: false }, { reason: "Mute role sync" }).catch(() => { });
                }
            }
            await GuildModel.findOneAndUpdate({ guildId: guild.id }, { muteRoleId: role.id }, { upsert: true });
            ...
            if (!doc?.muteRoleId) {
                await ctx.reply({ embeds: [errorEmbed("No mute role set. Use `/muterole set` or `/muterole create` first.")] });
                return;
            }
            const role = guild.roles.cache.get(doc.muteRoleId);
            if (!role) {
                await ctx.reply({ embeds: [errorEmbed("Mute role not found. It may have been deleted.")] });
                return;
            }
            ...
            for (const [, channel] of guild.channels.cache) {
                if (channel.isTextBased()) {
                    try {
                        await channel.permissionOverwrites.edit(role, { SendMessages: false, AddReactions: false, CreatePublicThreads: false }, { reason: "Mute role sync" });
                        synced++;
                    }
                    catch {
                        failed++;
            ...
            const doc = await GuildModel.findOne({ guildId: guild.id }).lean();
            if (!doc?.muteRoleId) {
                await ctx.reply({ embeds: [infoEmbed("No mute role configured. Use `/muterole create` or `/muterole set`.")] });
                return;
            }
