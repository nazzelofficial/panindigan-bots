...
const command = {
    ...
    slashData: (b) => b
        .addSubcommand((s) => s.setName("set").setDescription("Set which roles are allowed to use a command")
        ...
        .addSubcommand((s) => s.setName("clear").setDescription("Remove custom role restrictions from a command (revert to default access)")
        .addStringOption((o) => o.setName("command").setDescription("Command name").setRequired(true)))
    ...
    async execute(ctx) {
        ...
        if (sub === "set") {
            ...
            if (ctx.isSlash) {
                ...
                ctx.args.slice(2).forEach((a) => { const id = a.replace(/\D/g, ""); if (id)
                    roleIds.push(id); });
            }
            if (!roleIds.length) {
                await ctx.reply({ embeds: [errorEmbed("Provide at least one role.")] });
                return;
            }
            ...
            await ctx.reply({
                embeds: [
                    baseEmbed("primary")
                        ...
                        .setDescription(perms.map((p) => `**\`${p.command}\`** → ${p.roleIds.map((id) => `<@&${id}>`).join(", ")}`).join("\n")),
                ],
            });
            return;
        }
...
//# sourceMappingURL=commandperms.js.map