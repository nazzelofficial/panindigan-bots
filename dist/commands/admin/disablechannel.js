...
const command = {
    ...
    slashData: (b) => b
        .addChannelOption((o) => o.setName("channel").setDescription("Channel to restrict").setRequired(true))
        .addStringOption((o) => o.setName("command").setDescription("Command to disable in that channel").setRequired(true)),
    async execute(ctx) {
        ...
        const already = cfg?.disabledChannels?.some((d) => d.channelId === channelId && d.command === cmdName);
        if (already) {
            await ctx.reply({ embeds: [errorEmbed(`\`${cmdName}\` is already disabled in <#${channelId}>.`)] });
            return;
        }
        await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $push: { disabledChannels: { channelId, command: cmdName } } }, { upsert: true });
        await ctx.reply({ embeds: [successEmbed(`\`${cmdName}\` has been disabled in <#${channelId}>.`)] });
    },
};
export default command;
//# sourceMappingURL=disablechannel.js.map