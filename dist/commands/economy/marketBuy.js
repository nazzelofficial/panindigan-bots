...
const command = {
    ...
    async execute(ctx) {
        ...
        const buyerProfile = buyerUser.guilds.find((g) => g.guildId === guildId);
        const buyerBalance = buyerProfile ? buyerProfile.balance ?? 0 : 0;
        ...
        // Deduct from buyer
        if (buyerProfile) {
            buyerProfile.balance = buyerBalance - totalCost;
        }
        else {
            buyerUser.guilds.push({ guildId: guild.id, balance: -totalCost });
        }
        await buyerUser.save();
        // Pay seller
        const sellerUser = await UserModel.findOne({ userId: listing.sellerId });
        if (sellerUser) {
            const sellerProfile = sellerUser.guilds.find((g) => g.guildId === guildId);
            if (sellerProfile) {
                sellerProfile.balance = (sellerProfile.balance ?? 0) + totalCost;
            }
            else {
                sellerUser.guilds.push({ guildId: guild.id, balance: totalCost });
            }
        ...
        const embed = baseEmbed("success")
            ...
            .addFields({ name: "📦 Item", value: `${qty}x ${listing.item}`, inline: true }, { name: "💰 Total Paid", value: `🪙 ${totalCost.toLocaleString()}`, inline: true }, { name: "👤 Seller", value: `<@${listing.sellerId}>`, inline: true });
        await ctx.reply({ embeds: [embed] });
    },
};
export default command;
//# sourceMappingURL=marketBuy.js.map