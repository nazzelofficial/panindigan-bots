...
const command = {
    ...
    async execute(ctx) {
        ...
        if (winnerUser) {
            const winnerProfile = winnerUser.guilds.find((g) => g.guildId === guild.id);
            if (winnerProfile) {
                winnerProfile.balance = (winnerProfile.balance ?? 0) + prize;
                winnerProfile.lotteryTickets = 0;
            }
        }
        ...
        for (const user of users) {
            const profile = user.guilds?.find((g) => g.guildId === guild.id);
            if (profile) {
                await UserModel.updateOne({ userId: user.userId, "guilds.guildId": guild.id }, { $set: { "guilds.$.lotteryTickets": 0 } });
            }
        }
...
//# sourceMappingURL=lotteryDraw.js.map