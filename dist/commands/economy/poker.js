...
// ── Helpers ──────────────────────────────────────────────────────────────────
function handEmbed(cards, held, phase, bet, handName, payout, userId) {
    ...
    if (phase === "deal") {
        embed.setDescription("**Select cards to HOLD**, then press **Draw** to replace the rest.\nYou have **30 seconds** to decide.");
    }
    else {
...
const command = {
    ...
    guildOnly: true,
    slashData: (b) => b
        .addIntegerOption((o) => o.setName("bet").setDescription("Amount to bet (min 10)").setRequired(true).setMinValue(10).setMaxValue(100_000)),
    async execute(ctx) {
        ...
        if (profile.balance < bet) {
            return ctx.reply({
                embeds: [errorEmbed(`❌ You don't have enough coins. Your balance: **${(profile.balance ?? 0).toLocaleString()}** coins.`)],
            });
        }
        ...
        collector.on("end", async (_, reason) => {
            ...
            if (updatedProfile) {
                updatedProfile.balance = (updatedProfile.balance ?? 0) + winAmount;
                updatedProfile.totalGambledWon = (updatedProfile.totalGambledWon ?? 0) + winAmount;
                if (multiplier > 0)
                    updatedProfile.gamesWon = (updatedProfile.gamesWon ?? 0) + 1;
                else
                    updatedProfile.gamesLost = (updatedProfile.gamesLost ?? 0) + 1;
