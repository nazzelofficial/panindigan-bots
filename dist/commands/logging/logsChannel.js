...
const command = {
    ...
    slashData: (b) => b
        ...
        .addStringOption((o) => o.setName("event").setDescription("Event type (leave empty to set as the default for all)").setRequired(false)
    ...
    async execute(ctx) {
        ...
        await GuildModel.findOneAndUpdate({ guildId: guild.id }, { $set: { [`logging.channels.${event}`]: channelId, "logging.enabled": true } }, { upsert: true });
