...
const command = {
    ...
    cooldown: 10,
    aliases: ["level", "lvl"],
    slashData: (b) => b.addUserOption((o) => o.setName("user").setDescription("User to check (defaults to you)").setRequired(false)),
    async execute(ctx) {
        ...
        const xp = guildData.xp ?? 0;
        const level = guildData.level ?? 0;
        const prestige = guildData.prestige ?? 0;
        ...
        const rank = allUsers.filter((u) => {
            const gp = u.guilds?.find((g) => g.guildId === guild.id);
            return (gp?.xp ?? 0) > xp;
        }).length + 1;
        ...
        if (attachment) {
            ...
            const embed = baseEmbed("primary")
                ...
                .addFields({ name: "Rank", value: `#${rank}`, inline: true }, { name: "Level", value: String(level), inline: true }, { name: "Prestige", value: String(prestige), inline: true }, { name: "XP Progress", value: `${xpInLevel.toLocaleString()} / ${xpNeeded.toLocaleString()} XP\n[${bar}] ${percent}%`, inline: false }, { name: "Total XP", value: xp.toLocaleString(), inline: true });
