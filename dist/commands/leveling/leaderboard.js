...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!pageEntries.length) {
            await ctx.reply({ embeds: [errorEmbed("No entries on this page.")] });
            return;
        }
        const totalPages = Math.ceil(total / perPage);
        const userRank = entries.findIndex((e) => e.userId === ctx.userId) + 1;
        const medal = ["🥇", "🥈", "🥉"];
        const lines = await Promise.all(pageEntries.map(async (entry, idx) => {
            const position = skip + idx + 1;
            const prefix = position <= 3 ? medal[position - 1] : `**#${position}**`;
            let tag;
            try {
                tag = (await ctx.client.users.fetch(entry.userId)).username;
            }
            catch {
                tag = `<@${entry.userId}>`;
            }
            if (type === "prestige")
                return `${prefix} ${tag} — Prestige **${entry.prestige}** · Lvl **${entry.level}**`;
            if (type === "level")
                return `${prefix} ${tag} — Level **${entry.level}** · **${entry.xp.toLocaleString()}** XP`;
