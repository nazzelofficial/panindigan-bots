...
const command = {
    ...
    async execute(ctx) {
        ...
        const embed = baseEmbed("primary")
            ...
            .addFields({ name: "🆔 User ID", value: target.id, inline: true }, { name: "🤖 Bot", value: target.bot ? "Yes" : "No", inline: true }, { name: "📅 Account Created", value: `<t:${createdAt}:F> (<t:${createdAt}:R>)`, inline: false });
        if (member) {
            embed.addFields({ name: "📅 Server Joined", value: joinedAt ? `<t:${joinedAt}:F> (<t:${joinedAt}:R>)` : "Unknown", inline: false }, { name: "📛 Nickname", value: member.nickname ?? "None", inline: true }, { name: "🎨 Highest Role", value: member.roles.highest.toString(), inline: true }, { name: `🏷️ Roles (${Math.min(roles.length, 10)})`, value: roles.length ? roles.join(", ").slice(0, 1000) : "None", inline: false });
