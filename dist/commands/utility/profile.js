...
const command = {
    ...
    slashData: (b) => b.addUserOption((o) => o.setName("user").setDescription("User na titingnan (default: ikaw)").setRequired(false)),
    async execute(ctx) {
        ...
        if (!member) {
            await ctx.reply({ embeds: [baseEmbed("danger").setDescription("Member not found in this server.")] });
            return;
        }
        const user = await UserModel.findOneAndUpdate({ userId: targetId }, { $setOnInsert: { userId: targetId } }, { upsert: true, new: true });
        ...
        const embed = baseEmbed("primary")
            ...
            .addFields({ name: "📛 Username", value: member.user.username, inline: true }, { name: "🆔 ID", value: member.id, inline: true }, { name: "🤖 Bot", value: member.user.bot ? "Yes" : "No", inline: true }, { name: "📅 Joined Server", value: joinedAt, inline: true }, { name: "📅 Account Created", value: createdAt, inline: true }, { name: "💼 Job", value: job ? job.name : "Unemployed", inline: true }, { name: "⭐ Level", value: `**${level}** (${xp}/${nextXp} XP)\n${xpBar}`, inline: false }, { name: "🪙 Wallet", value: balance.toLocaleString(), inline: true }, { name: "🏦 Bank", value: bank.toLocaleString(), inline: true }, { name: "⚠️ Warnings", value: String(warnCount), inline: true }, { name: "💬 Messages", value: String(profile.messageCount ?? 0), inline: true }, { name: "🔥 Daily Streak", value: String(profile.dailyStreak ?? 0), inline: true }, { name: "🏅 Prestige", value: String(profile.prestige ?? 0), inline: true });
        if (roles.length) {
