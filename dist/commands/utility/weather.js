...
const command = {
    ...
    async execute(ctx) {
        ...
        if (!location) {
            await ctx.reply({ embeds: [errorEmbed("Provide a location.")] });
            return;
        }
        ...
        if (!geoRes?.ok) {
            await ctx.reply({ embeds: [errorEmbed("Geocoding service unavailable.")] });
            return;
        }
        const geoData = await geoRes.json();
        const place = geoData?.results?.[0];
        if (!place) {
            await ctx.reply({ embeds: [infoEmbed(`No results found for **${location}**.`)] });
            return;
        }
        // Fetch weather
        const wxRes = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${place.latitude}&longitude=${place.longitude}` +
            `&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weather_code,apparent_temperature` +
            `&wind_speed_unit=kmh&temperature_unit=celsius&timezone=auto`).catch(() => null);
        if (!wxRes?.ok) {
            await ctx.reply({ embeds: [errorEmbed("Weather service unavailable.")] });
            return;
        }
        ...
        const embed = baseEmbed("info")
            ...
            .addFields({ name: "🌡️ Temperature", value: `**${curr?.temperature_2m ?? "?"}°C** (feels like ${curr?.apparent_temperature ?? "?"}°C)`, inline: true }, { name: "💧 Humidity", value: `${curr?.relative_humidity_2m ?? "?"}%`, inline: true }, { name: "💨 Wind Speed", value: `${curr?.wind_speed_10m ?? "?"} km/h`, inline: true }, { name: "📍 Coordinates", value: `${place.latitude.toFixed(2)}, ${place.longitude.toFixed(2)}`, inline: true }, { name: "🕐 Timezone", value: wxData?.timezone ?? "Unknown", inline: true })
