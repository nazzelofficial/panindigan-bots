...
const command = {
    ...
    slashData: (b) => b
        ...
        .addSubcommand((s) => s.setName("create").setDescription("Create bagong invite link")
        .addChannelOption((o) => o.setName("channel").setDescription("Channel para sa invite (default: ito)").setRequired(false))
        .addIntegerOption((o) => o.setName("expires").setDescription("Expires in (seconds, 0 = never)").setRequired(false)
        .addChoices({ name: "1 hour", value: 3600 }, { name: "6 hours", value: 21600 }, { name: "12 hours", value: 43200 }, { name: "1 day", value: 86400 }, { name: "7 days", value: 604800 }, { name: "Never", value: 0 }))
        ...
        .addSubcommand((s) => s.setName("top").setDescription("View mga miyembro na may pinakamaraming na-invite"))
        .addSubcommand((s) => s.setName("delete").setDescription("Burahin ang isang invite code (requires Manage Guild)")
    ...
    async execute(ctx) {
        ...
        if (sub === "list") {
            if (!guild.members.me?.permissions.has(PermissionFlagsBits.ManageGuild)) {
                await ctx.reply({ embeds: [errorEmbed("Kailangan ko ng **Manage Guild** permission para makita ang invites.")] });
                return;
            }
            if (ctx.isSlash)
            ...
            if (!invites) {
                await ctx.reply({ embeds: [errorEmbed("Hindi ma-fetch ang invites.")] });
                return;
            }
            if (!invites.size) {
                await ctx.reply({ embeds: [baseEmbed("primary").setTitle("📨 Server Invites").setDescription("No active invites.")] });
                return;
            }
            const sorted = [...invites.values()].sort((a, b) => (b.uses ?? 0) - (a.uses ?? 0)).slice(0, 20);
            const embed = baseEmbed("primary")
                .setTitle(`📨 Server Invites (${invites.size})`)
                .setDescription(sorted.map((inv) => {
        ...
        if (sub === "create") {
            ...
            const channel = chOpt ? guild.channels.cache.get(chOpt.id) : (ctx.interaction?.channel ?? ctx.message?.channel);
            if (!channel) {
            ...
            const invite = await channel.createInvite({ maxAge, maxUses, reason: `Created by ${ctx.userId}` }).catch(() => null);
            if (!invite) {
                await ctx.reply({ embeds: [errorEmbed("Hindi magawa ang invite. Siguraduhing may Create Invite permission ako sa channel.")] });
                return;
            }
        ...
        if (sub === "top") {
            if (!guild.members.me?.permissions.has(PermissionFlagsBits.ManageGuild)) {
                await ctx.reply({ embeds: [errorEmbed("Kailangan ko ng **Manage Guild** permission.")] });
                return;
            }
            if (ctx.isSlash)
                await ctx.interaction.deferReply();
            const invites = await guild.invites.fetch().catch(() => null);
            if (!invites?.size) {
                await ctx.reply({ embeds: [baseEmbed("primary").setTitle("📨 Top Inviters").setDescription("No invite data available.")] });
                return;
            }
            const inviters = new Map();
            for (const inv of invites.values()) {
                if (inv.inviterId)
                    inviters.set(inv.inviterId, (inviters.get(inv.inviterId) ?? 0) + (inv.uses ?? 0));
            }
        ...
        if (sub === "delete") {
            ...
            if (!code) {
                await ctx.reply({ embeds: [errorEmbed("Provide a invite code.")] });
                return;
            }
