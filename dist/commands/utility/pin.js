...
const command = {
    ...
    slashData: (b) => b
        .addSubcommand((s) => s.setName("add").setDescription("I-pin ang isang message")
        .addStringOption((o) => o.setName("messageid").setDescription("ID ng message na ipe-pin").setRequired(true)))
        .addSubcommand((s) => s.setName("remove").setDescription("I-unpin ang isang message")
    ...
    async execute(ctx) {
        ...
        if (sub === "add" || sub === "remove") {
            ...
            if (!msgId) {
                await ctx.reply({ embeds: [errorEmbed("Provide a Message ID.")] });
                return;
            }
            const msg = await channel.messages.fetch(msgId).catch(() => null);
            if (!msg) {
                await ctx.reply({ embeds: [errorEmbed("Message not found. Make sure the ID is correct and the message is in this channel.")] });
                return;
            }
            if (sub === "add") {
                if (msg.pinned) {
                    await ctx.reply({ embeds: [errorEmbed("Naka-pin na ang message na iyan.")] });
                    return;
                }
                await msg.pin();
                ...
                if (!msg.pinned) {
                    await ctx.reply({ embeds: [errorEmbed("That message is not pinned.")] });
                    return;
                }
                await msg.unpin();
        ...
        if (sub === "list") {
            ...
            if (!pins) {
                await ctx.reply({ embeds: [errorEmbed("Hindi ma-fetch ang pinned messages.")] });
                return;
            }
            if (!pins.size) {
                await ctx.reply({ embeds: [baseEmbed("primary").setTitle("📌 Pinned Messages").setDescription("No pinned messages in this channel.")] });
                return;
            }
            const embed = baseEmbed("primary")
                .setTitle(`📌 Pinned Messages (${pins.size})`)
                .setDescription([...pins.values()].slice(0, 10).map((m) => `• [Message](${m.url}) ni **${m.author.username}** (<t:${Math.floor(m.createdTimestamp / 1000)}:R>)\n  *${m.content?.slice(0, 80) || "[embed/attachment]"}*`).join("\n\n"));
