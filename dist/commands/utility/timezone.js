...
const command = {
    ...
    slashData: (b) => b
        .addSubcommand((s) => s.setName("city").setDescription("View oras sa isang lungsod")
        .addStringOption((o) => o.setName("city").setDescription("Lungsod").setRequired(true)
        .addChoices(...Object.keys(TIMEZONES).map((c) => ({ name: c, value: c })))))
        .addSubcommand((s) => s.setName("list").setDescription("View all available na timezone"))
        .addSubcommand((s) => s.setName("convert").setDescription("I-convert ang oras mula sa isang timezone papunta sa isa pa")
        .addStringOption((o) => o.setName("time").setDescription("Oras na ico-convert (e.g. 18:00)").setRequired(true))
        .addStringOption((o) => o.setName("from").setDescription("Source timezone").setRequired(true)
        .addChoices(...Object.keys(TIMEZONES).map((c) => ({ name: c, value: c }))))
        .addStringOption((o) => o.setName("to").setDescription("Target timezone").setRequired(true)
        .addChoices(...Object.keys(TIMEZONES).map((c) => ({ name: c, value: c }))))),
    async execute(ctx) {
        ...
        if (sub === "city") {
            ...
            if (!city || !TIMEZONES[city]) {
                await ctx.reply({ embeds: [errorEmbed(`Unknown city. Use \`/timezone list\` para makita ang available na cities.`)] });
                return;
            }
            ...
            await ctx.reply({
                embeds: [
                    baseEmbed("primary")
                        .setTitle(`🕐 ${city}`)
                        .addFields({ name: "Time", value: `**${time}**`, inline: true }, { name: "Date", value: date, inline: true }, { name: "Timezone", value: `\`${tz}\``, inline: true }),
                ],
            });
            return;
        }
        if (sub === "convert") {
            ...
            if (!timeStr || !fromCity || !toCity) {
                await ctx.reply({ embeds: [errorEmbed("Provide a time, from, at to timezone.")] });
                return;
            }
            if (!TIMEZONES[fromCity] || !TIMEZONES[toCity]) {
                await ctx.reply({ embeds: [errorEmbed("Unknown city.")] });
                return;
            }
            const [hours, minutes] = timeStr.split(":").map(Number);
            if (isNaN(hours) || isNaN(minutes)) {
                await ctx.reply({ embeds: [errorEmbed("Invalid na oras. Use format: `18:00`")] });
                return;
            }
            ...
            await ctx.reply({
                embeds: [
                    baseEmbed("primary")
                        ...
                        .addFields({ name: `From: ${fromCity}`, value: `**${timeStr}**`, inline: true }, { name: `To: ${toCity}`, value: `**${converted}** (${convertedDate})`, inline: true }),
                ],
            });
            return;
        }
