...
const command = {
    ...
    async execute(ctx) {
        ...
        const votes = { a: 0, b: 0 };
        const voted = new Set();
        const buildEmbed = () => baseEmbed("primary")
            .setTitle("🤔 Would You Rather…")
            .addFields({ name: "🅰️ Option A", value: optA, inline: true }, { name: "🅱️ Option B", value: optB, inline: true }, { name: "Votes", value: `🅰️ **${votes.a}** vs 🅱️ **${votes.b}**`, inline: false })
            .setFooter({ text: "Vote with the buttons below! Closes in 60 seconds." });
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`wyr:${id}:a`).setLabel("🅰️ " + optA.slice(0, 40)).setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`wyr:${id}:b`).setLabel("🅱️ " + optB.slice(0, 40)).setStyle(ButtonStyle.Secondary));
        ...
        setTimeout(async () => {
            ...
            const finalEmbed = baseEmbed("success")
                ...
                .addFields({ name: "🅰️ " + optA, value: `${votes.a} vote(s)`, inline: true }, { name: "🅱️ " + optB, value: `${votes.b} vote(s)`, inline: true }, { name: "Winner", value: winner, inline: false });
            const disabledRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`wyr:${id}:a:d`).setLabel("🅰️ " + optA.slice(0, 40)).setStyle(ButtonStyle.Primary).setDisabled(true), new ButtonBuilder().setCustomId(`wyr:${id}:b:d`).setLabel("🅱️ " + optB.slice(0, 40)).setStyle(ButtonStyle.Secondary).setDisabled(true));
            // Try to edit the original reply
            if (ctx.isSlash)
                ctx.interaction.editReply({ embeds: [finalEmbed], components: [disabledRow] }).catch(() => { });
        }, 60_000);
    },
};
