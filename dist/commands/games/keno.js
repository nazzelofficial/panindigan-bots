...
const command = {
    ...
    slashData: (b) => b
        ...
        .addStringOption(o => o.setName('numbers').setDescription('Your picks, space-separated (e.g. "5 12 33 67 80")').setRequired(true)),
    async execute(ctx) {
        ...
        const embed = new EmbedBuilder()
            .setTitle('🎰 Keno')
            .setColor(color)
            .addFields({ name: `Your Picks (${picks.length})`, value: picksDisplay, inline: false }, { name: '20 Numbers Drawn', value: drawnDisplay, inline: false }, { name: '✅ Matches', value: `${matchCount}/${picks.length}`, inline: true }, { name: '💰 Result', value: payout > 0 ? `+${(payout - bet).toLocaleString()} coins` : `-${bet.toLocaleString()} coins`, inline: true })
