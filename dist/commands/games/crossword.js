...
const command = {
    ...
    async execute(ctx) {
        ...
        const row = new ActionRowBuilder().addComponents(options.map((opt, i) => new ButtonBuilder()
            .setCustomId(`ans_${i}`)
            .setLabel(opt)
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(0x5865F2)
            .addFields({ name: '📖 Clue', value: clue, inline: false }, { name: '💡 Hint', value: `\`${hint}\``, inline: false })
        ...
        collector.on('collect', async (btn) => {
            ...
            const resultEmbed = new EmbedBuilder()
                ...
                .addFields({ name: '📖 Clue', value: clue, inline: false }, { name: '🎯 Correct Answer', value: `**${answer}**`, inline: true }, { name: '🎮 Your Answer', value: `**${options[chosen]}**`, inline: true })
                .setTimestamp();
                ...
                await msg.edit({
                    embeds: [
                        new EmbedBuilder()
                            .setTitle('⏰ Time\'s Up!')
                            .setColor(0xFF4444)
                            .addFields({ name: '📖 Clue', value: clue }, { name: '✅ The answer was', value: `**${answer}**` }),
                    ],
                    components: [],
                }).catch(() => { });
            }
