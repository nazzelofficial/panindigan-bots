...
const command = {
    ...
    slashData: (b) => b
        .addSubcommand((s) => s.setName("set").setDescription("Set the goodbye message")
    ...
    async execute(ctx) {
        ...
        if (sub === "set") {
            ...
            const msg = cfg?.goodbye?.message ?? "{user} has left {server}. We now have {memberCount} members.";
