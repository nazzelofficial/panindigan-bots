...
const command = {
    ...
    async execute(ctx) {
        ...
        if (sub === "setup") {
            ...
            if (!channel?.isTextBased()) {
                await ctx.reply({ embeds: [errorEmbed("Configured welcome channel not found.")] });
                return;
            }
            ...
            if (welcomeCfg.cardEnabled) {
                try {
                    ...
                    c.fillText(`Welcome, ${user.username.slice(0, 20)}!`, 240, 100);
                    c.fillStyle = "#a0a0b0";
                    c.font = "18px Sans";
                    c.fillText(`You are member #${guild.memberCount}`, 240, 140);
                    c.fillStyle = "#c0c0d0";
                    c.font = "16px Sans";
            ...
            const update = {};
            if (channel?.isTextBased?.())
                update["goodbye.channelId"] = channel.id;
            if (msg)
                update["goodbye.message"] = msg;
