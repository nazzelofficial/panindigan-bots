...
const command = {
    ...
    cooldown: 5,
    slashData: (b) => b
        .addSubcommand((s) => s.setName("set").setDescription("Set a new prefix (Admin only)")
    ...
    async execute(ctx) {
        ...
        if (sub === "set") {
            ...
            if (!newPrefix) {
                await ctx.reply({ embeds: [errorEmbed("Please provide a prefix.")] });
                return;
            }
            if (newPrefix.length > 5) {
                await ctx.reply({ embeds: [errorEmbed("Prefix must be 5 characters or fewer.")] });
                return;
            }
            ...
            await ctx.reply({ embeds: [successEmbed(`Prefix reset to the default: \`${config.prefix ?? "p!"}\`.`)] });
        }
        else {
            const cfg = await GuildModel.findOne({ guildId: guild.id }).lean();
            const current = cfg?.prefix ?? config.prefix ?? "p!";
            const embed = baseEmbed("primary")
                .setTitle("⚙️ Server Prefix")
                .addFields({ name: "Current Prefix", value: `\`${current}\``, inline: true }, { name: "Slash Commands", value: "Always use `/`", inline: true })
