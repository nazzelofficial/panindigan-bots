...
export const GuildRepository = {
    ...
    async findById(guildId) {
        const cached = botCache.get(cacheKey(guildId));
        if (cached)
            return cached;
        const t0 = Date.now();
        const doc = await withRetry(() => GuildModel.findOne({ guildId }).lean().exec(), { label: `guild-find:${guildId}` });
    ...
    async update(guildId, patch) {
        ...
        if (patch.prefix !== undefined)
            raw["prefix"] = patch.prefix;
        if (patch.language !== undefined)
            raw["language"] = patch.language;
        if (patch.premiumTier !== undefined)
            raw["premiumTier"] = patch.premiumTier;
        if (patch.adminRoleIds !== undefined)
            raw["adminRoles"] = patch.adminRoleIds;
        if (patch.modRoleIds !== undefined)
            raw["modRoles"] = patch.modRoleIds;
        if (patch.muteRoleId !== undefined)
            raw["muteRole"] = patch.muteRoleId;
        if (patch.logChannelId !== undefined)
            raw["logChannel"] = patch.logChannelId;
        if (patch.welcomeChannelId !== undefined)
            raw["welcomeChannel"] = patch.welcomeChannelId;
        if (patch.goodbyeChannelId !== undefined)
            raw["goodbyeChannel"] = patch.goodbyeChannelId;
        const doc = await withRetry(() => GuildModel.findOneAndUpdate({ guildId }, { $set: { ...raw, updatedAt: new Date() } }, { new: true, upsert: true, lean: true }).exec(), { label: `guild-update:${guildId}` });
        if (!doc)
            throw new DatabaseError(`Failed to update guild ${guildId}`, "findOneAndUpdate");
        const dto = toDTO(doc);
        botCache.set(cacheKey(guildId), dto, TTL);
    ...
    async upsert(guildId, data) {
        const doc = await withRetry(() => GuildModel.findOneAndUpdate({ guildId }, { $setOnInsert: { guildId }, $set: { updatedAt: new Date(), ...data } }, { new: true, upsert: true, lean: true }).exec(), { label: `guild-upsert:${guildId}` });
        if (!doc)
            throw new DatabaseError(`Failed to upsert guild ${guildId}`, "upsert");
        const dto = toDTO(doc);
        botCache.set(cacheKey(guildId), dto, TTL);
        return dto;
    },
    ...
    async delete(guildId) {
        await withRetry(() => GuildModel.deleteOne({ guildId }).exec(), { label: `guild-delete:${guildId}` });
