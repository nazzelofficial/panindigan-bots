 ...
 */
export function getConversationMemory(userId, guildId) {
 ...
 */
export function addMessageToMemory(userId, role, content, guildId) {
 ...
 */
export function clearConversationMemory(userId, guildId) {
    const key = getCacheKey(userId, guildId);
 ...
 */
export function setConversationPersona(userId, persona, guildId) {
    const memory = getConversationMemory(userId, guildId);
...
// ── Response caching ───────────────────────────────────────────────────────────
function generateCacheKey(prompt, model, persona) {
 ...
 */
export function getCachedResponse(prompt, model, persona) {
 ...
 * Cache a response.
 */
export function cacheResponse(prompt, response, model, persona) {
    const key = generateCacheKey(prompt, model, persona);
...
export async function completeChat(options) {
    const { messages, maxTokens = 1024, temperature = 0.7, stream = false, persona, useCache = true, } = options;
    ...
    for (let attempt = 1; attempt <= RETRY.MAX_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            const completion = await withRecovery(`ai:${currentProvider.name}`, async () => {
 ...
 */
export async function* completeChatStream(options) {
    const { messages, maxTokens = 1024, temperature = 0.7, persona, } = options;
    const provider = getActiveProvider();
    if (!provider) {
        throw new Error("No AI provider available");
    }
 ...
 */
export async function startTyping(channel, durationMs = 5000) {
 ...
 */
export function stopTyping(interval) {
