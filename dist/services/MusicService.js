...
export class MusicService {
     ...
     */
    static validateMusicOperation(client) {
     ...
     */
    static async getOrCreatePlayer(client, guildId, voiceChannelId, textChannelId) {
     ...
     * Search for tracks
     */
    static async searchTracks(player, query, botUserId) {
        try {
     ...
     */
    static async loadPlaylist(player, result, userId, isSlash, interaction, message, botMessage) {
     ...
     */
    static async loadTrack(player, track, userId, isSlash, interaction, message, botMessage) {
        track.requester = userId;
        player.queue.add(track);
        if (!player.playing) {
            await player.play().catch(() => { });
     ...
     */
    static async sendResponse(isSlash, embed, interaction, message, botMessage) {
     ...
     * Set volume
     */
    static async setVolume(player, volume) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
     ...
     * Clear queue
     */
    static async clearQueue(player) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
     ...
     * Shuffle queue
     */
    static async shuffleQueue(player) {
     ...
     */
    static async disconnect(player) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
        player.queue.clear();
     ...
     * Add track to queue
     */
    static async addToQueue(player, track) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
        player.queue.add(track);
     ...
     */
    static async removeFromQueue(player, index) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
        const tracks = player.queue?.tracks ?? [];
     ...
     * Move track in queue
     */
    static async moveTrackInQueue(player, fromIndex, toIndex) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
        const tracks = player.queue?.tracks ?? [];
     ...
     * Set loop mode
     */
    static async setLoopMode(player, mode) {
        if (!player) {
     ...
     */
    static async seek(player, positionMs) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
     ...
     * Play previous track
     */
    static async playPrevious(player) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
        if (!player.playing && !player.paused) {
     ...
     */
    static async skipTo(player, position) {
        if (!player) {
            return { success: false, message: "No active player." };
        }
        const tracks = player.queue?.tracks ?? [];
